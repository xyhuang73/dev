...
STRATEGY_ID = "DUOYU009878"
STRATEGY_NAME = "多雨定制质量低波动异象策略"
SOURCE_FACTOR_ID = "DUOYU009878"
CATEGORY = "低波动异象"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
RISK_NOTE_CN = "低波异象在流动性驱动牛市中表现弱。 质量风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 低波动异象_v6 | style: 质量 | seed: 20300315
