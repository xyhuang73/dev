...
STRATEGY_ID = "DUOYU004076"
STRATEGY_NAME = "多雨定制持有行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004076"
CATEGORY = "行业资金轮动动量"
STYLE = "持有"
OPERATOR_ID = "行业资金轮动动量_v4"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 4
FAST_WINDOW = 7
SLOW_WINDOW = 35
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    sector_relative = _cs_rank(_zscore(net_flow, 12)) - _cs_rank(_zscore(net_flow, 23))
    raw = 0.33 * VARIANT_WEIGHT * sector_relative + _ret(close, 12)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 行业资金轮动动量_v4 | style: 持有 | seed: 20262113
