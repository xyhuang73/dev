...
STRATEGY_ID = "DUOYU005937"
STRATEGY_NAME = "多雨定制动量隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU005937"
CATEGORY = "隐含波动率偏斜"
STYLE = "动量"
OPERATOR_ID = "隐含波动率偏斜_v1"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 16
FAST_WINDOW = 12
SLOW_WINDOW = 16
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["close", "option_implied_vol"]

ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "多数 标的 没有活跃期权市场。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 16 个交易日。", "先计算 隐含波动率偏斜 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU005937 - 多雨定制动量隐含波动率偏斜策略

策略主题：隐含波动率偏斜 / 动量
适用范围：宽基/行业
主要周期：16 个交易日
必需字段：close, option_implied_vol

经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    iv = _field(panel, "option_implied_vol") if "option_implied_vol" in panel.columns else _downside_vol(close, 19)
    iv_change = _diff(iv, 11)
    raw = -0.28 * VARIANT_WEIGHT * _zscore(iv_change, 11) + _ret(close, 19)
    raw = raw * _max_dd_guard(drawdown, -0.12)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 隐含波动率偏斜_v1 | style: 动量 | seed: 20273210
