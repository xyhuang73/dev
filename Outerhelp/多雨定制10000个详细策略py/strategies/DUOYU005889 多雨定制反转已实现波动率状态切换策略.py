...
STRATEGY_ID = "DUOYU005889"
STRATEGY_NAME = "多雨定制反转已实现波动率状态切换策略"
SOURCE_FACTOR_ID = "DUOYU005889"
CATEGORY = "已实现波动率状态切换"
STYLE = "反转"
OPERATOR_ID = "已实现波动率状态切换_v7"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
"""
DUOYU005889 - 多雨定制反转已实现波动率状态切换策略
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    vol_regime = _vol_regime(close, 5, 15)
    trend_term = _ret(close, 15) * _sigmoid_gate(vol_regime, 0, 3)
    rev_term = -_ret(close, 5) * (1.0 - _sigmoid_gate(vol_regime, 0, 3))
    raw = np.where(vol_regime > 0, trend_term + 0.1 * VARIANT_WEIGHT * _amount_z(panel, 5),
                   rev_term + 0.1 * VARIANT_WEIGHT * _amount_z(panel, 5))
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 已实现波动率状态切换_v7 | style: 反转 | seed: 20271416
