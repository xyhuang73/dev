...
STRATEGY_ID = "DUOYU007464"
STRATEGY_NAME = "多雨定制修复政策事件评分策略"
SOURCE_FACTOR_ID = "DUOYU007464"
CATEGORY = "政策事件评分"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 政策事件评分_v6 | style: 修复 | seed: 20280915
