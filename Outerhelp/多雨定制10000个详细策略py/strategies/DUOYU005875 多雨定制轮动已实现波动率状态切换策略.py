...
STRATEGY_ID = "DUOYU005875"
STRATEGY_NAME = "多雨定制轮动已实现波动率状态切换策略"
SOURCE_FACTOR_ID = "DUOYU005875"
CATEGORY = "已实现波动率状态切换"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "状态判断滞后可能导致切换在错误时机。 轮动风格下回撤控制较为严格。"
...
"""
DUOYU005875 - 多雨定制轮动已实现波动率状态切换策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 已实现波动率状态切换_v6 | style: 轮动 | seed: 20272015
