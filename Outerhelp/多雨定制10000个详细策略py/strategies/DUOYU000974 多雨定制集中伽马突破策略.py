...
STRATEGY_ID = "DUOYU000974"
STRATEGY_NAME = "多雨定制集中伽马突破策略"
SOURCE_FACTOR_ID = "DUOYU000974"
CATEGORY = "伽马突破"
STYLE = "集中"
OPERATOR_ID = "伽马突破_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    vol_compression = 1.0 - _cs_rank(_zscore(true_range, 16).abs()).abs()
    breakout = _breakout(panel, 11)
    raw = breakout * vol_compression * 0.76 * VARIANT_WEIGHT + 0.19 * VARIANT_WEIGHT * _amount_z(panel, 11)
    raw = raw * _sigmoid_gate(_amount_z(panel, 11), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 伽马突破_v5 | style: 集中 | seed: 20245914
