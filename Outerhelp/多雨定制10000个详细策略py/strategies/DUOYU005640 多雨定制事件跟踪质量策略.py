...
STRATEGY_ID = "DUOYU005640"
STRATEGY_NAME = "多雨定制事件跟踪质量策略"
SOURCE_FACTOR_ID = "DUOYU005640"
CATEGORY = "跟踪质量"
STYLE = "事件"
OPERATOR_ID = "跟踪质量_v2"
MARKET_SCOPE = "宽基/行业"
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "跟踪误差数据频率可能低于价格数据。 事件风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 17 个交易日。", "先计算 跟踪质量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU005640 - 多雨定制事件跟踪质量策略
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _ret(index_close, 25) - 0.35 * VARIANT_WEIGHT * _zscore(tracking_error, 25).abs() - 0.19 * VARIANT_WEIGHT * _zscore(tracking_diff, 25).abs()
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 跟踪质量_v2 | style: 事件 | seed: 20271511
