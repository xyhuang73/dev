...
STRATEGY_ID = "DUOYU008500"
STRATEGY_NAME = "多雨定制事件成份广度扩散策略"
SOURCE_FACTOR_ID = "DUOYU008500"
CATEGORY = "成份广度扩散"
STYLE = "事件"
OPERATOR_ID = "成份广度扩散_v1"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 16
FAST_WINDOW = 14
SLOW_WINDOW = 15
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.54 * VARIANT_WEIGHT * _zscore(up_ratio, 13) + 0.39 * VARIANT_WEIGHT * _zscore(new_high_ratio, 17) - _zscore(dispersion, 17).abs()
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 成份广度扩散_v1 | style: 事件 | seed: 20290510
