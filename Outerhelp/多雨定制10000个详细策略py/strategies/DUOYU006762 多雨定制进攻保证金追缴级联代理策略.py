...
STRATEGY_ID = "DUOYU006762"
STRATEGY_NAME = "多雨定制进攻保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006762"
CATEGORY = "保证金追缴级联代理"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 19 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
RISK_NOTE_CN = "保证金数据频率低，代理变量精度有限。 进攻风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 19 个交易日。", "先计算 保证金追缴级联代理 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU006762 - 多雨定制进攻保证金追缴级联代理策略
...
经济逻辑：
该策略以约 19 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 保证金追缴级联代理_v3 | style: 进攻 | seed: 20276712
