...
STRATEGY_ID = "DUOYU007531"
STRATEGY_NAME = "多雨定制过滤解禁到期压力策略"
SOURCE_FACTOR_ID = "DUOYU007531"
CATEGORY = "解禁到期压力"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：解禁比例大+临近解禁日=抛售压力大，解禁过后压力衰减。逆向思维：提前规避。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：解禁比例大+临近解禁日=抛售压力大，解禁过后压力衰减。逆向思维：提前规避。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 解禁到期压力_v2 | style: 过滤 | seed: 20282611
