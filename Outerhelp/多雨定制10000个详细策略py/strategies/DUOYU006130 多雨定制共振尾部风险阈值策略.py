...
STRATEGY_ID = "DUOYU006130"
STRATEGY_NAME = "多雨定制共振尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006130"
CATEGORY = "尾部风险阈值"
STYLE = "共振"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 尾部风险阈值_v3 | style: 共振 | seed: 20273512
