...
STRATEGY_ID = "DUOYU005780"
STRATEGY_NAME = "多雨定制事件已实现波动率状态切换策略"
SOURCE_FACTOR_ID = "DUOYU005780"
CATEGORY = "已实现波动率状态切换"
STYLE = "事件"
OPERATOR_ID = "已实现波动率状态切换_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
"""
DUOYU005780 - 多雨定制事件已实现波动率状态切换策略
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    vol_regime = _vol_regime(close, 6, 18)
    trend_term = _ret(close, 18) * _sigmoid_gate(vol_regime, 0, 3)
    rev_term = -_ret(close, 6) * (1.0 - _sigmoid_gate(vol_regime, 0, 3))
    raw = np.where(vol_regime > 0, trend_term + 0.11 * VARIANT_WEIGHT * _amount_z(panel, 6),
                   rev_term + 0.11 * VARIANT_WEIGHT * _amount_z(panel, 6))
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 已实现波动率状态切换_v1 | style: 事件 | seed: 20272510
