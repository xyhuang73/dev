...
STRATEGY_ID = "DUOYU009822"
STRATEGY_NAME = "多雨定制进攻低波动异象策略"
SOURCE_FACTOR_ID = "DUOYU009822"
CATEGORY = "低波动异象"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 低波动异象_v4 | style: 进攻 | seed: 20298713
