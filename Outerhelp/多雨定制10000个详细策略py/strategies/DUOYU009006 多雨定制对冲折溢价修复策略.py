...
STRATEGY_ID = "DUOYU009006"
STRATEGY_NAME = "多雨定制对冲折溢价修复策略"
SOURCE_FACTOR_ID = "DUOYU009006"
CATEGORY = "折溢价修复"
STYLE = "对冲"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。"
RISK_NOTE_CN = "跨境 标的 折溢价受汇率和申赎机制影响。 对冲风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 折溢价修复_v6 | style: 对冲 | seed: 20292115
