...
STRATEGY_ID = "DUOYU005030"
STRATEGY_NAME = "多雨定制共振Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU005030"
CATEGORY = "Amihud 非流动性"
STYLE = "共振"
OPERATOR_ID = "amihud_非流动性_v7"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 10
FAST_WINDOW = 14
SLOW_WINDOW = 19
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
"""
DUOYU005030 - 多雨定制共振Amihud 非流动性策略
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    amihud = _zscore(ret_abs / (np.log1p(amount.clip(lower=0)) + EPS), 19)
    raw = -amihud + 0.3 * VARIANT_WEIGHT * _amount_z(panel, 14)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: amihud_非流动性_v7 | style: 共振 | seed: 20266516
