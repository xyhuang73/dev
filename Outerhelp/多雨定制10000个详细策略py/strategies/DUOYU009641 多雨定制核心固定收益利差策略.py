...
STRATEGY_ID = "DUOYU009641"
STRATEGY_NAME = "多雨定制核心固定收益利差策略"
SOURCE_FACTOR_ID = "DUOYU009641"
CATEGORY = "固定收益利差"
STYLE = "核心"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 固定收益利差_v1 | style: 核心 | seed: 20297610
