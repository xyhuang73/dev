...
STRATEGY_ID = "DUOYU006309"
STRATEGY_NAME = "多雨定制反转相关性破裂检测策略"
SOURCE_FACTOR_ID = "DUOYU006309"
CATEGORY = "相关性破裂检测"
STYLE = "反转"
OPERATOR_ID = "相关性破裂检测_v4"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 16
FAST_WINDOW = 11
SLOW_WINDOW = 21
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    beta = _field(panel, "beta_to_market") if "beta_to_market" in panel.columns else _zscore(_ret(close, 24) / (_ret(index_close, 24).abs() + EPS), 24)
    corr_break = _correlation_breakdown(_ret(close, 10), _ret(index_close, 10), 24, 0.3)
    raw = _ret(close, 24) - 0.34 * VARIANT_WEIGHT * _zscore(beta, 24).abs() + 0.21 * VARIANT_WEIGHT * corr_break
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 相关性破裂检测_v4 | style: 反转 | seed: 20274413
