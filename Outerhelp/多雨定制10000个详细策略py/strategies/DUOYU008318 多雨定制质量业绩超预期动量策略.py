...
STRATEGY_ID = "DUOYU008318"
STRATEGY_NAME = "多雨定制质量业绩超预期动量策略"
SOURCE_FACTOR_ID = "DUOYU008318"
CATEGORY = "业绩超预期动量"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 业绩超预期动量_v6 | style: 质量 | seed: 20288315
