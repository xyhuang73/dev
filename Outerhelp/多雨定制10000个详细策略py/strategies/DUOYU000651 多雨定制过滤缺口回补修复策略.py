...
STRATEGY_ID = "DUOYU000651"
STRATEGY_NAME = "多雨定制过滤缺口回补修复策略"
SOURCE_FACTOR_ID = "DUOYU000651"
CATEGORY = "缺口回补修复"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
RISK_NOTE_CN = "重大事件驱动缺口可能长期不回补。 过滤风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 缺口回补修复_v11 | style: 过滤 | seed: 20243620
