...
STRATEGY_ID = "DUOYU001367"
STRATEGY_NAME = "多雨定制加速相对强弱动量策略"
SOURCE_FACTOR_ID = "DUOYU001367"
CATEGORY = "相对强弱动量"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 相对强弱动量_v5 | style: 加速 | seed: 20247214
