...
STRATEGY_ID = "DUOYU002650"
STRATEGY_NAME = "多雨定制共振资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002650"
CATEGORY = "资金流确认强度"
STYLE = "共振"
OPERATOR_ID = "资金流确认强度_v9"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "大额申赎可能导致资金流指标短期失真。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 18 个交易日。", "先计算 资金流确认强度 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU002650 - 多雨定制共振资金流确认强度策略
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.44 * VARIANT_WEIGHT * _zscore(net_flow, 14) + _ret(close, 25) + 0.3 * VARIANT_WEIGHT * _diff(fund_share, 25)
    raw = raw * _sigmoid_gate(_zscore(net_flow, 14), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 18)
    # instance: 资金流确认强度_v9 | style: 共振 | seed: 20253518
