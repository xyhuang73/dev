...
STRATEGY_ID = "DUOYU009515"
STRATEGY_NAME = "多雨定制轮动汇率风险轮动策略"
SOURCE_FACTOR_ID = "DUOYU009515"
CATEGORY = "汇率风险轮动"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 汇率风险轮动_v6 | style: 轮动 | seed: 20297015
