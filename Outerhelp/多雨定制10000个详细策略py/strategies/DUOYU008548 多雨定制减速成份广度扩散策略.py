...
STRATEGY_ID = "DUOYU008548"
STRATEGY_NAME = "多雨定制减速成份广度扩散策略"
SOURCE_FACTOR_ID = "DUOYU008548"
CATEGORY = "成份广度扩散"
STYLE = "减速"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 成份广度扩散_v4 | style: 减速 | seed: 20289313
