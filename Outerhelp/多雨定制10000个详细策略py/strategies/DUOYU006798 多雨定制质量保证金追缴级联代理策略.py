...
STRATEGY_ID = "DUOYU006798"
STRATEGY_NAME = "多雨定制质量保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006798"
CATEGORY = "保证金追缴级联代理"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
RISK_NOTE_CN = "保证金数据频率低，代理变量精度有限。 质量风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 保证金追缴级联代理_v4 | style: 质量 | seed: 20278313
