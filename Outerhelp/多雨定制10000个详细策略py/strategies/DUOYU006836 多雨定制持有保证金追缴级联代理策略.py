...
STRATEGY_ID = "DUOYU006836"
STRATEGY_NAME = "多雨定制持有保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006836"
CATEGORY = "保证金追缴级联代理"
STYLE = "持有"
OPERATOR_ID = "保证金追缴级联代理_v6"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 4
FAST_WINDOW = 9
SLOW_WINDOW = 29
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    margin_ratio = _field(panel, "margin_ratio") if "margin_ratio" in panel.columns else _zscore(_amount_z(panel, 14), 19)
    downside = _downside_vol(close, 19)
    margin_stress = _zscore(margin_ratio, 14) * downside
    raw = -0.38 * VARIANT_WEIGHT * margin_stress + _ret(close, 19)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 保证金追缴级联代理_v6 | style: 持有 | seed: 20278115
