...
STRATEGY_ID = "DUOYU009975"
STRATEGY_NAME = "多雨定制轮动ESG 资金偏好策略"
SOURCE_FACTOR_ID = "DUOYU009975"
CATEGORY = "ESG 资金偏好"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 19 个交易日为主要持有观察期，核心逻辑是：高环保合规+资金流入=ESG 主题资金偏好，捕捉可持续投资趋势。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "ESG 评分标准不统一，存在评级分歧。 轮动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 19 个交易日为主要持有观察期，核心逻辑是：高环保合规+资金流入=ESG 主题资金偏好，捕捉可持续投资趋势。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: esg_资金偏好_v5 | style: 轮动 | seed: 20301014
