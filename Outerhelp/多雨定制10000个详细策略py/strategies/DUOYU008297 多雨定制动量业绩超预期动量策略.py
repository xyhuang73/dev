...
STRATEGY_ID = "DUOYU008297"
STRATEGY_NAME = "多雨定制动量业绩超预期动量策略"
SOURCE_FACTOR_ID = "DUOYU008297"
CATEGORY = "业绩超预期动量"
STYLE = "动量"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 业绩超预期动量_v5 | style: 动量 | seed: 20288214
