...
STRATEGY_ID = "DUOYU000440"
STRATEGY_NAME = "多雨定制事件多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000440"
CATEGORY = "多时间框架动量"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "多周期共振失效于剧烈反转行情。 事件风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 多时间框架动量_v11 | style: 事件 | seed: 20243520
