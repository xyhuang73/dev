...
STRATEGY_ID = "DUOYU008851"
STRATEGY_NAME = "多雨定制过滤防御切换策略"
SOURCE_FACTOR_ID = "DUOYU008851"
CATEGORY = "防御切换"
STYLE = "过滤"
OPERATOR_ID = "防御切换_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 防御切换_v5 | style: 过滤 | seed: 20291614
