...
STRATEGY_ID = "DUOYU000220"
STRATEGY_NAME = "多雨定制事件风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000220"
CATEGORY = "风险调整动量"
STYLE = "事件"
OPERATOR_ID = "风险调整动量_v11"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    raw = _ret(close, 13) - 0.44 * VARIANT_WEIGHT * _downside_vol(close, 13) + 0.2 * VARIANT_WEIGHT * _amount_z(panel, 6)
    raw = raw * _sigmoid_gate(_ret(close, 6), 0, 4)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 风险调整动量_v11 | style: 事件 | seed: 20242520
