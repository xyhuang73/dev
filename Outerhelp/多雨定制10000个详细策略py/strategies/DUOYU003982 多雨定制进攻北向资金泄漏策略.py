...
STRATEGY_ID = "DUOYU003982"
STRATEGY_NAME = "多雨定制进攻北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003982"
CATEGORY = "北向资金泄漏"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 北向资金泄漏_v9 | style: 进攻 | seed: 20259718
