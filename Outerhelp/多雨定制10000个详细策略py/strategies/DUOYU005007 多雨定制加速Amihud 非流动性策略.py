...
STRATEGY_ID = "DUOYU005007"
STRATEGY_NAME = "多雨定制加速Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU005007"
CATEGORY = "Amihud 非流动性"
STYLE = "加速"
OPERATOR_ID = "amihud_非流动性_v6"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 11
FAST_WINDOW = 8
SLOW_WINDOW = 12
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
"""
DUOYU005007 - 多雨定制加速Amihud 非流动性策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    amihud = _zscore(ret_abs / (np.log1p(amount.clip(lower=0)) + EPS), 17)
    raw = -amihud + 0.26 * VARIANT_WEIGHT * _amount_z(panel, 13)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: amihud_非流动性_v6 | style: 加速 | seed: 20266215
