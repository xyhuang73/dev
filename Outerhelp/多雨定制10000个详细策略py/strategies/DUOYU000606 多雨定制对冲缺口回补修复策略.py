...
STRATEGY_ID = "DUOYU000606"
STRATEGY_NAME = "多雨定制对冲缺口回补修复策略"
SOURCE_FACTOR_ID = "DUOYU000606"
CATEGORY = "缺口回补修复"
STYLE = "对冲"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 缺口回补修复_v9 | style: 对冲 | seed: 20243118
