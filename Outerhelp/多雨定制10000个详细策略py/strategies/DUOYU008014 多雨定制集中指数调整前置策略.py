...
STRATEGY_ID = "DUOYU008014"
STRATEGY_NAME = "多雨定制集中指数调整前置策略"
SOURCE_FACTOR_ID = "DUOYU008014"
CATEGORY = "指数调整前置"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
RISK_NOTE_CN = "指数调整规则公开后前置收益衰减。 集中风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 指数调整前置_v5 | style: 集中 | seed: 20285914
