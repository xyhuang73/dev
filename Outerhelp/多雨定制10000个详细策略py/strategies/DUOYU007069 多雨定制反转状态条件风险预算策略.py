...
STRATEGY_ID = "DUOYU007069"
STRATEGY_NAME = "多雨定制反转状态条件风险预算策略"
SOURCE_FACTOR_ID = "DUOYU007069"
CATEGORY = "状态条件风险预算"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "风险预算参数依赖历史估计。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 9 个交易日。", "先计算 状态条件风险预算 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU007069 - 多雨定制反转状态条件风险预算策略
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 状态条件风险预算_v2 | style: 反转 | seed: 20279411
