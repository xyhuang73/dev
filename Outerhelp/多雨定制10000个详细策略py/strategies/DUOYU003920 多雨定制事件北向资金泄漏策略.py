...
STRATEGY_ID = "DUOYU003920"
STRATEGY_NAME = "多雨定制事件北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003920"
CATEGORY = "北向资金泄漏"
STYLE = "事件"
OPERATOR_ID = "北向资金泄漏_v5"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 10
FAST_WINDOW = 14
SLOW_WINDOW = 18
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.38 * VARIANT_WEIGHT * _zscore(northbound, 13) + _ret(close, 20)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 北向资金泄漏_v5 | style: 事件 | seed: 20261514
