...
STRATEGY_ID = "DUOYU004672"
STRATEGY_NAME = "多雨定制择时买卖盘深度失衡策略"
SOURCE_FACTOR_ID = "DUOYU004672"
CATEGORY = "买卖盘深度失衡"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：高换手+窄价差=深度买方市场，流动性充裕信号。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
RISK_NOTE_CN = "高频 microstructure 数据获取困难。 择时风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：高换手+窄价差=深度买方市场，流动性充裕信号。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 买卖盘深度失衡_v7 | style: 择时 | seed: 20264716
