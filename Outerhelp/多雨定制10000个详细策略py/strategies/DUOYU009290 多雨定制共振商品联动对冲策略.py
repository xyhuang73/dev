...
STRATEGY_ID = "DUOYU009290"
STRATEGY_NAME = "多雨定制共振商品联动对冲策略"
SOURCE_FACTOR_ID = "DUOYU009290"
CATEGORY = "商品联动对冲"
STYLE = "共振"
OPERATOR_ID = "商品联动对冲_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    commodity_link = _zscore(lead_asset, 8) * _ret(close, 8)
    raw = 0.27 * VARIANT_WEIGHT * commodity_link + _ret(close, 25)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 商品联动对冲_v1 | style: 共振 | seed: 20295510
