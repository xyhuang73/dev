...
STRATEGY_ID = "DUOYU000900"
STRATEGY_NAME = "多雨定制事件伽马突破策略"
SOURCE_FACTOR_ID = "DUOYU000900"
CATEGORY = "伽马突破"
STYLE = "事件"
OPERATOR_ID = "伽马突破_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "假突破在低流动性环境下更频繁。 事件风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 伽马突破_v1 | style: 事件 | seed: 20246510
