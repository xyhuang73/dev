...
STRATEGY_ID = "DUOYU006401"
STRATEGY_NAME = "多雨定制核心波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006401"
CATEGORY = "波动率的波动率"
STYLE = "核心"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。"
RISK_NOTE_CN = "vol-of-vol 估计对参数敏感。 核心风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v1 | style: 核心 | seed: 20274610
