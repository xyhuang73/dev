...
STRATEGY_ID = "DUOYU003810"
STRATEGY_NAME = "多雨定制共振融资融券压力策略"
SOURCE_FACTOR_ID = "DUOYU003810"
CATEGORY = "融资融券压力"
STYLE = "共振"
OPERATOR_ID = "融资融券压力_v9"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 13
FAST_WINDOW = 9
SLOW_WINDOW = 15
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    short_ratio = _field(panel, "short_interest_ratio") if "short_interest_ratio" in panel.columns else _zscore(_field(panel, "turnover"), 9)
    raw = -0.26 * VARIANT_WEIGHT * _zscore(margin_ratio, 15) + 0.26 * VARIANT_WEIGHT * _ret(close, 9) - _zscore(short_ratio, 9).abs()
    raw = raw * _max_dd_guard(drawdown, -0.15)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 融资融券压力_v9 | style: 共振 | seed: 20259518
