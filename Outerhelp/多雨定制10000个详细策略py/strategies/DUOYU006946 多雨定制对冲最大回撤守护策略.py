...
STRATEGY_ID = "DUOYU006946"
STRATEGY_NAME = "多雨定制对冲最大回撤守护策略"
SOURCE_FACTOR_ID = "DUOYU006946"
CATEGORY = "最大回撤守护"
STYLE = "对冲"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。"
RISK_NOTE_CN = "阈值设置影响保护效果与误关概率。 对冲风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 最大回撤守护_v4 | style: 对冲 | seed: 20278113
