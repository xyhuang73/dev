...
STRATEGY_ID = "DUOYU005417"
STRATEGY_NAME = "多雨定制动量委托簿斜率策略"
SOURCE_FACTOR_ID = "DUOYU005417"
CATEGORY = "委托簿斜率"
STYLE = "动量"
OPERATOR_ID = "委托簿斜率_v8"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 14
FAST_WINDOW = 9
SLOW_WINDOW = 13
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    book_slope = _zscore(spread, 8) / (_zscore(turnover, 8).abs() + EPS)
    raw = -0.3 * VARIANT_WEIGHT * _zscore(book_slope, 8) + _ret(close, 16)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 委托簿斜率_v8 | style: 动量 | seed: 20269217
