...
STRATEGY_ID = "DUOYU000267"
STRATEGY_NAME = "多雨定制加速多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000267"
CATEGORY = "多时间框架动量"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 多时间框架动量_v3 | style: 加速 | seed: 20242212
