...
STRATEGY_ID = "DUOYU009027"
STRATEGY_NAME = "多雨定制加速折溢价修复策略"
SOURCE_FACTOR_ID = "DUOYU009027"
CATEGORY = "折溢价修复"
STYLE = "加速"
OPERATOR_ID = "折溢价修复_v7"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -_zscore(premium, 14) + 0.41 * VARIANT_WEIGHT * _ret(nav, 14) + 0.16 * VARIANT_WEIGHT * _ret(close, 8)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 折溢价修复_v7 | style: 加速 | seed: 20292216
