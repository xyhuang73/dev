...
STRATEGY_ID = "DUOYU000415"
STRATEGY_NAME = "多雨定制轮动多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000415"
CATEGORY = "多时间框架动量"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 多时间框架动量_v10 | style: 轮动 | seed: 20243019
