...
STRATEGY_ID = "DUOYU001279"
STRATEGY_NAME = "多雨定制波动三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001279"
CATEGORY = "三重滤网组合"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 三重滤网组合_v10 | style: 波动 | seed: 20247419
