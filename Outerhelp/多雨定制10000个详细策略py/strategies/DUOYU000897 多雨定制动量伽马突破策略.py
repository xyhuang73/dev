...
STRATEGY_ID = "DUOYU000897"
STRATEGY_NAME = "多雨定制动量伽马突破策略"
SOURCE_FACTOR_ID = "DUOYU000897"
CATEGORY = "伽马突破"
STYLE = "动量"
OPERATOR_ID = "伽马突破_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "假突破在低流动性环境下更频繁。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 16 个交易日。", "先计算 伽马突破 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU000897 - 多雨定制动量伽马突破策略

策略主题：伽马突破 / 动量
适用范围：宽基/行业/主题
主要周期：16 个交易日
必需字段：close, high, low, amount

经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _rolling_max(series: pd.Series, window: int) -> pd.Series:
    w = max(2, int(window))
    return _by_etf(pd.to_numeric(series, errors="coerce"), lambda x: x.rolling(w, min_periods=max(2, w // 4)).max())
...
def _breakout(panel: pd.DataFrame, window: int) -> pd.Series:
    close = _field(panel, "close")
    prior_high = _rolling_max(close, max(3, int(window))).groupby(level=1).shift(1)
    return (close / (prior_high.abs() + EPS) - 1.0).replace([np.inf, -np.inf], np.nan)
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    true_range = (high - low) / (close.abs() + EPS)
    vol_compression = 1.0 - _cs_rank(_zscore(true_range, 20).abs()).abs()
    breakout = _breakout(panel, 11)
    raw = breakout * vol_compression * 0.74 * VARIANT_WEIGHT + 0.15 * VARIANT_WEIGHT * _amount_z(panel, 11)
    raw = raw * _sigmoid_gate(_amount_z(panel, 11), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 伽马突破_v1 | style: 动量 | seed: 20246210
