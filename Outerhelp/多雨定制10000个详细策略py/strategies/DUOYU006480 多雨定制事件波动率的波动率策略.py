...
STRATEGY_ID = "DUOYU006480"
STRATEGY_NAME = "多雨定制事件波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006480"
CATEGORY = "波动率的波动率"
STYLE = "事件"
OPERATOR_ID = "波动率的波动率_v4"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    close = _field(panel, "close")
    ret_1d = _ret(close, 1)
    vol = _by_etf(ret_1d.abs(), lambda x: x.rolling(16, min_periods=5).std())
    vol_of_vol = _by_etf(vol, lambda x: x.rolling(16, min_periods=5).std())
    raw = -0.35 * VARIANT_WEIGHT * _zscore(vol_of_vol, 8) + _ret(close, 16)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 波动率的波动率_v4 | style: 事件 | seed: 20276513
