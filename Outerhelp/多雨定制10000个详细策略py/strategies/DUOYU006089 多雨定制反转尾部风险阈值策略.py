...
STRATEGY_ID = "DUOYU006089"
STRATEGY_NAME = "多雨定制反转尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006089"
CATEGORY = "尾部风险阈值"
STYLE = "反转"
OPERATOR_ID = "尾部风险阈值_v1"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    tail_z = _tail_risk_z(close, 24, 0.05)
    raw = 0.3 * VARIANT_WEIGHT * _zscore(tail_z, 5) + _ret(close, 24)
    raw = raw * _max_dd_guard(drawdown, -0.10)
    score = _risk_scale(_cs_rank(raw), panel, 18)
    # instance: 尾部风险阈值_v1 | style: 反转 | seed: 20273410
