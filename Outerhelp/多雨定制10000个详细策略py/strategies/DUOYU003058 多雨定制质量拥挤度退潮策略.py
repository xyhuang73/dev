...
STRATEGY_ID = "DUOYU003058"
STRATEGY_NAME = "多雨定制质量拥挤度退潮策略"
SOURCE_FACTOR_ID = "DUOYU003058"
CATEGORY = "拥挤度退潮"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 拥挤度退潮_v9 | style: 质量 | seed: 20256318
