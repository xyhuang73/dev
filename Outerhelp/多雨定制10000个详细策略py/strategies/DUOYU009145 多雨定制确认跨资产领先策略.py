...
STRATEGY_ID = "DUOYU009145"
STRATEGY_NAME = "多雨定制确认跨资产领先策略"
SOURCE_FACTOR_ID = "DUOYU009145"
CATEGORY = "跨资产领先"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 跨资产领先_v6 | style: 确认 | seed: 20293015
