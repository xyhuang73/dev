...
STRATEGY_ID = "DUOYU009867"
STRATEGY_NAME = "多雨定制加速低波动异象策略"
SOURCE_FACTOR_ID = "DUOYU009867"
CATEGORY = "低波动异象"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 20 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
RISK_NOTE_CN = "低波异象在流动性驱动牛市中表现弱。 加速风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 20 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 低波动异象_v6 | style: 加速 | seed: 20299215
