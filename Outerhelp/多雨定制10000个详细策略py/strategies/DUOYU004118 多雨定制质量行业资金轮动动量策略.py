...
STRATEGY_ID = "DUOYU004118"
STRATEGY_NAME = "多雨定制质量行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004118"
CATEGORY = "行业资金轮动动量"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 行业资金轮动动量_v6 | style: 质量 | seed: 20262315
