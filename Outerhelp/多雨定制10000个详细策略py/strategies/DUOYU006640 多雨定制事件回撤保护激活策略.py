...
STRATEGY_ID = "DUOYU006640"
STRATEGY_NAME = "多雨定制事件回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006640"
CATEGORY = "回撤保护激活"
STYLE = "事件"
OPERATOR_ID = "回撤保护激活_v4"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    if "drawdown" in panel.columns:
        ...
        rolling_peak = _rolling_max(close, 15)
        drawdown = close / (rolling_peak.abs() + EPS) - 1.0
    dd_guard = _max_dd_guard(drawdown, -0.15)
    raw = -_zscore(drawdown, 15) * dd_guard + 0.33 * VARIANT_WEIGHT * _zscore(net_flow, 5) + _repair_slope(close, 5)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 回撤保护激活_v4 | style: 事件 | seed: 20277513
