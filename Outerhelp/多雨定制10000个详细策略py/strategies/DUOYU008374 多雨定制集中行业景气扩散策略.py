...
STRATEGY_ID = "DUOYU008374"
STRATEGY_NAME = "多雨定制集中行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008374"
CATEGORY = "行业景气扩散"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 行业景气扩散_v2 | style: 集中 | seed: 20288911
