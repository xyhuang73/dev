...
STRATEGY_ID = "DUOYU007530"
STRATEGY_NAME = "多雨定制共振解禁到期压力策略"
SOURCE_FACTOR_ID = "DUOYU007530"
CATEGORY = "解禁到期压力"
STYLE = "共振"
OPERATOR_ID = "解禁到期压力_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：解禁比例大+临近解禁日=抛售压力大，解禁过后压力衰减。逆向思维：提前规避。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：解禁比例大+临近解禁日=抛售压力大，解禁过后压力衰减。逆向思维：提前规避。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    pressure = _lockup_pressure(lockup_ratio, lockup_days, 19)
    raw = -0.46 * VARIANT_WEIGHT * _zscore(pressure, 9) + _ret(close, 27)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 解禁到期压力_v2 | style: 共振 | seed: 20282511
