...
STRATEGY_ID = "DUOYU000390"
STRATEGY_NAME = "多雨定制共振多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000390"
CATEGORY = "多时间框架动量"
STYLE = "共振"
OPERATOR_ID = "多时间框架动量_v9"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "多周期共振失效于剧烈反转行情。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 多时间框架动量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU000390 - 多雨定制共振多时间框架动量策略
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    ret_m = _ret(close, 11)
    ret_l = _ret(close, 24)
    raw = 0.43 * VARIANT_WEIGHT * ret_s + 0.43 * VARIANT_WEIGHT * ret_m + 0.33 * VARIANT_WEIGHT * ret_l - 0.18 * _downside_vol(close, 24)
    raw = raw * _sigmoid_gate(_ret(close, 11), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 多时间框架动量_v9 | style: 共振 | seed: 20242518
