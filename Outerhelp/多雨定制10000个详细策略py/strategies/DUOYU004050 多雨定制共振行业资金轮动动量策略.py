...
STRATEGY_ID = "DUOYU004050"
STRATEGY_NAME = "多雨定制共振行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004050"
CATEGORY = "行业资金轮动动量"
STYLE = "共振"
OPERATOR_ID = "行业资金轮动动量_v3"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 7
FAST_WINDOW = 9
SLOW_WINDOW = 14
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    sector_relative = _cs_rank(_zscore(net_flow, 9)) - _cs_rank(_zscore(net_flow, 14))
    raw = 0.29 * VARIANT_WEIGHT * sector_relative + _ret(close, 9)
    score = _risk_scale(_cs_rank(raw), panel, 7)
    # instance: 行业资金轮动动量_v3 | style: 共振 | seed: 20261512
