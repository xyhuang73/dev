...
STRATEGY_ID = "DUOYU006740"
STRATEGY_NAME = "多雨定制事件保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006740"
CATEGORY = "保证金追缴级联代理"
STYLE = "事件"
OPERATOR_ID = "保证金追缴级联代理_v1"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 16
FAST_WINDOW = 5
SLOW_WINDOW = 20
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    margin_ratio = _field(panel, "margin_ratio") if "margin_ratio" in panel.columns else _zscore(_amount_z(panel, 5), 22)
    downside = _downside_vol(close, 22)
    margin_stress = _zscore(margin_ratio, 5) * downside
    raw = -0.34 * VARIANT_WEIGHT * margin_stress + _ret(close, 22)
    raw = raw * _max_dd_guard(drawdown, -0.15)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 保证金追缴级联代理_v1 | style: 事件 | seed: 20278510
