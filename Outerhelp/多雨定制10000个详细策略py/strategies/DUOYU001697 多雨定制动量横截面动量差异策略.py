...
STRATEGY_ID = "DUOYU001697"
STRATEGY_NAME = "多雨定制动量横截面动量差异策略"
SOURCE_FACTOR_ID = "DUOYU001697"
CATEGORY = "横截面动量差异"
STYLE = "动量"
OPERATOR_ID = "横截面动量差异_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.36 * VARIANT_WEIGHT * _zscore(momentum_diff, 6) + _ret(close, 16)
    raw = raw * _sigmoid_gate(_ret(close, 6), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 横截面动量差异_v1 | style: 动量 | seed: 20250210
