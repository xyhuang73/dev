...
STRATEGY_ID = "DUOYU003215"
STRATEGY_NAME = "多雨定制轮动基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003215"
CATEGORY = "基金持仓集中度"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 19 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "机构持仓数据季度披露存在严重滞后。 轮动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 19 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 基金持仓集中度_v7 | style: 轮动 | seed: 20257016
