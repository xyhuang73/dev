...
STRATEGY_ID = "DUOYU002069"
STRATEGY_NAME = "多雨定制反转路径效率动量策略"
SOURCE_FACTOR_ID = "DUOYU002069"
CATEGORY = "路径效率动量"
STYLE = "反转"
OPERATOR_ID = "路径效率动量_v10"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "效率比在V形反转中可能给出滞后信号。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 路径效率动量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU002069 - 多雨定制反转路径效率动量策略
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _rolling_efficiency_ratio(close: pd.Series, window: int) -> pd.Series:
    """Kaufman 效率比：方向性 / 路径波动。"""
    w = max(5, int(window))
    ret = _ret(close, w)
    path = _by_etf(close.diff().abs(), lambda x: x.rolling(w, min_periods=3).sum())
    return (ret.abs() / (path / (close.groupby(level=1).shift(w).abs() + EPS) + EPS)).replace([np.inf, -np.inf], np.nan)
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    eff = _rolling_efficiency_ratio(close, 26)
    raw = 0.41 * VARIANT_WEIGHT * _zscore(eff, 11) + _ret(close, 26)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 路径效率动量_v10 | style: 反转 | seed: 20250419
