...
STRATEGY_ID = "DUOYU006160"
STRATEGY_NAME = "多雨定制事件尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006160"
CATEGORY = "尾部风险阈值"
STYLE = "事件"
OPERATOR_ID = "尾部风险阈值_v4"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    tail_z = _tail_risk_z(close, 23, 0.05)
    raw = 0.22 * VARIANT_WEIGHT * _zscore(tail_z, 5) + _ret(close, 23)
    score = _risk_scale(_cs_rank(raw), panel, 17)
    # instance: 尾部风险阈值_v4 | style: 事件 | seed: 20274513
