...
STRATEGY_ID = "DUOYU006116"
STRATEGY_NAME = "多雨定制持有尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006116"
CATEGORY = "尾部风险阈值"
STYLE = "持有"
OPERATOR_ID = "尾部风险阈值_v2"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    tail_z = _tail_risk_z(close, 16, 0.05)
    raw = 0.26 * VARIANT_WEIGHT * _zscore(tail_z, 5) + _ret(close, 16)
    score = _risk_scale(_cs_rank(raw), panel, 18)
    # instance: 尾部风险阈值_v2 | style: 持有 | seed: 20274111
