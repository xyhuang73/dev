...
STRATEGY_ID = "DUOYU008349"
STRATEGY_NAME = "多雨定制反转行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008349"
CATEGORY = "行业景气扩散"
STYLE = "反转"
OPERATOR_ID = "行业景气扩散_v1"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 7
FAST_WINDOW = 11
SLOW_WINDOW = 14
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.45 * VARIANT_WEIGHT * _zscore(prosperity, 16) + 0.34 * VARIANT_WEIGHT * _zscore(revision, 16) + _ret(industry_momentum, 10)
    score = _risk_scale(_cs_rank(raw), panel, 7)
    # instance: 行业景气扩散_v1 | style: 反转 | seed: 20288410
