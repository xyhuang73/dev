...
STRATEGY_ID = "DUOYU001380"
STRATEGY_NAME = "多雨定制事件相对强弱动量策略"
SOURCE_FACTOR_ID = "DUOYU001380"
CATEGORY = "相对强弱动量"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 相对强弱动量_v5 | style: 事件 | seed: 20248514
