...
STRATEGY_ID = "DUOYU007421"
STRATEGY_NAME = "多雨定制核心政策事件评分策略"
SOURCE_FACTOR_ID = "DUOYU007421"
CATEGORY = "政策事件评分"
STYLE = "核心"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。"
RISK_NOTE_CN = "政策事件评分的 NLP 模型存在误判风险。 核心风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 政策事件评分_v4 | style: 核心 | seed: 20280613
