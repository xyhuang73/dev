...
STRATEGY_ID = "DUOYU002785"
STRATEGY_NAME = "多雨定制确认份额扩张吸收策略"
SOURCE_FACTOR_ID = "DUOYU002785"
CATEGORY = "份额扩张吸收"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
RISK_NOTE_CN = "份额变化数据存在 T+1 以上滞后。 确认风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 份额扩张吸收_v6 | style: 确认 | seed: 20254015
