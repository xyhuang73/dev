...
STRATEGY_ID = "DUOYU008377"
STRATEGY_NAME = "多雨定制动量行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008377"
CATEGORY = "行业景气扩散"
STYLE = "动量"
OPERATOR_ID = "行业景气扩散_v2"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 14
FAST_WINDOW = 14
SLOW_WINDOW = 16
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["prosperity_score", "earnings_revision", "industry_momentum"]

ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "景气度指标有发布周期限制。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 14 个交易日。", "先计算 行业景气扩散 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008377 - 多雨定制动量行业景气扩散策略

策略主题：行业景气扩散 / 动量
适用范围：行业/主题
主要周期：14 个交易日
必需字段：prosperity_score, earnings_revision, industry_momentum

经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    prosperity = _field(panel, "prosperity_score")
    revision = _field(panel, "earnings_revision")
    industry_momentum = _field(panel, "industry_momentum")
    raw = 0.49 * VARIANT_WEIGHT * _zscore(prosperity, 19) + 0.32 * VARIANT_WEIGHT * _zscore(revision, 19) + _ret(industry_momentum, 12)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 行业景气扩散_v2 | style: 动量 | seed: 20289211
