...
STRATEGY_ID = "DUOYU003694"
STRATEGY_NAME = "多雨定制集中融资融券压力策略"
SOURCE_FACTOR_ID = "DUOYU003694"
CATEGORY = "融资融券压力"
STYLE = "集中"
OPERATOR_ID = "融资融券压力_v3"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 10
FAST_WINDOW = 10
SLOW_WINDOW = 20
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    short_ratio = _field(panel, "short_interest_ratio") if "short_interest_ratio" in panel.columns else _zscore(_field(panel, "turnover"), 8)
    raw = -0.24 * VARIANT_WEIGHT * _zscore(margin_ratio, 26) + 0.31 * VARIANT_WEIGHT * _ret(close, 8) - _zscore(short_ratio, 8).abs()
    raw = raw * _max_dd_guard(drawdown, -0.15)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 融资融券压力_v3 | style: 集中 | seed: 20259912
