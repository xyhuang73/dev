...
STRATEGY_ID = "DUOYU005270"
STRATEGY_NAME = "多雨定制共振委托簿斜率策略"
SOURCE_FACTOR_ID = "DUOYU005270"
CATEGORY = "委托簿斜率"
STYLE = "共振"
OPERATOR_ID = "委托簿斜率_v1"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 12
FAST_WINDOW = 5
SLOW_WINDOW = 18
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    book_slope = _zscore(spread, 5) / (_zscore(turnover, 5).abs() + EPS)
    raw = -0.28 * VARIANT_WEIGHT * _zscore(book_slope, 5) + _ret(close, 18)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 委托簿斜率_v1 | style: 共振 | seed: 20268510
