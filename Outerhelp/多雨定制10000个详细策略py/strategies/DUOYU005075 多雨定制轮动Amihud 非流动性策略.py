...
STRATEGY_ID = "DUOYU005075"
STRATEGY_NAME = "多雨定制轮动Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU005075"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
"""
DUOYU005075 - 多雨定制轮动Amihud 非流动性策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: amihud_非流动性_v9 | style: 轮动 | seed: 20267018
