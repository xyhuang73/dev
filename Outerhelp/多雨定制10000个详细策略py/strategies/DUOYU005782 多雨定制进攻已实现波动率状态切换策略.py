...
STRATEGY_ID = "DUOYU005782"
STRATEGY_NAME = "多雨定制进攻已实现波动率状态切换策略"
SOURCE_FACTOR_ID = "DUOYU005782"
CATEGORY = "已实现波动率状态切换"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
"""
DUOYU005782 - 多雨定制进攻已实现波动率状态切换策略
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 已实现波动率状态切换_v2 | style: 进攻 | seed: 20270711
