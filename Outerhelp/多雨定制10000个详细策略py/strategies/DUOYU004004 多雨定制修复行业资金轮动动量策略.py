...
STRATEGY_ID = "DUOYU004004"
STRATEGY_NAME = "多雨定制修复行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004004"
CATEGORY = "行业资金轮动动量"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
RISK_NOTE_CN = "行业分类标准变化影响信号连续性。 修复风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 行业资金轮动动量_v1 | style: 修复 | seed: 20260910
