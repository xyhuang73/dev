...
STRATEGY_ID = "DUOYU007010"
STRATEGY_NAME = "多雨定制共振最大回撤守护策略"
SOURCE_FACTOR_ID = "DUOYU007010"
CATEGORY = "最大回撤守护"
STYLE = "共振"
OPERATOR_ID = "最大回撤守护_v7"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    rolling_peak = _rolling_max(close, 27)
    drawdown = close / (rolling_peak.abs() + EPS) - 1.0
    dd_score = _zscore(drawdown, 6)
    raw = _ret(close, 27) * _max_dd_guard(drawdown, -0.08) + 0.35 * VARIANT_WEIGHT * dd_score
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 最大回撤守护_v7 | style: 共振 | seed: 20278516
