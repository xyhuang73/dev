...
STRATEGY_ID = "DUOYU004754"
STRATEGY_NAME = "多雨定制集中流动性冲击吸收策略"
SOURCE_FACTOR_ID = "DUOYU004754"
CATEGORY = "流动性冲击吸收"
STYLE = "集中"
OPERATOR_ID = "流动性冲击吸收_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    shock = _zscore(turnover, 11) - _zscore(turnover, 23)
    absorption = _ret(close, 11) / (_zscore(turnover, 11).abs() + EPS)
    raw = 0.28 * VARIANT_WEIGHT * shock + 0.26 * VARIANT_WEIGHT * _zscore(absorption, 11)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 流动性冲击吸收_v2 | style: 集中 | seed: 20265911
