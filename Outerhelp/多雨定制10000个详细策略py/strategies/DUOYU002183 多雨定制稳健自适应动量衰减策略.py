...
STRATEGY_ID = "DUOYU002183"
STRATEGY_NAME = "多雨定制稳健自适应动量衰减策略"
SOURCE_FACTOR_ID = "DUOYU002183"
CATEGORY = "自适应动量衰减"
STYLE = "稳健"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。"
RISK_NOTE_CN = "自适应衰减可能在低波动趋势中过度放大信号。 稳健风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 自适应动量衰减_v6 | style: 稳健 | seed: 20250815
