...
STRATEGY_ID = "DUOYU002110"
STRATEGY_NAME = "多雨定制共振自适应动量衰减策略"
SOURCE_FACTOR_ID = "DUOYU002110"
CATEGORY = "自适应动量衰减"
STYLE = "共振"
OPERATOR_ID = "自适应动量衰减_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    close = _field(panel, "close")
    ret = _ret(close, 24)
    decay = np.exp(-5.97 * VARIANT_WEIGHT * _downside_vol(close, 24).abs())
    raw = ret * decay + 0.14 * VARIANT_WEIGHT * _amount_z(panel, 9)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 自适应动量衰减_v2 | style: 共振 | seed: 20251511
