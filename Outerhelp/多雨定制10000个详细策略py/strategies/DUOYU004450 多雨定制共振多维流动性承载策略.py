...
STRATEGY_ID = "DUOYU004450"
STRATEGY_NAME = "多雨定制共振多维流动性承载策略"
SOURCE_FACTOR_ID = "DUOYU004450"
CATEGORY = "多维流动性承载"
STYLE = "共振"
OPERATOR_ID = "多维流动性承载_v5"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 13
FAST_WINDOW = 12
SLOW_WINDOW = 28
VARIANT_WEIGHT = 0.91
REQUIRED_FIELDS = ["amount", "turnover", "bid_ask_spread"]

ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "流动性指标在极端行情中可能突变。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 多维流动性承载 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU004450 - 多雨定制共振多维流动性承载策略

策略主题：多维流动性承载 / 共振
适用范围：流动性较好
主要周期：13 个交易日
必需字段：amount, turnover, bid_ask_spread

经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.49 * VARIANT_WEIGHT * _amount_z(panel, 28) + 0.16 * VARIANT_WEIGHT * _zscore(turnover, 12) - _zscore(spread, 12).abs()
    score = _cs_rank(raw)
    # instance: 多维流动性承载_v5 | style: 共振 | seed: 20263514
