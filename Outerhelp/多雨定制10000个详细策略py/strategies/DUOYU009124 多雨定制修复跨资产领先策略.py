...
STRATEGY_ID = "DUOYU009124"
STRATEGY_NAME = "多雨定制修复跨资产领先策略"
SOURCE_FACTOR_ID = "DUOYU009124"
CATEGORY = "跨资产领先"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
RISK_NOTE_CN = "跨资产领先关系并非稳定不变。 修复风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 15 个交易日。", "先计算 跨资产领先 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009124 - 多雨定制修复跨资产领先策略
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 跨资产领先_v5 | style: 修复 | seed: 20292914
