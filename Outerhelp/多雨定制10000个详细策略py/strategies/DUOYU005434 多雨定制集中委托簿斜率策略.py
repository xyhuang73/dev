...
STRATEGY_ID = "DUOYU005434"
STRATEGY_NAME = "多雨定制集中委托簿斜率策略"
SOURCE_FACTOR_ID = "DUOYU005434"
CATEGORY = "委托簿斜率"
STYLE = "集中"
OPERATOR_ID = "委托簿斜率_v9"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 10
FAST_WINDOW = 15
SLOW_WINDOW = 19
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    book_slope = _zscore(spread, 12) / (_zscore(turnover, 12).abs() + EPS)
    raw = -0.38 * VARIANT_WEIGHT * _zscore(book_slope, 12) + _ret(close, 24)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 委托簿斜率_v9 | style: 集中 | seed: 20268918
