...
STRATEGY_ID = "DUOYU009207"
STRATEGY_NAME = "多雨定制加速风格轮动价值质量策略"
SOURCE_FACTOR_ID = "DUOYU009207"
CATEGORY = "风格轮动价值质量"
STYLE = "加速"
OPERATOR_ID = "风格轮动价值质量_v3"
MARKET_SCOPE = "风格/宽基"
HORIZON_DAYS = 11
FAST_WINDOW = 11
SLOW_WINDOW = 12
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
RISK_NOTE_CN = "风格因子在不同市场周期表现差异大。 加速风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 11 个交易日。", "先计算 风格轮动价值质量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009207 - 多雨定制加速风格轮动价值质量策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.31 * VARIANT_WEIGHT * _zscore(valuation, 18) + 0.23 * VARIANT_WEIGHT * _zscore(quality, 18) + 0.27 * VARIANT_WEIGHT * _zscore(dividend, 18) + _ret(close, 8)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 风格轮动价值质量_v3 | style: 加速 | seed: 20294212
