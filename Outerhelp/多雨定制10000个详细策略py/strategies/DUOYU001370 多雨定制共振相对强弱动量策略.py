...
STRATEGY_ID = "DUOYU001370"
STRATEGY_NAME = "多雨定制共振相对强弱动量策略"
SOURCE_FACTOR_ID = "DUOYU001370"
CATEGORY = "相对强弱动量"
STYLE = "共振"
OPERATOR_ID = "相对强弱动量_v5"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 10
SLOW_WINDOW = 15
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _ret(close, 15) - _ret(index_close, 15) + 0.4 * VARIANT_WEIGHT * _zscore(breadth, 10)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 相对强弱动量_v5 | style: 共振 | seed: 20247514
