...
STRATEGY_ID = "DUOYU003976"
STRATEGY_NAME = "多雨定制持有北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003976"
CATEGORY = "北向资金泄漏"
STYLE = "持有"
OPERATOR_ID = "北向资金泄漏_v8"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 4
FAST_WINDOW = 9
SLOW_WINDOW = 35
...
ECONOMIC_LOGIC_CN = "该策略以约 4 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
RISK_NOTE_CN = "北向资金并非总是聪明钱，存在跟随性交易。 持有风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 4 个交易日。", "先计算 北向资金泄漏 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU003976 - 多雨定制持有北向资金泄漏策略
...
经济逻辑：
该策略以约 4 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    northbound = _field(panel, "northbound_flow") if "northbound_flow" in panel.columns else _diff(_field(panel, "fund_share"), 14)
    raw = 0.48 * VARIANT_WEIGHT * _zscore(northbound, 14) + _ret(close, 24)
    score = _risk_scale(_cs_rank(raw), panel, 7)
    # instance: 北向资金泄漏_v8 | style: 持有 | seed: 20261117
