...
STRATEGY_ID = "DUOYU006556"
STRATEGY_NAME = "多雨定制持有波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006556"
CATEGORY = "波动率的波动率"
STYLE = "持有"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v8 | style: 持有 | seed: 20276117
