...
STRATEGY_ID = "DUOYU007271"
STRATEGY_NAME = "多雨定制过滤波动率压缩预警策略"
SOURCE_FACTOR_ID = "DUOYU007271"
CATEGORY = "波动率压缩预警"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
RISK_NOTE_CN = "波动压缩持续时间不可预测。 过滤风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率压缩预警_v4 | style: 过滤 | seed: 20280613
