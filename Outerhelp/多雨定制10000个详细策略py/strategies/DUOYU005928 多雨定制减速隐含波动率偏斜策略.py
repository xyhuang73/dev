...
STRATEGY_ID = "DUOYU005928"
STRATEGY_NAME = "多雨定制减速隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU005928"
CATEGORY = "隐含波动率偏斜"
STYLE = "减速"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。"
RISK_NOTE_CN = "多数 标的 没有活跃期权市场。 减速风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 隐含波动率偏斜_v1 | style: 减速 | seed: 20272310
