...
STRATEGY_ID = "DUOYU007078"
STRATEGY_NAME = "多雨定制质量状态条件风险预算策略"
SOURCE_FACTOR_ID = "DUOYU007078"
CATEGORY = "状态条件风险预算"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 状态条件风险预算_v2 | style: 质量 | seed: 20280311
