...
STRATEGY_ID = "DUOYU001947"
STRATEGY_NAME = "多雨定制加速路径效率动量策略"
SOURCE_FACTOR_ID = "DUOYU001947"
CATEGORY = "路径效率动量"
STYLE = "加速"
OPERATOR_ID = "路径效率动量_v4"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    eff = _rolling_efficiency_ratio(close, 24)
    raw = 0.34 * VARIANT_WEIGHT * _zscore(eff, 7) + _ret(close, 24)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 路径效率动量_v4 | style: 加速 | seed: 20250213
