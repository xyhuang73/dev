...
STRATEGY_ID = "DUOYU004229"
STRATEGY_NAME = "多雨定制反转申赎套利流策略"
SOURCE_FACTOR_ID = "DUOYU004229"
CATEGORY = "申赎套利流"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：折溢价套利资金流：溢价回归+净值上涨=套利与趋势共振，折价扩大+净值下跌=压力。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "跨境 标的 申赎机制不同于境内标的。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 16 个交易日。", "先计算 申赎套利流 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU004229 - 多雨定制反转申赎套利流策略
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：折溢价套利资金流：溢价回归+净值上涨=套利与趋势共振，折价扩大+净值下跌=压力。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 申赎套利流_v3 | style: 反转 | seed: 20262412
