...
STRATEGY_ID = "DUOYU007800"
STRATEGY_NAME = "多雨定制事件政策监管体制策略"
SOURCE_FACTOR_ID = "DUOYU007800"
CATEGORY = "政策监管体制"
STYLE = "事件"
OPERATOR_ID = "政策监管体制_v1"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 20
FAST_WINDOW = 6
SLOW_WINDOW = 22
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    policy_cycle = _field(panel, "industry_policy_cycle") if "industry_policy_cycle" in panel.columns else _zscore(policy_sent, 24)
    raw = -0.36 * VARIANT_WEIGHT * _zscore(reg_risk, 24) + 0.29 * VARIANT_WEIGHT * _zscore(policy_sent, 6) + 0.25 * VARIANT_WEIGHT * _zscore(policy_cycle, 24)
    score = _risk_scale(_cs_rank(raw), panel, 19)
    # instance: 政策监管体制_v1 | style: 事件 | seed: 20285510
