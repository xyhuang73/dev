...
STRATEGY_ID = "DUOYU001587"
STRATEGY_NAME = "多雨定制加速回撤反转策略"
SOURCE_FACTOR_ID = "DUOYU001587"
CATEGORY = "回撤反转"
STYLE = "加速"
OPERATOR_ID = "回撤反转_v6"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤反转_v6 | style: 加速 | seed: 20248215
