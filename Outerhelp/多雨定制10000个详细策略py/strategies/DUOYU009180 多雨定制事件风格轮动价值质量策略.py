...
STRATEGY_ID = "DUOYU009180"
STRATEGY_NAME = "多雨定制事件风格轮动价值质量策略"
SOURCE_FACTOR_ID = "DUOYU009180"
CATEGORY = "风格轮动价值质量"
STYLE = "事件"
OPERATOR_ID = "风格轮动价值质量_v1"
MARKET_SCOPE = "风格/宽基"
HORIZON_DAYS = 6
FAST_WINDOW = 8
SLOW_WINDOW = 25
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.4 * VARIANT_WEIGHT * _zscore(valuation, 28) + 0.27 * VARIANT_WEIGHT * _zscore(quality, 28) + 0.18 * VARIANT_WEIGHT * _zscore(dividend, 28) + _ret(close, 8)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 风格轮动价值质量_v1 | style: 事件 | seed: 20295510
