...
STRATEGY_ID = "DUOYU009240"
STRATEGY_NAME = "多雨定制事件风格轮动价值质量策略"
SOURCE_FACTOR_ID = "DUOYU009240"
CATEGORY = "风格轮动价值质量"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风格轮动价值质量_v4 | style: 事件 | seed: 20295513
