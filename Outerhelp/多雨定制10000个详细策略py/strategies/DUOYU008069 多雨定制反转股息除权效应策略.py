...
STRATEGY_ID = "DUOYU008069"
STRATEGY_NAME = "多雨定制反转股息除权效应策略"
SOURCE_FACTOR_ID = "DUOYU008069"
CATEGORY = "股息除权效应"
STYLE = "反转"
OPERATOR_ID = "股息除权效应_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "红利税和除权价格调整需考虑。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 股息除权效应 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008069 - 多雨定制反转股息除权效应策略
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    div_score = _zscore(dividend_yield, 28) * _event_proximity(div_date_dist, 12)
    raw = 0.35 * VARIANT_WEIGHT * div_score + _ret(close, 12)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 股息除权效应_v1 | style: 反转 | seed: 20286410
