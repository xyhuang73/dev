...
STRATEGY_ID = "DUOYU008877"
STRATEGY_NAME = "多雨定制动量防御切换策略"
SOURCE_FACTOR_ID = "DUOYU008877"
CATEGORY = "防御切换"
STYLE = "动量"
OPERATOR_ID = "防御切换_v6"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "防御切换可能错失反弹行情。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 16 个交易日。", "先计算 防御切换 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008877 - 多雨定制动量防御切换策略
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -_zscore(appetite, 11) + 0.45 * VARIANT_WEIGHT * _zscore(dividend, 27) - _downside_vol(close, 27) - 0.22 * VARIANT_WEIGHT * _zscore(downside, 27).abs()
    score = _cs_rank(raw)
    # instance: 防御切换_v6 | style: 动量 | seed: 20292215
