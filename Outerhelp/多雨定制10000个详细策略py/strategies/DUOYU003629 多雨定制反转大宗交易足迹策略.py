...
STRATEGY_ID = "DUOYU003629"
STRATEGY_NAME = "多雨定制反转大宗交易足迹策略"
SOURCE_FACTOR_ID = "DUOYU003629"
CATEGORY = "大宗交易足迹"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：大宗交易溢价/折价反映机构投资者对当前价格的判断，溢价=看好、折价=看淡。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "大宗交易数据覆盖面和及时性有限。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 大宗交易足迹 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU003629 - 多雨定制反转大宗交易足迹策略
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：大宗交易溢价/折价反映机构投资者对当前价格的判断，溢价=看好、折价=看淡。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 大宗交易足迹_v9 | style: 反转 | seed: 20258418
