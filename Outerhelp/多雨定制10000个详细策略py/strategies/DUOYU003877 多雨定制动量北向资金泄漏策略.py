...
STRATEGY_ID = "DUOYU003877"
STRATEGY_NAME = "多雨定制动量北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003877"
CATEGORY = "北向资金泄漏"
STYLE = "动量"
OPERATOR_ID = "北向资金泄漏_v3"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 12
FAST_WINDOW = 11
SLOW_WINDOW = 22
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["close", "northbound_flow", "fund_share"]

ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "北向资金并非总是聪明钱，存在跟随性交易。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 12 个交易日。", "先计算 北向资金泄漏 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU003877 - 多雨定制动量北向资金泄漏策略

策略主题：北向资金泄漏 / 动量
适用范围：宽基/行业
主要周期：12 个交易日
必需字段：close, northbound_flow, fund_share

经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    northbound = _field(panel, "northbound_flow") if "northbound_flow" in panel.columns else _diff(_field(panel, "fund_share"), 10)
    raw = 0.43 * VARIANT_WEIGHT * _zscore(northbound, 10) + _ret(close, 26)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 北向资金泄漏_v3 | style: 动量 | seed: 20261212
