...
STRATEGY_ID = "DUOYU002191"
STRATEGY_NAME = "多雨定制过滤自适应动量衰减策略"
SOURCE_FACTOR_ID = "DUOYU002191"
CATEGORY = "自适应动量衰减"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
RISK_NOTE_CN = "自适应衰减可能在低波动趋势中过度放大信号。 过滤风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 自适应动量衰减_v6 | style: 过滤 | seed: 20251615
