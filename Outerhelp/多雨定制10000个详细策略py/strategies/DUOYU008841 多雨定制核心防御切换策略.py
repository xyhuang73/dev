...
STRATEGY_ID = "DUOYU008841"
STRATEGY_NAME = "多雨定制核心防御切换策略"
SOURCE_FACTOR_ID = "DUOYU008841"
CATEGORY = "防御切换"
STYLE = "核心"
OPERATOR_ID = "防御切换_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 防御切换_v5 | style: 核心 | seed: 20290614
