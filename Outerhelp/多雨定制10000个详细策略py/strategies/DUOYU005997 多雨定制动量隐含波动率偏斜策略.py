...
STRATEGY_ID = "DUOYU005997"
STRATEGY_NAME = "多雨定制动量隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU005997"
CATEGORY = "隐含波动率偏斜"
STYLE = "动量"
OPERATOR_ID = "隐含波动率偏斜_v4"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 8
FAST_WINDOW = 14
SLOW_WINDOW = 22
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    iv = _field(panel, "option_implied_vol") if "option_implied_vol" in panel.columns else _downside_vol(close, 26)
    iv_change = _diff(iv, 12)
    raw = -0.32 * VARIANT_WEIGHT * _zscore(iv_change, 12) + _ret(close, 26)
    score = _risk_scale(_cs_rank(raw), panel, 7)
    # instance: 隐含波动率偏斜_v4 | style: 动量 | seed: 20273213
