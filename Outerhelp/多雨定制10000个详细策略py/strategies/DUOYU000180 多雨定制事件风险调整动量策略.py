...
STRATEGY_ID = "DUOYU000180"
STRATEGY_NAME = "多雨定制事件风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000180"
CATEGORY = "风险调整动量"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "连续上涨后拥挤和高波动会降低信号稳定性。 事件风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风险调整动量_v9 | style: 事件 | seed: 20242518
