...
STRATEGY_ID = "DUOYU007810"
STRATEGY_NAME = "多雨定制共振政策监管体制策略"
SOURCE_FACTOR_ID = "DUOYU007810"
CATEGORY = "政策监管体制"
STYLE = "共振"
OPERATOR_ID = "政策监管体制_v2"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 17
FAST_WINDOW = 6
SLOW_WINDOW = 18
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    policy_cycle = _field(panel, "industry_policy_cycle") if "industry_policy_cycle" in panel.columns else _zscore(policy_sent, 18)
    raw = -0.3 * VARIANT_WEIGHT * _zscore(reg_risk, 18) + 0.32 * VARIANT_WEIGHT * _zscore(policy_sent, 6) + 0.29 * VARIANT_WEIGHT * _zscore(policy_cycle, 18)
    score = _risk_scale(_cs_rank(raw), panel, 17)
    # instance: 政策监管体制_v2 | style: 共振 | seed: 20284511
