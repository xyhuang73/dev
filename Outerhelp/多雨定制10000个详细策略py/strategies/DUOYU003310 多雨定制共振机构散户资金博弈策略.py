...
STRATEGY_ID = "DUOYU003310"
STRATEGY_NAME = "多雨定制共振机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003310"
CATEGORY = "机构散户资金博弈"
STYLE = "共振"
OPERATOR_ID = "机构散户资金博弈_v2"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 17
FAST_WINDOW = 9
SLOW_WINDOW = 28
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    holder_change = _field(panel, "holder_count_change") if "holder_count_change" in panel.columns else _diff(_field(panel, "fund_share"), 28)
    smart_money = _inst_flow_divergence(_ret(close, 9), inst_change, 9)
    retail_pressure = -_zscore(holder_change, 28)
    raw = 0.44 * VARIANT_WEIGHT * smart_money + 0.25 * VARIANT_WEIGHT * retail_pressure + _ret(close, 28)
    score = _risk_scale(_cs_rank(raw), panel, 17)
    # instance: 机构散户资金博弈_v2 | style: 共振 | seed: 20257511
