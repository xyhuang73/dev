...
STRATEGY_ID = "DUOYU000037"
STRATEGY_NAME = "多雨定制动量风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000037"
CATEGORY = "风险调整动量"
STYLE = "动量"
...
ECONOMIC_LOGIC_CN = "该策略以约 20 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 20 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风险调整动量_v2 | style: 动量 | seed: 20242211
