...
STRATEGY_ID = "DUOYU001104"
STRATEGY_NAME = "多雨定制修复三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001104"
CATEGORY = "三重滤网组合"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 三重滤网组合_v2 | style: 修复 | seed: 20245911
