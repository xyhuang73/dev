...
STRATEGY_ID = "DUOYU006215"
STRATEGY_NAME = "多雨定制轮动尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006215"
CATEGORY = "尾部风险阈值"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 尾部风险阈值_v7 | style: 轮动 | seed: 20274016
