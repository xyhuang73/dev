...
STRATEGY_ID = "DUOYU008984"
STRATEGY_NAME = "多雨定制修复折溢价修复策略"
SOURCE_FACTOR_ID = "DUOYU008984"
CATEGORY = "折溢价修复"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 折溢价修复_v5 | style: 修复 | seed: 20291914
