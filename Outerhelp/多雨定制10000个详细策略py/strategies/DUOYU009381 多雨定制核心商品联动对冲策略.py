...
STRATEGY_ID = "DUOYU009381"
STRATEGY_NAME = "多雨定制核心商品联动对冲策略"
SOURCE_FACTOR_ID = "DUOYU009381"
CATEGORY = "商品联动对冲"
STYLE = "核心"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。"
RISK_NOTE_CN = "商品 标的 受 contango/backwardation 影响。 核心风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 商品联动对冲_v6 | style: 核心 | seed: 20294615
