...
STRATEGY_ID = "DUOYU007210"
STRATEGY_NAME = "多雨定制共振波动率压缩预警策略"
SOURCE_FACTOR_ID = "DUOYU007210"
CATEGORY = "波动率压缩预警"
STYLE = "共振"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率压缩预警_v1 | style: 共振 | seed: 20280510
