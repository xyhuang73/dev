...
STRATEGY_ID = "DUOYU001644"
STRATEGY_NAME = "多雨定制修复回撤反转策略"
SOURCE_FACTOR_ID = "DUOYU001644"
CATEGORY = "回撤反转"
STYLE = "修复"
OPERATOR_ID = "回撤反转_v9"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
RISK_NOTE_CN = "趋势反转而非回撤时会产生连续亏损。 修复风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 11 个交易日。", "先计算 回撤反转 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU001644 - 多雨定制修复回撤反转策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤反转_v9 | style: 修复 | seed: 20247918
