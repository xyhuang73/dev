...
STRATEGY_ID = "DUOYU007153"
STRATEGY_NAME = "多雨定制分散状态条件风险预算策略"
SOURCE_FACTOR_ID = "DUOYU007153"
CATEGORY = "状态条件风险预算"
STYLE = "分散"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。"
RISK_NOTE_CN = "风险预算参数依赖历史估计。 分散风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 状态条件风险预算_v6 | style: 分散 | seed: 20279815
