...
STRATEGY_ID = "DUOYU000257"
STRATEGY_NAME = "多雨定制动量多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000257"
CATEGORY = "多时间框架动量"
STYLE = "动量"
OPERATOR_ID = "多时间框架动量_v2"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    ret_m = _ret(close, 10)
    ret_l = _ret(close, 29)
    raw = 0.34 * VARIANT_WEIGHT * ret_s + 0.32 * VARIANT_WEIGHT * ret_m + 0.26 * VARIANT_WEIGHT * ret_l - 0.2 * _downside_vol(close, 29)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 多时间框架动量_v2 | style: 动量 | seed: 20243211
