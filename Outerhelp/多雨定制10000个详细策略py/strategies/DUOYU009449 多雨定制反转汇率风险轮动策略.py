...
STRATEGY_ID = "DUOYU009449"
STRATEGY_NAME = "多雨定制反转汇率风险轮动策略"
SOURCE_FACTOR_ID = "DUOYU009449"
CATEGORY = "汇率风险轮动"
STYLE = "反转"
OPERATOR_ID = "汇率风险轮动_v3"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    currency_impact = _ret(close, 8) - _ret(nav, 8)
    raw = 0.31 * VARIANT_WEIGHT * _zscore(currency_impact, 8) + _ret(nav, 20)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 汇率风险轮动_v3 | style: 反转 | seed: 20296412
