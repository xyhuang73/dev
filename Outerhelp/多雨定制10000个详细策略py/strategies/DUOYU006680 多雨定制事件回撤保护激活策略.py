...
STRATEGY_ID = "DUOYU006680"
STRATEGY_NAME = "多雨定制事件回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006680"
CATEGORY = "回撤保护激活"
STYLE = "事件"
OPERATOR_ID = "回撤保护激活_v6"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 14
FAST_WINDOW = 11
SLOW_WINDOW = 23
VARIANT_WEIGHT = 0.98
REQUIRED_FIELDS = ["close", "net_flow", "drawdown"]

ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "保护机制在V形底可能过早离场。 事件风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 14 个交易日。", "先计算 回撤保护激活 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU006680 - 多雨定制事件回撤保护激活策略

策略主题：回撤保护激活 / 事件
适用范围：宽基/行业/主题
主要周期：14 个交易日
必需字段：close, net_flow, drawdown

经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _repair_slope(close: pd.Series, window: int) -> pd.Series:
    w = max(4, int(window))
    lag = max(2, w // 2)
    return _by_etf(close, lambda x: (x - x.shift(lag)) / (x.shift(lag).abs() + EPS))
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    net_flow = _field(panel, "net_flow")
    if "drawdown" in panel.columns:
        drawdown = _field(panel, "drawdown")
    else:
        rolling_peak = _rolling_max(close, 25)
        drawdown = close / (rolling_peak.abs() + EPS) - 1.0
    dd_guard = _max_dd_guard(drawdown, -0.16)
    raw = -_zscore(drawdown, 25) * dd_guard + 0.37 * VARIANT_WEIGHT * _zscore(net_flow, 11) + _repair_slope(close, 11)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 回撤保护激活_v6 | style: 事件 | seed: 20277515
