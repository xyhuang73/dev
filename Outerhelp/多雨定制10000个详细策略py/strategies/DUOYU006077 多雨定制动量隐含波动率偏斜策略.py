...
STRATEGY_ID = "DUOYU006077"
STRATEGY_NAME = "多雨定制动量隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU006077"
CATEGORY = "隐含波动率偏斜"
STYLE = "动量"
OPERATOR_ID = "隐含波动率偏斜_v8"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 9
SLOW_WINDOW = 16
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.37 * VARIANT_WEIGHT * _zscore(iv_change, 8) + _ret(close, 19)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 隐含波动率偏斜_v8 | style: 动量 | seed: 20273217
