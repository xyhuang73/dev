...
STRATEGY_ID = "DUOYU008744"
STRATEGY_NAME = "多雨定制修复日历效应季节策略"
SOURCE_FACTOR_ID = "DUOYU008744"
CATEGORY = "日历效应季节"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：日历效应（季节性规律）+成交确认，捕捉每年固定时间窗口的统计规律。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：日历效应（季节性规律）+成交确认，捕捉每年固定时间窗口的统计规律。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 日历效应季节_v7 | style: 修复 | seed: 20289916
