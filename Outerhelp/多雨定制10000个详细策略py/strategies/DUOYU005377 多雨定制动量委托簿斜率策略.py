...
STRATEGY_ID = "DUOYU005377"
STRATEGY_NAME = "多雨定制动量委托簿斜率策略"
SOURCE_FACTOR_ID = "DUOYU005377"
CATEGORY = "委托簿斜率"
STYLE = "动量"
OPERATOR_ID = "委托簿斜率_v6"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 11
FAST_WINDOW = 12
SLOW_WINDOW = 16
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.39 * VARIANT_WEIGHT * _zscore(book_slope, 11) + _ret(close, 20)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 委托簿斜率_v6 | style: 动量 | seed: 20269215
