...
STRATEGY_ID = "DUOYU001618"
STRATEGY_NAME = "多雨定制质量回撤反转策略"
SOURCE_FACTOR_ID = "DUOYU001618"
CATEGORY = "回撤反转"
STYLE = "质量"
OPERATOR_ID = "回撤反转_v7"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤反转_v7 | style: 质量 | seed: 20249316
