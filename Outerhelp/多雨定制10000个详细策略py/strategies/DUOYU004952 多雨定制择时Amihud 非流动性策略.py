...
STRATEGY_ID = "DUOYU004952"
STRATEGY_NAME = "多雨定制择时Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU004952"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
RISK_NOTE_CN = "Amihud 在小盘 标的 中噪声较大。 择时风格下回撤控制较为严格。"
...
"""
DUOYU004952 - 多雨定制择时Amihud 非流动性策略
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: amihud_非流动性_v3 | style: 择时 | seed: 20266712
