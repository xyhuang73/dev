...
STRATEGY_ID = "DUOYU008360"
STRATEGY_NAME = "多雨定制事件行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008360"
CATEGORY = "行业景气扩散"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 行业景气扩散_v1 | style: 事件 | seed: 20289510
