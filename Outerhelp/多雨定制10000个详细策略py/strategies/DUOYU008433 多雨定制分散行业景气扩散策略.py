...
STRATEGY_ID = "DUOYU008433"
STRATEGY_NAME = "多雨定制分散行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008433"
CATEGORY = "行业景气扩散"
STYLE = "分散"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。"
RISK_NOTE_CN = "景气度指标有发布周期限制。 分散风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 10 个交易日。", "先计算 行业景气扩散 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008433 - 多雨定制分散行业景气扩散策略
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 行业景气扩散_v5 | style: 分散 | seed: 20288814
