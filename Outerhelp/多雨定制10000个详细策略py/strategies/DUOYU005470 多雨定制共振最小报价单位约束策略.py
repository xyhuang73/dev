...
STRATEGY_ID = "DUOYU005470"
STRATEGY_NAME = "多雨定制共振最小报价单位约束策略"
SOURCE_FACTOR_ID = "DUOYU005470"
CATEGORY = "最小报价单位约束"
STYLE = "共振"
OPERATOR_ID = "最小报价单位约束_v2"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 15
FAST_WINDOW = 12
SLOW_WINDOW = 17
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.41 * VARIANT_WEIGHT * _zscore(tick_constraint, 17) + 0.24 * VARIANT_WEIGHT * _amount_z(panel, 12)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 最小报价单位约束_v2 | style: 共振 | seed: 20269511
