...
STRATEGY_ID = "DUOYU006726"
STRATEGY_NAME = "多雨定制对冲保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006726"
CATEGORY = "保证金追缴级联代理"
STYLE = "对冲"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 保证金追缴级联代理_v1 | style: 对冲 | seed: 20277110
