...
STRATEGY_ID = "DUOYU004896"
STRATEGY_NAME = "多雨定制持有流动性冲击吸收策略"
SOURCE_FACTOR_ID = "DUOYU004896"
CATEGORY = "流动性冲击吸收"
STYLE = "持有"
OPERATOR_ID = "流动性冲击吸收_v9"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    shock = _zscore(turnover, 12) - _zscore(turnover, 17)
    absorption = _ret(close, 12) / (_zscore(turnover, 12).abs() + EPS)
    raw = 0.17 * VARIANT_WEIGHT * shock + 0.36 * VARIANT_WEIGHT * _zscore(absorption, 12)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 流动性冲击吸收_v9 | style: 持有 | seed: 20266118
