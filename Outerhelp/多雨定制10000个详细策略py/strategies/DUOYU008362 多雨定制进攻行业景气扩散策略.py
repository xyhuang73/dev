...
STRATEGY_ID = "DUOYU008362"
STRATEGY_NAME = "多雨定制进攻行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008362"
CATEGORY = "行业景气扩散"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
RISK_NOTE_CN = "景气度指标有发布周期限制。 进攻风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 行业景气扩散_v2 | style: 进攻 | seed: 20287711
