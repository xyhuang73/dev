...
STRATEGY_ID = "DUOYU004870"
STRATEGY_NAME = "多雨定制共振流动性冲击吸收策略"
SOURCE_FACTOR_ID = "DUOYU004870"
CATEGORY = "流动性冲击吸收"
STYLE = "共振"
OPERATOR_ID = "流动性冲击吸收_v8"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "成交放量可能由事件驱动而非流动性变化。 共振风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    shock = _zscore(turnover, 10) - _zscore(turnover, 21)
    absorption = _ret(close, 10) / (_zscore(turnover, 10).abs() + EPS)
    raw = 0.19 * VARIANT_WEIGHT * shock + 0.36 * VARIANT_WEIGHT * _zscore(absorption, 10)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 流动性冲击吸收_v8 | style: 共振 | seed: 20265517
