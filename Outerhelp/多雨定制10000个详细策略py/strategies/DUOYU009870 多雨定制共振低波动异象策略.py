...
STRATEGY_ID = "DUOYU009870"
STRATEGY_NAME = "多雨定制共振低波动异象策略"
SOURCE_FACTOR_ID = "DUOYU009870"
CATEGORY = "低波动异象"
STYLE = "共振"
OPERATOR_ID = "低波动异象_v6"
MARKET_SCOPE = "债券/红利/低波"
HORIZON_DAYS = 18
FAST_WINDOW = 13
SLOW_WINDOW = 21
VARIANT_WEIGHT = 0.91
REQUIRED_FIELDS = ["close", "downside_vol", "quality_score", "dividend_yield"]

ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "低波异象在流动性驱动牛市中表现弱。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 18 个交易日。", "先计算 低波动异象 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009870 - 多雨定制共振低波动异象策略

策略主题：低波动异象 / 共振
适用范围：债券/红利/低波
主要周期：18 个交易日
必需字段：close, downside_vol, quality_score, dividend_yield

经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    downside = _field(panel, "downside_vol")
    quality = _field(panel, "quality_score") if "quality_score" in panel.columns else _zscore(_field(panel, "dividend_yield"), 21)
    raw = -0.83 * VARIANT_WEIGHT * _downside_vol(close, 21) + 0.15 * VARIANT_WEIGHT * _zscore(quality, 21) + _ret(close, 21)
    score = _risk_scale(_cs_rank(raw), panel, 18)
    # instance: 低波动异象_v6 | style: 共振 | seed: 20299515
