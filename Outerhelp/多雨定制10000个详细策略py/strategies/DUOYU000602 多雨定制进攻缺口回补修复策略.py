...
STRATEGY_ID = "DUOYU000602"
STRATEGY_NAME = "多雨定制进攻缺口回补修复策略"
SOURCE_FACTOR_ID = "DUOYU000602"
CATEGORY = "缺口回补修复"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 缺口回补修复_v9 | style: 进攻 | seed: 20242718
