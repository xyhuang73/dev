...
STRATEGY_ID = "DUOYU005785"
STRATEGY_NAME = "多雨定制确认已实现波动率状态切换策略"
SOURCE_FACTOR_ID = "DUOYU005785"
CATEGORY = "已实现波动率状态切换"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
...
"""
DUOYU005785 - 多雨定制确认已实现波动率状态切换策略
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 已实现波动率状态切换_v2 | style: 确认 | seed: 20271011
