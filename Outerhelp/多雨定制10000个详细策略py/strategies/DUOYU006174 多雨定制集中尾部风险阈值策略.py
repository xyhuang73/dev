...
STRATEGY_ID = "DUOYU006174"
STRATEGY_NAME = "多雨定制集中尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006174"
CATEGORY = "尾部风险阈值"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 20 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
RISK_NOTE_CN = "尾部风险模型依赖历史分布假设。 集中风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 20 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 尾部风险阈值_v5 | style: 集中 | seed: 20273914
