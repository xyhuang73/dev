...
STRATEGY_ID = "DUOYU000029"
STRATEGY_NAME = "多雨定制反转风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000029"
CATEGORY = "风险调整动量"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 19 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 19 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风险调整动量_v2 | style: 反转 | seed: 20241411
