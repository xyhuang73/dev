...
STRATEGY_ID = "DUOYU001842"
STRATEGY_NAME = "多雨定制进攻横截面动量差异策略"
SOURCE_FACTOR_ID = "DUOYU001842"
CATEGORY = "横截面动量差异"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 横截面动量差异_v9 | style: 进攻 | seed: 20248718
