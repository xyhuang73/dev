...
STRATEGY_ID = "DUOYU002290"
STRATEGY_NAME = "多雨定制共振日内积累分布策略"
SOURCE_FACTOR_ID = "DUOYU002290"
CATEGORY = "日内积累分布"
STYLE = "共振"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 日内积累分布_v1 | style: 共振 | seed: 20252510
