...
STRATEGY_ID = "DUOYU003949"
STRATEGY_NAME = "多雨定制反转北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003949"
CATEGORY = "北向资金泄漏"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 北向资金泄漏_v7 | style: 反转 | seed: 20260416
