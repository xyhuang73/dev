...
STRATEGY_ID = "DUOYU005375"
STRATEGY_NAME = "多雨定制轮动委托簿斜率策略"
SOURCE_FACTOR_ID = "DUOYU005375"
CATEGORY = "委托簿斜率"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 委托簿斜率_v6 | style: 轮动 | seed: 20269015
