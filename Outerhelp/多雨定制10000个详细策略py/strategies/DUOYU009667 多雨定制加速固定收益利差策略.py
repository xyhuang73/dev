...
STRATEGY_ID = "DUOYU009667"
STRATEGY_NAME = "多雨定制加速固定收益利差策略"
SOURCE_FACTOR_ID = "DUOYU009667"
CATEGORY = "固定收益利差"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
RISK_NOTE_CN = "利率变化对 carry 策略影响重大。 加速风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 固定收益利差_v2 | style: 加速 | seed: 20298211
