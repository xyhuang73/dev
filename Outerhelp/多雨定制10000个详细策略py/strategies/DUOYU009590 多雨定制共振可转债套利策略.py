...
STRATEGY_ID = "DUOYU009590"
STRATEGY_NAME = "多雨定制共振可转债套利策略"
SOURCE_FACTOR_ID = "DUOYU009590"
CATEGORY = "可转债套利"
STYLE = "共振"
OPERATOR_ID = "可转债套利_v4"
MARKET_SCOPE = "跨境/商品/场内"
HORIZON_DAYS = 14
FAST_WINDOW = 12
SLOW_WINDOW = 24
VARIANT_WEIGHT = 0.91
REQUIRED_FIELDS = ["close", "premium_discount", "net_flow"]

ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "套利机制依赖 标的 申赎效率。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 14 个交易日。", "先计算 可转债套利 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009590 - 多雨定制共振可转债套利策略

策略主题：可转债套利 / 共振
适用范围：跨境/商品/场内
主要周期：14 个交易日
必需字段：close, premium_discount, net_flow

经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    arb_signal = -_zscore(premium, 12) * _sigmoid_gate(_zscore(net_flow, 12), 0, 3)
    raw = 0.29 * VARIANT_WEIGHT * arb_signal + _ret(close, 24)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 可转债套利_v4 | style: 共振 | seed: 20297513
