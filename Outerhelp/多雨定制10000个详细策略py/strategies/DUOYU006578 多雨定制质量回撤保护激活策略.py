...
STRATEGY_ID = "DUOYU006578"
STRATEGY_NAME = "多雨定制质量回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006578"
CATEGORY = "回撤保护激活"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤保护激活_v1 | style: 质量 | seed: 20277310
