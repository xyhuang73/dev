...
STRATEGY_ID = "DUOYU002115"
STRATEGY_NAME = "多雨定制轮动自适应动量衰减策略"
SOURCE_FACTOR_ID = "DUOYU002115"
CATEGORY = "自适应动量衰减"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 自适应动量衰减_v2 | style: 轮动 | seed: 20252011
