...
STRATEGY_ID = "DUOYU007450"
STRATEGY_NAME = "多雨定制共振政策事件评分策略"
SOURCE_FACTOR_ID = "DUOYU007450"
CATEGORY = "政策事件评分"
STYLE = "共振"
OPERATOR_ID = "政策事件评分_v5"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 14
FAST_WINDOW = 14
SLOW_WINDOW = 17
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "政策事件评分的 NLP 模型存在误判风险。 共振风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _zscore(policy, 14) * _event_proximity(distance, 14) + 0.14 * VARIANT_WEIGHT * _amount_z(panel, 14)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 政策事件评分_v5 | style: 共振 | seed: 20281514
