...
STRATEGY_ID = "DUOYU000815"
STRATEGY_NAME = "多雨定制轮动成交量剖面支撑策略"
SOURCE_FACTOR_ID = "DUOYU000815"
CATEGORY = "成交量剖面支撑"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：基于 VWAP 锚定的成交量支撑信号，衡量价格位于成交量加权均价上方的强度。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：基于 VWAP 锚定的成交量支撑信号，衡量价格位于成交量加权均价上方的强度。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 成交量剖面支撑_v8 | style: 轮动 | seed: 20245017
