...
STRATEGY_ID = "DUOYU006137"
STRATEGY_NAME = "多雨定制动量尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006137"
CATEGORY = "尾部风险阈值"
STYLE = "动量"
OPERATOR_ID = "尾部风险阈值_v3"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    tail_z = _tail_risk_z(close, 22, 0.05)
    raw = 0.4 * VARIANT_WEIGHT * _zscore(tail_z, 5) + _ret(close, 22)
    raw = raw * _max_dd_guard(drawdown, -0.10)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 尾部风险阈值_v3 | style: 动量 | seed: 20274212
