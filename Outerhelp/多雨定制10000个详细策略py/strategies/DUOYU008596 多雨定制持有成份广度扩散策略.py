...
STRATEGY_ID = "DUOYU008596"
STRATEGY_NAME = "多雨定制持有成份广度扩散策略"
SOURCE_FACTOR_ID = "DUOYU008596"
CATEGORY = "成份广度扩散"
STYLE = "持有"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 成份广度扩散_v6 | style: 持有 | seed: 20290115
