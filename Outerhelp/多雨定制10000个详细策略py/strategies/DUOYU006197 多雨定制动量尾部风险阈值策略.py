...
STRATEGY_ID = "DUOYU006197"
STRATEGY_NAME = "多雨定制动量尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006197"
CATEGORY = "尾部风险阈值"
STYLE = "动量"
OPERATOR_ID = "尾部风险阈值_v6"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    tail_z = _tail_risk_z(close, 19, 0.05)
    raw = 0.35 * VARIANT_WEIGHT * _zscore(tail_z, 14) + _ret(close, 19)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 尾部风险阈值_v6 | style: 动量 | seed: 20274215
