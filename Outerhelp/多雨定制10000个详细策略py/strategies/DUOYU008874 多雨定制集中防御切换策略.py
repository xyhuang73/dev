...
STRATEGY_ID = "DUOYU008874"
STRATEGY_NAME = "多雨定制集中防御切换策略"
SOURCE_FACTOR_ID = "DUOYU008874"
CATEGORY = "防御切换"
STYLE = "集中"
OPERATOR_ID = "防御切换_v6"
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 防御切换_v6 | style: 集中 | seed: 20291915
