...
STRATEGY_ID = "DUOYU008412"
STRATEGY_NAME = "多雨定制择时行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008412"
CATEGORY = "行业景气扩散"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 行业景气扩散_v4 | style: 择时 | seed: 20288713
