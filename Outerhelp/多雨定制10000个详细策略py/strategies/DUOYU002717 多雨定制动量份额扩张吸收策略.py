...
STRATEGY_ID = "DUOYU002717"
STRATEGY_NAME = "多雨定制动量份额扩张吸收策略"
SOURCE_FACTOR_ID = "DUOYU002717"
CATEGORY = "份额扩张吸收"
STYLE = "动量"
OPERATOR_ID = "份额扩张吸收_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    resilience = _ret(close, 11) - _downside_vol(close, 11)
    raw = _diff(fund_share, 26) * resilience + 0.24 * VARIANT_WEIGHT * _amount_z(panel, 26)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 份额扩张吸收_v2 | style: 动量 | seed: 20255211
