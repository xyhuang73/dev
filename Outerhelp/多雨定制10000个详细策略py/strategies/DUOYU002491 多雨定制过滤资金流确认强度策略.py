...
STRATEGY_ID = "DUOYU002491"
STRATEGY_NAME = "多雨定制过滤资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002491"
CATEGORY = "资金流确认强度"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
RISK_NOTE_CN = "大额申赎可能导致资金流指标短期失真。 过滤风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 资金流确认强度 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU002491 - 多雨定制过滤资金流确认强度策略
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 资金流确认强度_v1 | style: 过滤 | seed: 20253610
