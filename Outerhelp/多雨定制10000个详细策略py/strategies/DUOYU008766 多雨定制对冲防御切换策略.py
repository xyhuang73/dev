...
STRATEGY_ID = "DUOYU008766"
STRATEGY_NAME = "多雨定制对冲防御切换策略"
SOURCE_FACTOR_ID = "DUOYU008766"
CATEGORY = "防御切换"
STYLE = "对冲"
OPERATOR_ID = "防御切换_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。"
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 防御切换_v1 | style: 对冲 | seed: 20291110
