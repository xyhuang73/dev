...
STRATEGY_ID = "DUOYU001270"
STRATEGY_NAME = "多雨定制共振三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001270"
CATEGORY = "三重滤网组合"
STYLE = "共振"
OPERATOR_ID = "三重滤网组合_v10"
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    amount_z = _amount_z(panel, 11)
    trend_score = _ret(close, 20)
    vol_score = -_downside_vol(close, 17)
    flow_confirm = _zscore(_field(panel, "net_flow"), 11) if "net_flow" in panel.columns else amount_z
    raw = 0.49 * VARIANT_WEIGHT * trend_score + 0.38 * VARIANT_WEIGHT * vol_score + 0.19 * VARIANT_WEIGHT * flow_confirm
    score = _risk_scale(_cs_rank(raw), panel, 17)
    # instance: 三重滤网组合_v10 | style: 共振 | seed: 20246519
