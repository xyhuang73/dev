...
STRATEGY_ID = "DUOYU003117"
STRATEGY_NAME = "多雨定制动量基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003117"
CATEGORY = "基金持仓集中度"
STYLE = "动量"
OPERATOR_ID = "基金持仓集中度_v2"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 18
FAST_WINDOW = 10
SLOW_WINDOW = 22
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    divergence = _inst_flow_divergence(_ret(_field(panel, "close"), 9), inst_change, 9)
    raw = 0.25 * VARIANT_WEIGHT * _zscore(inst_holding, 27) + 0.49 * VARIANT_WEIGHT * divergence - 0.26 * VARIANT_WEIGHT * _concentration_risk(top10_conc, 27)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 基金持仓集中度_v2 | style: 动量 | seed: 20257211
