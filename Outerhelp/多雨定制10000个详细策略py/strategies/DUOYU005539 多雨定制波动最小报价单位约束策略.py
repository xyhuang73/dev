...
STRATEGY_ID = "DUOYU005539"
STRATEGY_NAME = "多雨定制波动最小报价单位约束策略"
SOURCE_FACTOR_ID = "DUOYU005539"
CATEGORY = "最小报价单位约束"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 最小报价单位约束_v5 | style: 波动 | seed: 20270414
