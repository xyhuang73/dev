...
STRATEGY_ID = "DUOYU006770"
STRATEGY_NAME = "多雨定制共振保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006770"
CATEGORY = "保证金追缴级联代理"
STYLE = "共振"
OPERATOR_ID = "保证金追缴级联代理_v3"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 16
FAST_WINDOW = 8
SLOW_WINDOW = 19
VARIANT_WEIGHT = 0.91
REQUIRED_FIELDS = ["close", "margin_ratio", "amount"]

ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "保证金数据频率低，代理变量精度有限。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 16 个交易日。", "先计算 保证金追缴级联代理 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU006770 - 多雨定制共振保证金追缴级联代理策略

策略主题：保证金追缴级联代理 / 共振
适用范围：宽基/行业
主要周期：16 个交易日
必需字段：close, margin_ratio, amount

经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    margin_ratio = _field(panel, "margin_ratio") if "margin_ratio" in panel.columns else _zscore(_amount_z(panel, 8), 19)
    downside = _downside_vol(close, 19)
    margin_stress = _zscore(margin_ratio, 8) * downside
    raw = -0.3 * VARIANT_WEIGHT * margin_stress + _ret(close, 19)
    raw = raw * _max_dd_guard(drawdown, -0.15)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 保证金追缴级联代理_v3 | style: 共振 | seed: 20277512
