...
STRATEGY_ID = "DUOYU007279"
STRATEGY_NAME = "多雨定制波动波动率压缩预警策略"
SOURCE_FACTOR_ID = "DUOYU007279"
CATEGORY = "波动率压缩预警"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
RISK_NOTE_CN = "波动压缩持续时间不可预测。 波动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率压缩预警_v4 | style: 波动 | seed: 20281413
