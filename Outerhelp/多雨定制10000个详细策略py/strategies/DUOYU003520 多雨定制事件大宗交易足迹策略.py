...
STRATEGY_ID = "DUOYU003520"
STRATEGY_NAME = "多雨定制事件大宗交易足迹策略"
SOURCE_FACTOR_ID = "DUOYU003520"
CATEGORY = "大宗交易足迹"
STYLE = "事件"
OPERATOR_ID = "大宗交易足迹_v3"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 16
FAST_WINDOW = 14
SLOW_WINDOW = 20
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：大宗交易溢价/折价反映机构投资者对当前价格的判断，溢价=看好、折价=看淡。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：大宗交易溢价/折价反映机构投资者对当前价格的判断，溢价=看好、折价=看淡。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    block_premium = _field(panel, "block_trade_premium") if "block_trade_premium" in panel.columns else _zscore(_amount_z(panel, 13) - _amount_z(panel, 22), 13)
    raw = 0.37 * VARIANT_WEIGHT * _zscore(block_premium, 13) + _ret(close, 22)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 大宗交易足迹_v3 | style: 事件 | seed: 20259512
