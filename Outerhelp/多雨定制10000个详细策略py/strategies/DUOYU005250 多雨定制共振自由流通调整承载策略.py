...
STRATEGY_ID = "DUOYU005250"
STRATEGY_NAME = "多雨定制共振自由流通调整承载策略"
SOURCE_FACTOR_ID = "DUOYU005250"
CATEGORY = "自由流通调整承载"
STYLE = "共振"
OPERATOR_ID = "自由流通调整承载_v9"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 14
FAST_WINDOW = 6
SLOW_WINDOW = 22
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    free_float_adj = _zscore(turnover, 22) / (1.0 + _zscore(spread, 22).abs())
    raw = 0.29 * VARIANT_WEIGHT * _zscore(free_float_adj, 6) + 0.15 * VARIANT_WEIGHT * _amount_z(panel, 6)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 自由流通调整承载_v9 | style: 共振 | seed: 20267518
