...
STRATEGY_ID = "DUOYU003714"
STRATEGY_NAME = "多雨定制集中融资融券压力策略"
SOURCE_FACTOR_ID = "DUOYU003714"
CATEGORY = "融资融券压力"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高融资比例+价格上涨=杠杆驱动行情（需警惕），高融券+价格下跌=空头挤压机会。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高融资比例+价格上涨=杠杆驱动行情（需警惕），高融券+价格下跌=空头挤压机会。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 融资融券压力_v4 | style: 集中 | seed: 20259913
