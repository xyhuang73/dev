...
STRATEGY_ID = "DUOYU007984"
STRATEGY_NAME = "多雨定制修复指数调整前置策略"
SOURCE_FACTOR_ID = "DUOYU007984"
CATEGORY = "指数调整前置"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 指数调整前置_v4 | style: 修复 | seed: 20284913
