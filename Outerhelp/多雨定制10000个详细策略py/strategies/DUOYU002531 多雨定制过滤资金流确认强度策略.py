...
STRATEGY_ID = "DUOYU002531"
STRATEGY_NAME = "多雨定制过滤资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002531"
CATEGORY = "资金流确认强度"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 资金流确认强度_v3 | style: 过滤 | seed: 20253612
