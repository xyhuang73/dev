...
STRATEGY_ID = "DUOYU004351"
STRATEGY_NAME = "多雨定制过滤申赎套利流策略"
SOURCE_FACTOR_ID = "DUOYU004351"
CATEGORY = "申赎套利流"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：折溢价套利资金流：溢价回归+净值上涨=套利与趋势共振，折价扩大+净值下跌=压力。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：折溢价套利资金流：溢价回归+净值上涨=套利与趋势共振，折价扩大+净值下跌=压力。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 申赎套利流_v9 | style: 过滤 | seed: 20262618
