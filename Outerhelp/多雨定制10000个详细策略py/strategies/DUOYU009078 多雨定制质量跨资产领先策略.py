...
STRATEGY_ID = "DUOYU009078"
STRATEGY_NAME = "多雨定制质量跨资产领先策略"
SOURCE_FACTOR_ID = "DUOYU009078"
CATEGORY = "跨资产领先"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 跨资产领先_v2 | style: 质量 | seed: 20294311
