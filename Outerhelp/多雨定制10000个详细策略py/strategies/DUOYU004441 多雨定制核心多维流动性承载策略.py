...
STRATEGY_ID = "DUOYU004441"
STRATEGY_NAME = "多雨定制核心多维流动性承载策略"
SOURCE_FACTOR_ID = "DUOYU004441"
CATEGORY = "多维流动性承载"
STYLE = "核心"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。"
RISK_NOTE_CN = "流动性指标在极端行情中可能突变。 核心风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 多维流动性承载_v5 | style: 核心 | seed: 20262614
