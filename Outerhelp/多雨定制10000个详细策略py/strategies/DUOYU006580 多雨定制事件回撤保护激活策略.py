...
STRATEGY_ID = "DUOYU006580"
STRATEGY_NAME = "多雨定制事件回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006580"
CATEGORY = "回撤保护激活"
STYLE = "事件"
OPERATOR_ID = "回撤保护激活_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    if "drawdown" in panel.columns:
        ...
        rolling_peak = _rolling_max(close, 28)
        drawdown = close / (rolling_peak.abs() + EPS) - 1.0
    dd_guard = _max_dd_guard(drawdown, -0.13)
    raw = -_zscore(drawdown, 28) * dd_guard + 0.31 * VARIANT_WEIGHT * _zscore(net_flow, 12) + _repair_slope(close, 12)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 回撤保护激活_v1 | style: 事件 | seed: 20277510
