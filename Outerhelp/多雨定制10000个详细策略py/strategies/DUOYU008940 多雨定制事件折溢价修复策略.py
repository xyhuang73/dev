...
STRATEGY_ID = "DUOYU008940"
STRATEGY_NAME = "多雨定制事件折溢价修复策略"
SOURCE_FACTOR_ID = "DUOYU008940"
CATEGORY = "折溢价修复"
STYLE = "事件"
OPERATOR_ID = "折溢价修复_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -_zscore(premium, 16) + 0.36 * VARIANT_WEIGHT * _ret(nav, 16) + 0.11 * VARIANT_WEIGHT * _ret(close, 10)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 折溢价修复_v2 | style: 事件 | seed: 20293511
