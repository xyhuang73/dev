...
STRATEGY_ID = "DUOYU004037"
STRATEGY_NAME = "多雨定制动量行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004037"
CATEGORY = "行业资金轮动动量"
STYLE = "动量"
OPERATOR_ID = "行业资金轮动动量_v2"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 10
FAST_WINDOW = 5
SLOW_WINDOW = 19
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    sector_relative = _cs_rank(_zscore(net_flow, 5)) - _cs_rank(_zscore(net_flow, 23))
    raw = 0.3 * VARIANT_WEIGHT * sector_relative + _ret(close, 5)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 行业资金轮动动量_v2 | style: 动量 | seed: 20262211
