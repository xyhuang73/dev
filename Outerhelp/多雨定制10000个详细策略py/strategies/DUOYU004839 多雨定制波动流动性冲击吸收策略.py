...
STRATEGY_ID = "DUOYU004839"
STRATEGY_NAME = "多雨定制波动流动性冲击吸收策略"
SOURCE_FACTOR_ID = "DUOYU004839"
CATEGORY = "流动性冲击吸收"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
RISK_NOTE_CN = "成交放量可能由事件驱动而非流动性变化。 波动风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 10 个交易日。", "先计算 流动性冲击吸收 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU004839 - 多雨定制波动流动性冲击吸收策略
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 流动性冲击吸收_v6 | style: 波动 | seed: 20266415
