...
STRATEGY_ID = "DUOYU008429"
STRATEGY_NAME = "多雨定制反转行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008429"
CATEGORY = "行业景气扩散"
STYLE = "反转"
OPERATOR_ID = "行业景气扩散_v5"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 14
FAST_WINDOW = 12
SLOW_WINDOW = 20
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.47 * VARIANT_WEIGHT * _zscore(prosperity, 22) + 0.28 * VARIANT_WEIGHT * _zscore(revision, 22) + _ret(industry_momentum, 11)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 行业景气扩散_v5 | style: 反转 | seed: 20288414
