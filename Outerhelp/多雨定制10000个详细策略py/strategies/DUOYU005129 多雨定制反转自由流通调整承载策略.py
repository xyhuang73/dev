...
STRATEGY_ID = "DUOYU005129"
STRATEGY_NAME = "多雨定制反转自由流通调整承载策略"
SOURCE_FACTOR_ID = "DUOYU005129"
CATEGORY = "自由流通调整承载"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 自由流通调整承载_v3 | style: 反转 | seed: 20267412
