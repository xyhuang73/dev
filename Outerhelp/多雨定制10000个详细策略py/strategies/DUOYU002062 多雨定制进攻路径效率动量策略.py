...
STRATEGY_ID = "DUOYU002062"
STRATEGY_NAME = "多雨定制进攻路径效率动量策略"
SOURCE_FACTOR_ID = "DUOYU002062"
CATEGORY = "路径效率动量"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
RISK_NOTE_CN = "效率比在V形反转中可能给出滞后信号。 进攻风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 路径效率动量_v10 | style: 进攻 | seed: 20249719
