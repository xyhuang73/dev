...
STRATEGY_ID = "DUOYU004357"
STRATEGY_NAME = "多雨定制动量申赎套利流策略"
SOURCE_FACTOR_ID = "DUOYU004357"
CATEGORY = "申赎套利流"
STYLE = "动量"
OPERATOR_ID = "申赎套利流_v9"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折溢价套利资金流：溢价回归+净值上涨=套利与趋势共振，折价扩大+净值下跌=压力。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折溢价套利资金流：溢价回归+净值上涨=套利与趋势共振，折价扩大+净值下跌=压力。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    arb_flow = -_zscore(premium, 10) * _zscore(_ret(nav, 26), 10)
    raw = 0.4 * VARIANT_WEIGHT * arb_flow + _ret(close, 26)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 申赎套利流_v9 | style: 动量 | seed: 20263218
