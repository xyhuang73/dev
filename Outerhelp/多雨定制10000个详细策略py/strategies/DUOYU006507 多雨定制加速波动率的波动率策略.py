...
STRATEGY_ID = "DUOYU006507"
STRATEGY_NAME = "多雨定制加速波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006507"
CATEGORY = "波动率的波动率"
STYLE = "加速"
OPERATOR_ID = "波动率的波动率_v6"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    close = _field(panel, "close")
    ret_1d = _ret(close, 1)
    vol = _by_etf(ret_1d.abs(), lambda x: x.rolling(28, min_periods=5).std())
    vol_of_vol = _by_etf(vol, lambda x: x.rolling(28, min_periods=5).std())
    raw = -0.3 * VARIANT_WEIGHT * _zscore(vol_of_vol, 10) + _ret(close, 28)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 波动率的波动率_v6 | style: 加速 | seed: 20275215
