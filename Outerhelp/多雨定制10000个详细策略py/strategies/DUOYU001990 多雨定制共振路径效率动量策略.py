...
STRATEGY_ID = "DUOYU001990"
STRATEGY_NAME = "多雨定制共振路径效率动量策略"
SOURCE_FACTOR_ID = "DUOYU001990"
CATEGORY = "路径效率动量"
STYLE = "共振"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 路径效率动量_v6 | style: 共振 | seed: 20250515
