...
STRATEGY_ID = "DUOYU002269"
STRATEGY_NAME = "多雨定制反转自适应动量衰减策略"
SOURCE_FACTOR_ID = "DUOYU002269"
CATEGORY = "自适应动量衰减"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 自适应动量衰减_v10 | style: 反转 | seed: 20251419
