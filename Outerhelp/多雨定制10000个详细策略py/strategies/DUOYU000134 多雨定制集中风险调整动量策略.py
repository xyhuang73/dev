...
STRATEGY_ID = "DUOYU000134"
STRATEGY_NAME = "多雨定制集中风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000134"
CATEGORY = "风险调整动量"
STYLE = "集中"
OPERATOR_ID = "风险调整动量_v7"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
RISK_NOTE_CN = "连续上涨后拥挤和高波动会降低信号稳定性。 集中风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    raw = _ret(close, 19) - 0.35 * VARIANT_WEIGHT * _downside_vol(close, 19) + 0.18 * VARIANT_WEIGHT * _amount_z(panel, 10)
    raw = raw * _sigmoid_gate(_ret(close, 10), 0, 4)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 风险调整动量_v7 | style: 集中 | seed: 20241916
