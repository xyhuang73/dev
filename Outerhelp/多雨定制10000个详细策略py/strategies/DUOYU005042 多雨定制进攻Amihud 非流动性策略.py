...
STRATEGY_ID = "DUOYU005042"
STRATEGY_NAME = "多雨定制进攻Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU005042"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
RISK_NOTE_CN = "Amihud 在小盘 标的 中噪声较大。 进攻风格下回撤风险相对较高。"
...
"""
DUOYU005042 - 多雨定制进攻Amihud 非流动性策略
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: amihud_非流动性_v8 | style: 进攻 | seed: 20265717
