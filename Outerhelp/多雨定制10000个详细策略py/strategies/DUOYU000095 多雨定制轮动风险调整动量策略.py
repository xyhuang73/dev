...
STRATEGY_ID = "DUOYU000095"
STRATEGY_NAME = "多雨定制轮动风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000095"
CATEGORY = "风险调整动量"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风险调整动量_v5 | style: 轮动 | seed: 20242014
