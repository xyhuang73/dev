...
STRATEGY_ID = "DUOYU001090"
STRATEGY_NAME = "多雨定制共振三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001090"
CATEGORY = "三重滤网组合"
STYLE = "共振"
OPERATOR_ID = "三重滤网组合_v1"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    amount_z = _amount_z(panel, 6)
    trend_score = _ret(close, 21)
    vol_score = -_downside_vol(close, 18)
    flow_confirm = _zscore(_field(panel, "net_flow"), 6) if "net_flow" in panel.columns else amount_z
    raw = 0.41 * VARIANT_WEIGHT * trend_score + 0.23 * VARIANT_WEIGHT * vol_score + 0.2 * VARIANT_WEIGHT * flow_confirm
    raw = raw * _max_dd_guard(drawdown, -0.10)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 三重滤网组合_v1 | style: 共振 | seed: 20246510
