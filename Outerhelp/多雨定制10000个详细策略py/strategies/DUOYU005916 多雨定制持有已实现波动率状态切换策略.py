...
STRATEGY_ID = "DUOYU005916"
STRATEGY_NAME = "多雨定制持有已实现波动率状态切换策略"
SOURCE_FACTOR_ID = "DUOYU005916"
CATEGORY = "已实现波动率状态切换"
STYLE = "持有"
OPERATOR_ID = "已实现波动率状态切换_v8"
...
ECONOMIC_LOGIC_CN = "该策略以约 4 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
...
"""
DUOYU005916 - 多雨定制持有已实现波动率状态切换策略
...
经济逻辑：
该策略以约 4 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    vol_regime = _vol_regime(close, 5, 23)
    trend_term = _ret(close, 23) * _sigmoid_gate(vol_regime, 0, 3)
    rev_term = -_ret(close, 5) * (1.0 - _sigmoid_gate(vol_regime, 0, 3))
    raw = np.where(vol_regime > 0, trend_term + 0.16 * VARIANT_WEIGHT * _amount_z(panel, 5),
                   rev_term + 0.16 * VARIANT_WEIGHT * _amount_z(panel, 5))
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 已实现波动率状态切换_v8 | style: 持有 | seed: 20272117
