...
STRATEGY_ID = "DUOYU009769"
STRATEGY_NAME = "多雨定制反转低波动异象策略"
SOURCE_FACTOR_ID = "DUOYU009769"
CATEGORY = "低波动异象"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 低波动异象_v1 | style: 反转 | seed: 20299410
