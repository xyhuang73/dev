...
STRATEGY_ID = "DUOYU009057"
STRATEGY_NAME = "多雨定制动量跨资产领先策略"
SOURCE_FACTOR_ID = "DUOYU009057"
CATEGORY = "跨资产领先"
STYLE = "动量"
OPERATOR_ID = "跨资产领先_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = lead + 0.44 * VARIANT_WEIGHT * _ret(close, 5) + 0.12 * VARIANT_WEIGHT * _zscore(appetite, 28)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 跨资产领先_v1 | style: 动量 | seed: 20294210
