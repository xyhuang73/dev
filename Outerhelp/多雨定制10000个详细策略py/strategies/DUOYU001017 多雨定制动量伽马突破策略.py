...
STRATEGY_ID = "DUOYU001017"
STRATEGY_NAME = "多雨定制动量伽马突破策略"
SOURCE_FACTOR_ID = "DUOYU001017"
CATEGORY = "伽马突破"
STYLE = "动量"
OPERATOR_ID = "伽马突破_v7"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    vol_compression = 1.0 - _cs_rank(_zscore(true_range, 17).abs()).abs()
    breakout = _breakout(panel, 8)
    raw = breakout * vol_compression * 0.82 * VARIANT_WEIGHT + 0.11 * VARIANT_WEIGHT * _amount_z(panel, 8)
    raw = raw * _sigmoid_gate(_amount_z(panel, 8), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 伽马突破_v7 | style: 动量 | seed: 20246216
