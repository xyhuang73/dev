...
STRATEGY_ID = "DUOYU002032"
STRATEGY_NAME = "多雨定制择时路径效率动量策略"
SOURCE_FACTOR_ID = "DUOYU002032"
CATEGORY = "路径效率动量"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 路径效率动量_v8 | style: 择时 | seed: 20250717
