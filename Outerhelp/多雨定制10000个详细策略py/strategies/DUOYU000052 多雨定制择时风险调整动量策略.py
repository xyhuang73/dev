...
STRATEGY_ID = "DUOYU000052"
STRATEGY_NAME = "多雨定制择时风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000052"
CATEGORY = "风险调整动量"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
RISK_NOTE_CN = "连续上涨后拥挤和高波动会降低信号稳定性。 择时风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风险调整动量_v3 | style: 择时 | seed: 20241712
