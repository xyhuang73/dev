...
STRATEGY_ID = "DUOYU006562"
STRATEGY_NAME = "多雨定制进攻回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006562"
CATEGORY = "回撤保护激活"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤保护激活_v1 | style: 进攻 | seed: 20275710
