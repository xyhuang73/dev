...
STRATEGY_ID = "DUOYU006954"
STRATEGY_NAME = "多雨定制集中最大回撤守护策略"
SOURCE_FACTOR_ID = "DUOYU006954"
CATEGORY = "最大回撤守护"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
RISK_NOTE_CN = "阈值设置影响保护效果与误关概率。 集中风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 最大回撤守护_v4 | style: 集中 | seed: 20278913
