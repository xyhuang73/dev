...
STRATEGY_ID = "DUOYU006407"
STRATEGY_NAME = "多雨定制加速波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006407"
CATEGORY = "波动率的波动率"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
RISK_NOTE_CN = "vol-of-vol 估计对参数敏感。 加速风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v1 | style: 加速 | seed: 20275210
