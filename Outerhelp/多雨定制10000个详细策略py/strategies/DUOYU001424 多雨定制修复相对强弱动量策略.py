...
STRATEGY_ID = "DUOYU001424"
STRATEGY_NAME = "多雨定制修复相对强弱动量策略"
SOURCE_FACTOR_ID = "DUOYU001424"
CATEGORY = "相对强弱动量"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 相对强弱动量_v8 | style: 修复 | seed: 20246917
