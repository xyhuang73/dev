...
STRATEGY_ID = "DUOYU004620"
STRATEGY_NAME = "多雨定制事件买卖盘深度失衡策略"
SOURCE_FACTOR_ID = "DUOYU004620"
CATEGORY = "买卖盘深度失衡"
STYLE = "事件"
OPERATOR_ID = "买卖盘深度失衡_v4"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 14
FAST_WINDOW = 5
SLOW_WINDOW = 20
VARIANT_WEIGHT = 0.98
REQUIRED_FIELDS = ["close", "bid_ask_spread", "turnover"]

ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高换手+窄价差=深度买方市场，流动性充裕信号。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "高频 microstructure 数据获取困难。 事件风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 14 个交易日。", "先计算 买卖盘深度失衡 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU004620 - 多雨定制事件买卖盘深度失衡策略

策略主题：买卖盘深度失衡 / 事件
适用范围：流动性较好
主要周期：14 个交易日
必需字段：close, bid_ask_spread, turnover

经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高换手+窄价差=深度买方市场，流动性充裕信号。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    depth_imbalance = (_zscore(turnover, 5) - _zscore(spread, 5).abs()) / (1.0 + _zscore(spread, 22).abs())
    raw = 0.33 * VARIANT_WEIGHT * _zscore(depth_imbalance, 5) + _ret(close, 22)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 买卖盘深度失衡_v4 | style: 事件 | seed: 20265513
