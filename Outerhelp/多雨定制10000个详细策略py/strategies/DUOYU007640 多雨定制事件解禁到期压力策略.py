...
STRATEGY_ID = "DUOYU007640"
STRATEGY_NAME = "多雨定制事件解禁到期压力策略"
SOURCE_FACTOR_ID = "DUOYU007640"
CATEGORY = "解禁到期压力"
STYLE = "事件"
OPERATOR_ID = "解禁到期压力_v7"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 15
FAST_WINDOW = 11
SLOW_WINDOW = 19
VARIANT_WEIGHT = 0.98
REQUIRED_FIELDS = ["close", "lockup_shares_ratio", "lockup_days_remaining"]

ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：解禁比例大+临近解禁日=抛售压力大，解禁过后压力衰减。逆向思维：提前规避。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "解禁计划可能变更或展期。 事件风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 15 个交易日。", "先计算 解禁到期压力 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU007640 - 多雨定制事件解禁到期压力策略

策略主题：解禁到期压力 / 事件
适用范围：宽基/行业/主题
主要周期：15 个交易日
必需字段：close, lockup_shares_ratio, lockup_days_remaining

经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：解禁比例大+临近解禁日=抛售压力大，解禁过后压力衰减。逆向思维：提前规避。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _lockup_pressure(lockup_ratio: pd.Series, days_remaining: pd.Series, decay_halflife: float = 10.0) -> pd.Series:
    """解禁压力指数：高比例+临近解禁日=高压力，随时间指数衰减。"""
    ratio = pd.to_numeric(lockup_ratio, errors="coerce")
    days = pd.to_numeric(days_remaining, errors="coerce")
    decay = np.exp(-np.maximum(days, 0) / decay_halflife)
    return (ratio * decay).replace([np.inf, -np.inf], np.nan)
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    lockup_ratio = _field(panel, "lockup_shares_ratio")
    lockup_days = _field(panel, "lockup_days_remaining")
    pressure = _lockup_pressure(lockup_ratio, lockup_days, 11)
    raw = -0.47 * VARIANT_WEIGHT * _zscore(pressure, 11) + _ret(close, 21)
    raw = raw * _max_dd_guard(drawdown, -0.12)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 解禁到期压力_v7 | style: 事件 | seed: 20283516
