...
STRATEGY_ID = "DUOYU006299"
STRATEGY_NAME = "多雨定制波动相关性破裂检测策略"
SOURCE_FACTOR_ID = "DUOYU006299"
CATEGORY = "相关性破裂检测"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
RISK_NOTE_CN = "相关性估计在短窗口内不稳定。 波动风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 11 个交易日。", "先计算 相关性破裂检测 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU006299 - 多雨定制波动相关性破裂检测策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 相关性破裂检测_v3 | style: 波动 | seed: 20275412
