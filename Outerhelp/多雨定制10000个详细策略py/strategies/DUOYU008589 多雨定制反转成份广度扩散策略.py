...
STRATEGY_ID = "DUOYU008589"
STRATEGY_NAME = "多雨定制反转成份广度扩散策略"
SOURCE_FACTOR_ID = "DUOYU008589"
CATEGORY = "成份广度扩散"
STYLE = "反转"
OPERATOR_ID = "成份广度扩散_v6"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 13
SLOW_WINDOW = 18
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.48 * VARIANT_WEIGHT * _zscore(up_ratio, 12) + 0.26 * VARIANT_WEIGHT * _zscore(new_high_ratio, 20) - _zscore(dispersion, 20).abs()
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 成份广度扩散_v6 | style: 反转 | seed: 20289415
