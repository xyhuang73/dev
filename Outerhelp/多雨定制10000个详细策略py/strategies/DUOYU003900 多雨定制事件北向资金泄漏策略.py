...
STRATEGY_ID = "DUOYU003900"
STRATEGY_NAME = "多雨定制事件北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003900"
CATEGORY = "北向资金泄漏"
STYLE = "事件"
OPERATOR_ID = "北向资金泄漏_v4"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 6
SLOW_WINDOW = 15
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    northbound = _field(panel, "northbound_flow") if "northbound_flow" in panel.columns else _diff(_field(panel, "fund_share"), 6)
    raw = 0.32 * VARIANT_WEIGHT * _zscore(northbound, 6) + _ret(close, 17)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 北向资金泄漏_v4 | style: 事件 | seed: 20261513
