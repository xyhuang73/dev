...
STRATEGY_ID = "DUOYU000017"
STRATEGY_NAME = "多雨定制动量风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000017"
CATEGORY = "风险调整动量"
STYLE = "动量"
OPERATOR_ID = "风险调整动量_v1"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    raw = _ret(close, 22) - 0.31 * VARIANT_WEIGHT * _downside_vol(close, 22) + 0.16 * VARIANT_WEIGHT * _amount_z(panel, 7)
    raw = raw * _sigmoid_gate(_ret(close, 7), 0, 4)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 风险调整动量_v1 | style: 动量 | seed: 20242210
