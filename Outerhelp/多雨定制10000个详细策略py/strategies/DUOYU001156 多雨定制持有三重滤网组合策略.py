...
STRATEGY_ID = "DUOYU001156"
STRATEGY_NAME = "多雨定制持有三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001156"
CATEGORY = "三重滤网组合"
STYLE = "持有"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
RISK_NOTE_CN = "多信号确认可能滞后于快速行情。 持有风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 5 个交易日。", "先计算 三重滤网组合 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU001156 - 多雨定制持有三重滤网组合策略
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 三重滤网组合_v4 | style: 持有 | seed: 20247113
