...
STRATEGY_ID = "DUOYU009322"
STRATEGY_NAME = "多雨定制进攻商品联动对冲策略"
SOURCE_FACTOR_ID = "DUOYU009322"
CATEGORY = "商品联动对冲"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 商品联动对冲_v3 | style: 进攻 | seed: 20294712
