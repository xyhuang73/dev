...
STRATEGY_ID = "DUOYU006294"
STRATEGY_NAME = "多雨定制集中相关性破裂检测策略"
SOURCE_FACTOR_ID = "DUOYU006294"
CATEGORY = "相关性破裂检测"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
RISK_NOTE_CN = "相关性估计在短窗口内不稳定。 集中风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 相关性破裂检测_v3 | style: 集中 | seed: 20274912
