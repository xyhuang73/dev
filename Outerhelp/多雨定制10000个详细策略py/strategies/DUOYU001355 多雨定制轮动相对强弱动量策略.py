...
STRATEGY_ID = "DUOYU001355"
STRATEGY_NAME = "多雨定制轮动相对强弱动量策略"
SOURCE_FACTOR_ID = "DUOYU001355"
CATEGORY = "相对强弱动量"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 相对强弱动量_v4 | style: 轮动 | seed: 20248013
