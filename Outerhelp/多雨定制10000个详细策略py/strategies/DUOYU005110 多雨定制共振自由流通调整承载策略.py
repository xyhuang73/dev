...
STRATEGY_ID = "DUOYU005110"
STRATEGY_NAME = "多雨定制共振自由流通调整承载策略"
SOURCE_FACTOR_ID = "DUOYU005110"
CATEGORY = "自由流通调整承载"
STYLE = "共振"
OPERATOR_ID = "自由流通调整承载_v2"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 15
FAST_WINDOW = 7
SLOW_WINDOW = 26
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    free_float_adj = _zscore(turnover, 26) / (1.0 + _zscore(spread, 26).abs())
    raw = 0.48 * VARIANT_WEIGHT * _zscore(free_float_adj, 7) + 0.18 * VARIANT_WEIGHT * _amount_z(panel, 7)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 自由流通调整承载_v2 | style: 共振 | seed: 20267511
