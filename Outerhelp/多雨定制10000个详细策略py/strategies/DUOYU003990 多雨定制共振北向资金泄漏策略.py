...
STRATEGY_ID = "DUOYU003990"
STRATEGY_NAME = "多雨定制共振北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003990"
CATEGORY = "北向资金泄漏"
STYLE = "共振"
OPERATOR_ID = "北向资金泄漏_v9"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 12
FAST_WINDOW = 13
SLOW_WINDOW = 21
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "北向资金并非总是聪明钱，存在跟随性交易。 共振风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    northbound = _field(panel, "northbound_flow") if "northbound_flow" in panel.columns else _diff(_field(panel, "fund_share"), 13)
    raw = 0.46 * VARIANT_WEIGHT * _zscore(northbound, 13) + _ret(close, 21)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 北向资金泄漏_v9 | style: 共振 | seed: 20260518
