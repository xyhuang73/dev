...
STRATEGY_ID = "DUOYU004789"
STRATEGY_NAME = "多雨定制反转流动性冲击吸收策略"
SOURCE_FACTOR_ID = "DUOYU004789"
CATEGORY = "流动性冲击吸收"
STYLE = "反转"
OPERATOR_ID = "流动性冲击吸收_v4"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    shock = _zscore(turnover, 6) - _zscore(turnover, 21)
    absorption = _ret(close, 6) / (_zscore(turnover, 6).abs() + EPS)
    raw = 0.18 * VARIANT_WEIGHT * shock + 0.35 * VARIANT_WEIGHT * _zscore(absorption, 6)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 流动性冲击吸收_v4 | style: 反转 | seed: 20265413
