...
STRATEGY_ID = "DUOYU008097"
STRATEGY_NAME = "多雨定制动量股息除权效应策略"
SOURCE_FACTOR_ID = "DUOYU008097"
CATEGORY = "股息除权效应"
STYLE = "动量"
OPERATOR_ID = "股息除权效应_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    div_score = _zscore(dividend_yield, 14) * _event_proximity(div_date_dist, 14)
    raw = 0.26 * VARIANT_WEIGHT * div_score + _ret(close, 7)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 股息除权效应_v2 | style: 动量 | seed: 20287211
