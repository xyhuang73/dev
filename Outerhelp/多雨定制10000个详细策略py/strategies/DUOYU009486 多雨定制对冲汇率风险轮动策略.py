...
STRATEGY_ID = "DUOYU009486"
STRATEGY_NAME = "多雨定制对冲汇率风险轮动策略"
SOURCE_FACTOR_ID = "DUOYU009486"
CATEGORY = "汇率风险轮动"
STYLE = "对冲"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。"
RISK_NOTE_CN = "汇率波动受宏观政策影响，可预测性低。 对冲风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 汇率风险轮动_v5 | style: 对冲 | seed: 20296114
