...
STRATEGY_ID = "DUOYU006535"
STRATEGY_NAME = "多雨定制轮动波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006535"
CATEGORY = "波动率的波动率"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v7 | style: 轮动 | seed: 20276016
