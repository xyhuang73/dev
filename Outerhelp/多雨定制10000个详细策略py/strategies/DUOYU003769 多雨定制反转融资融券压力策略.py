...
STRATEGY_ID = "DUOYU003769"
STRATEGY_NAME = "多雨定制反转融资融券压力策略"
SOURCE_FACTOR_ID = "DUOYU003769"
CATEGORY = "融资融券压力"
STYLE = "反转"
OPERATOR_ID = "融资融券压力_v7"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 15
SLOW_WINDOW = 20
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高融资比例+价格上涨=杠杆驱动行情（需警惕），高融券+价格下跌=空头挤压机会。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高融资比例+价格上涨=杠杆驱动行情（需警惕），高融券+价格下跌=空头挤压机会。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    short_ratio = _field(panel, "short_interest_ratio") if "short_interest_ratio" in panel.columns else _zscore(_field(panel, "turnover"), 14)
    raw = -0.21 * VARIANT_WEIGHT * _zscore(margin_ratio, 23) + 0.37 * VARIANT_WEIGHT * _ret(close, 14) - _zscore(short_ratio, 14).abs()
    raw = raw * _max_dd_guard(drawdown, -0.15)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 融资融券压力_v7 | style: 反转 | seed: 20259416
