...
STRATEGY_ID = "DUOYU005372"
STRATEGY_NAME = "多雨定制择时委托簿斜率策略"
SOURCE_FACTOR_ID = "DUOYU005372"
CATEGORY = "委托簿斜率"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 委托簿斜率_v6 | style: 择时 | seed: 20268715
