...
STRATEGY_ID = "DUOYU001250"
STRATEGY_NAME = "多雨定制共振三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001250"
CATEGORY = "三重滤网组合"
STYLE = "共振"
OPERATOR_ID = "三重滤网组合_v9"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 11
FAST_WINDOW = 14
SLOW_WINDOW = 23
VARIANT_WEIGHT = 0.91
REQUIRED_FIELDS = ["close", "amount", "net_flow"]

ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "多信号确认可能滞后于快速行情。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 11 个交易日。", "先计算 三重滤网组合 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU001250 - 多雨定制共振三重滤网组合策略

策略主题：三重滤网组合 / 共振
适用范围：宽基/行业/主题
主要周期：11 个交易日
必需字段：close, amount, net_flow

经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    amount_z = _amount_z(panel, 14)
    trend_score = _ret(close, 23)
    vol_score = -_downside_vol(close, 22)
    flow_confirm = _zscore(_field(panel, "net_flow"), 14) if "net_flow" in panel.columns else amount_z
    raw = 0.52 * VARIANT_WEIGHT * trend_score + 0.21 * VARIANT_WEIGHT * vol_score + 0.28 * VARIANT_WEIGHT * flow_confirm
    raw = raw * _max_dd_guard(drawdown, -0.10)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 三重滤网组合_v9 | style: 共振 | seed: 20246518
