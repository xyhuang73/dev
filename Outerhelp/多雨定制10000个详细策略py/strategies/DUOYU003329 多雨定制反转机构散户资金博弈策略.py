...
STRATEGY_ID = "DUOYU003329"
STRATEGY_NAME = "多雨定制反转机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003329"
CATEGORY = "机构散户资金博弈"
STYLE = "反转"
OPERATOR_ID = "机构散户资金博弈_v3"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 17
FAST_WINDOW = 12
SLOW_WINDOW = 20
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    holder_change = _field(panel, "holder_count_change") if "holder_count_change" in panel.columns else _diff(_field(panel, "fund_share"), 22)
    smart_money = _inst_flow_divergence(_ret(close, 11), inst_change, 11)
    retail_pressure = -_zscore(holder_change, 22)
    raw = 0.4 * VARIANT_WEIGHT * smart_money + 0.29 * VARIANT_WEIGHT * retail_pressure + _ret(close, 22)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 机构散户资金博弈_v3 | style: 反转 | seed: 20257412
