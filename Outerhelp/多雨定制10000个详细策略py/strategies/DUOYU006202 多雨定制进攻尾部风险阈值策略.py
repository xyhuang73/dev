...
STRATEGY_ID = "DUOYU006202"
STRATEGY_NAME = "多雨定制进攻尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006202"
CATEGORY = "尾部风险阈值"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
RISK_NOTE_CN = "尾部风险模型依赖历史分布假设。 进攻风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 尾部风险阈值_v7 | style: 进攻 | seed: 20272716
