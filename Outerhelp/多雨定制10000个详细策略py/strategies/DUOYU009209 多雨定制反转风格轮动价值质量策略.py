...
STRATEGY_ID = "DUOYU009209"
STRATEGY_NAME = "多雨定制反转风格轮动价值质量策略"
SOURCE_FACTOR_ID = "DUOYU009209"
CATEGORY = "风格轮动价值质量"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "风格因子在不同市场周期表现差异大。 反转风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风格轮动价值质量_v3 | style: 反转 | seed: 20294412
