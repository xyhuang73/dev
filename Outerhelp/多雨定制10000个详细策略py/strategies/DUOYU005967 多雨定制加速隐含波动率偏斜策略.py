...
STRATEGY_ID = "DUOYU005967"
STRATEGY_NAME = "多雨定制加速隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU005967"
CATEGORY = "隐含波动率偏斜"
STYLE = "加速"
OPERATOR_ID = "隐含波动率偏斜_v3"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 7
SLOW_WINDOW = 11
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    iv = _field(panel, "option_implied_vol") if "option_implied_vol" in panel.columns else _downside_vol(close, 16)
    iv_change = _diff(iv, 10)
    raw = -0.3 * VARIANT_WEIGHT * _zscore(iv_change, 10) + _ret(close, 16)
    raw = raw * _max_dd_guard(drawdown, -0.12)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 隐含波动率偏斜_v3 | style: 加速 | seed: 20272212
