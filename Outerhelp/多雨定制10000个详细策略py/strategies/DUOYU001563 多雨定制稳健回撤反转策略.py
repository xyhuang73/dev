...
STRATEGY_ID = "DUOYU001563"
STRATEGY_NAME = "多雨定制稳健回撤反转策略"
SOURCE_FACTOR_ID = "DUOYU001563"
CATEGORY = "回撤反转"
STYLE = "稳健"
OPERATOR_ID = "回撤反转_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤反转_v5 | style: 稳健 | seed: 20247814
