...
STRATEGY_ID = "DUOYU008450"
STRATEGY_NAME = "多雨定制共振行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008450"
CATEGORY = "行业景气扩散"
STYLE = "共振"
OPERATOR_ID = "行业景气扩散_v6"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 6
FAST_WINDOW = 11
SLOW_WINDOW = 14
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.43 * VARIANT_WEIGHT * _zscore(prosperity, 14) + 0.35 * VARIANT_WEIGHT * _zscore(revision, 14) + _ret(industry_momentum, 11)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 行业景气扩散_v6 | style: 共振 | seed: 20288515
