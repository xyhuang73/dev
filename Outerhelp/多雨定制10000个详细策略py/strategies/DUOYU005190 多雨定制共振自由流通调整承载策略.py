...
STRATEGY_ID = "DUOYU005190"
STRATEGY_NAME = "多雨定制共振自由流通调整承载策略"
SOURCE_FACTOR_ID = "DUOYU005190"
CATEGORY = "自由流通调整承载"
STYLE = "共振"
OPERATOR_ID = "自由流通调整承载_v6"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 6
FAST_WINDOW = 12
SLOW_WINDOW = 21
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    free_float_adj = _zscore(turnover, 21) / (1.0 + _zscore(spread, 21).abs())
    raw = 0.33 * VARIANT_WEIGHT * _zscore(free_float_adj, 12) + 0.22 * VARIANT_WEIGHT * _amount_z(panel, 12)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 自由流通调整承载_v6 | style: 共振 | seed: 20267515
