...
STRATEGY_ID = "DUOYU007457"
STRATEGY_NAME = "多雨定制动量政策事件评分策略"
SOURCE_FACTOR_ID = "DUOYU007457"
CATEGORY = "政策事件评分"
STYLE = "动量"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "政策事件评分的 NLP 模型存在误判风险。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 16 个交易日。", "先计算 政策事件评分 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU007457 - 多雨定制动量政策事件评分策略
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 政策事件评分_v5 | style: 动量 | seed: 20282214
