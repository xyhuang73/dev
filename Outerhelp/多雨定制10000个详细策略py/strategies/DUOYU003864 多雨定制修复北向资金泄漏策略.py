...
STRATEGY_ID = "DUOYU003864"
STRATEGY_NAME = "多雨定制修复北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003864"
CATEGORY = "北向资金泄漏"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
RISK_NOTE_CN = "北向资金并非总是聪明钱，存在跟随性交易。 修复风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 北向资金泄漏_v3 | style: 修复 | seed: 20259912
