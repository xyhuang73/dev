...
STRATEGY_ID = "DUOYU007288"
STRATEGY_NAME = "多雨定制减速波动率压缩预警策略"
SOURCE_FACTOR_ID = "DUOYU007288"
CATEGORY = "波动率压缩预警"
STYLE = "减速"
...
ECONOMIC_LOGIC_CN = "该策略以约 4 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。"
...
经济逻辑：
该策略以约 4 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率压缩预警_v5 | style: 减速 | seed: 20280314
