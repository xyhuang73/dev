...
STRATEGY_ID = "DUOYU007709"
STRATEGY_NAME = "多雨定制反转退市风险筛选策略"
SOURCE_FACTOR_ID = "DUOYU007709"
CATEGORY = "退市风险筛选"
STYLE = "反转"
OPERATOR_ID = "退市风险筛选_v4"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 17
FAST_WINDOW = 11
SLOW_WINDOW = 26
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：退市风险低+审计质量高+价格趋势好=安全边际充足，离退市越远越安全。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：退市风险低+审计质量高+价格趋势好=安全边际充足，离退市越远越安全。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.4 * VARIANT_WEIGHT * _zscore(risk_dist, 10) + 0.28 * VARIANT_WEIGHT * _zscore(audit, 29) + _ret(close, 29)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 退市风险筛选_v4 | style: 反转 | seed: 20283413
