...
STRATEGY_ID = "DUOYU009327"
STRATEGY_NAME = "多雨定制加速商品联动对冲策略"
SOURCE_FACTOR_ID = "DUOYU009327"
CATEGORY = "商品联动对冲"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 商品联动对冲_v3 | style: 加速 | seed: 20295212
