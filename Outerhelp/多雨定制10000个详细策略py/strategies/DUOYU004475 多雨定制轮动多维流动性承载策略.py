...
STRATEGY_ID = "DUOYU004475"
STRATEGY_NAME = "多雨定制轮动多维流动性承载策略"
SOURCE_FACTOR_ID = "DUOYU004475"
CATEGORY = "多维流动性承载"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 多维流动性承载_v6 | style: 轮动 | seed: 20264015
