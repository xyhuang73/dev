...
STRATEGY_ID = "DUOYU002365"
STRATEGY_NAME = "多雨定制确认日内积累分布策略"
SOURCE_FACTOR_ID = "DUOYU002365"
CATEGORY = "日内积累分布"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 日内积累分布_v5 | style: 确认 | seed: 20252014
