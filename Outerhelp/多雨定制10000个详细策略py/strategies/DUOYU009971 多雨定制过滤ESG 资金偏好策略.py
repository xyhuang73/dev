...
STRATEGY_ID = "DUOYU009971"
STRATEGY_NAME = "多雨定制过滤ESG 资金偏好策略"
SOURCE_FACTOR_ID = "DUOYU009971"
CATEGORY = "ESG 资金偏好"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高环保合规+资金流入=ESG 主题资金偏好，捕捉可持续投资趋势。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
RISK_NOTE_CN = "ESG 评分标准不统一，存在评级分歧。 过滤风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高环保合规+资金流入=ESG 主题资金偏好，捕捉可持续投资趋势。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: esg_资金偏好_v5 | style: 过滤 | seed: 20300614
