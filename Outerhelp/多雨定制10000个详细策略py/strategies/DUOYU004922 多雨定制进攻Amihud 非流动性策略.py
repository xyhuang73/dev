...
STRATEGY_ID = "DUOYU004922"
STRATEGY_NAME = "多雨定制进攻Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU004922"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
"""
DUOYU004922 - 多雨定制进攻Amihud 非流动性策略
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: amihud_非流动性_v2 | style: 进攻 | seed: 20265711
