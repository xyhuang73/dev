...
STRATEGY_ID = "DUOYU008277"
STRATEGY_NAME = "多雨定制动量业绩超预期动量策略"
SOURCE_FACTOR_ID = "DUOYU008277"
CATEGORY = "业绩超预期动量"
STYLE = "动量"
OPERATOR_ID = "业绩超预期动量_v4"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 16
FAST_WINDOW = 8
SLOW_WINDOW = 13
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    surprise = _zscore(earnings_revision, 7) - _zscore(earnings_revision, 16)
    raw = 0.39 * VARIANT_WEIGHT * surprise + 0.31 * VARIANT_WEIGHT * _ret(industry_momentum, 7) + _ret(close, 7)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 业绩超预期动量_v4 | style: 动量 | seed: 20288213
