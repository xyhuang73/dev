...
STRATEGY_ID = "DUOYU007354"
STRATEGY_NAME = "多雨定制集中波动率压缩预警策略"
SOURCE_FACTOR_ID = "DUOYU007354"
CATEGORY = "波动率压缩预警"
STYLE = "集中"
OPERATOR_ID = "波动率压缩预警_v8"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    breakout_potential = _breakout(panel, 7)
    raw = compression * 0.29 * VARIANT_WEIGHT + breakout_potential * 0.46 * VARIANT_WEIGHT
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 波动率压缩预警_v8 | style: 集中 | seed: 20280917
