...
STRATEGY_ID = "DUOYU008407"
STRATEGY_NAME = "多雨定制加速行业景气扩散策略"
SOURCE_FACTOR_ID = "DUOYU008407"
CATEGORY = "行业景气扩散"
STYLE = "加速"
OPERATOR_ID = "行业景气扩散_v4"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 11
FAST_WINDOW = 11
SLOW_WINDOW = 15
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：行业景气度高+盈利上调+行业指数动量=多维度行业基本面确认。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.33 * VARIANT_WEIGHT * _zscore(prosperity, 21) + 0.43 * VARIANT_WEIGHT * _zscore(revision, 21) + _ret(industry_momentum, 8)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 行业景气扩散_v4 | style: 加速 | seed: 20288213
