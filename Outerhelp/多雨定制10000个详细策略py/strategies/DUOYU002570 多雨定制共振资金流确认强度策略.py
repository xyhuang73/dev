...
STRATEGY_ID = "DUOYU002570"
STRATEGY_NAME = "多雨定制共振资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002570"
CATEGORY = "资金流确认强度"
STYLE = "共振"
OPERATOR_ID = "资金流确认强度_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.55 * VARIANT_WEIGHT * _zscore(net_flow, 9) + _ret(close, 28) + 0.25 * VARIANT_WEIGHT * _diff(fund_share, 28)
    raw = raw * _sigmoid_gate(_zscore(net_flow, 9), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 资金流确认强度_v5 | style: 共振 | seed: 20253514
