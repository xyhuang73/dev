...
STRATEGY_ID = "DUOYU005447"
STRATEGY_NAME = "多雨定制加速最小报价单位约束策略"
SOURCE_FACTOR_ID = "DUOYU005447"
CATEGORY = "最小报价单位约束"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 20 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
RISK_NOTE_CN = "标的拆分/合并事件影响 tick 约束计算。 加速风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 20 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 最小报价单位约束_v1 | style: 加速 | seed: 20269210
