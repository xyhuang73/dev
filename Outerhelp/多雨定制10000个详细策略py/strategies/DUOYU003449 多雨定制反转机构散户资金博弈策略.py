...
STRATEGY_ID = "DUOYU003449"
STRATEGY_NAME = "多雨定制反转机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003449"
CATEGORY = "机构散户资金博弈"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 机构散户资金博弈_v9 | style: 反转 | seed: 20257418
