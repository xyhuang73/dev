...
STRATEGY_ID = "DUOYU006503"
STRATEGY_NAME = "多雨定制稳健波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006503"
CATEGORY = "波动率的波动率"
STYLE = "稳健"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v6 | style: 稳健 | seed: 20274815
