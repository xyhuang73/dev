...
STRATEGY_ID = "DUOYU006849"
STRATEGY_NAME = "多雨定制反转保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006849"
CATEGORY = "保证金追缴级联代理"
STYLE = "反转"
OPERATOR_ID = "保证金追缴级联代理_v7"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 15
SLOW_WINDOW = 23
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    margin_ratio = _field(panel, "margin_ratio") if "margin_ratio" in panel.columns else _zscore(_amount_z(panel, 14), 26)
    downside = _downside_vol(close, 26)
    margin_stress = _zscore(margin_ratio, 14) * downside
    raw = -0.37 * VARIANT_WEIGHT * margin_stress + _ret(close, 26)
    raw = raw * _max_dd_guard(drawdown, -0.15)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 保证金追缴级联代理_v7 | style: 反转 | seed: 20277416
