...
STRATEGY_ID = "DUOYU003500"
STRATEGY_NAME = "多雨定制事件大宗交易足迹策略"
SOURCE_FACTOR_ID = "DUOYU003500"
CATEGORY = "大宗交易足迹"
STYLE = "事件"
OPERATOR_ID = "大宗交易足迹_v2"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 6
SLOW_WINDOW = 12
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    block_premium = _field(panel, "block_trade_premium") if "block_trade_premium" in panel.columns else _zscore(_amount_z(panel, 6) - _amount_z(panel, 14), 6)
    raw = 0.36 * VARIANT_WEIGHT * _zscore(block_premium, 6) + _ret(close, 14)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 大宗交易足迹_v2 | style: 事件 | seed: 20259511
