...
STRATEGY_ID = "DUOYU003397"
STRATEGY_NAME = "多雨定制动量机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003397"
CATEGORY = "机构散户资金博弈"
STYLE = "动量"
OPERATOR_ID = "机构散户资金博弈_v6"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 16
FAST_WINDOW = 10
SLOW_WINDOW = 14
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    holder_change = _field(panel, "holder_count_change") if "holder_count_change" in panel.columns else _diff(_field(panel, "fund_share"), 17)
    smart_money = _inst_flow_divergence(_ret(close, 9), inst_change, 9)
    retail_pressure = -_zscore(holder_change, 17)
    raw = 0.46 * VARIANT_WEIGHT * smart_money + 0.2 * VARIANT_WEIGHT * retail_pressure + _ret(close, 17)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 机构散户资金博弈_v6 | style: 动量 | seed: 20258215
