...
STRATEGY_ID = "DUOYU006040"
STRATEGY_NAME = "多雨定制事件隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU006040"
CATEGORY = "隐含波动率偏斜"
STYLE = "事件"
OPERATOR_ID = "隐含波动率偏斜_v6"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 17
FAST_WINDOW = 8
SLOW_WINDOW = 18
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    iv = _field(panel, "option_implied_vol") if "option_implied_vol" in panel.columns else _downside_vol(close, 20)
    iv_change = _diff(iv, 8)
    raw = -0.42 * VARIANT_WEIGHT * _zscore(iv_change, 8) + _ret(close, 20)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 隐含波动率偏斜_v6 | style: 事件 | seed: 20273515
