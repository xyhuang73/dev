...
STRATEGY_ID = "DUOYU001136"
STRATEGY_NAME = "多雨定制持有三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001136"
CATEGORY = "三重滤网组合"
STYLE = "持有"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 三重滤网组合_v3 | style: 持有 | seed: 20247112
