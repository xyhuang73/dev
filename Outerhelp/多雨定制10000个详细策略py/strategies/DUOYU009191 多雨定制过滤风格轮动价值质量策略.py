...
STRATEGY_ID = "DUOYU009191"
STRATEGY_NAME = "多雨定制过滤风格轮动价值质量策略"
SOURCE_FACTOR_ID = "DUOYU009191"
CATEGORY = "风格轮动价值质量"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风格轮动价值质量_v2 | style: 过滤 | seed: 20294611
