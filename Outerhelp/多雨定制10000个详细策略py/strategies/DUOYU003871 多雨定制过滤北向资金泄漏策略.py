...
STRATEGY_ID = "DUOYU003871"
STRATEGY_NAME = "多雨定制过滤北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003871"
CATEGORY = "北向资金泄漏"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 北向资金泄漏_v3 | style: 过滤 | seed: 20260612
