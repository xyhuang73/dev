...
STRATEGY_ID = "DUOYU000940"
STRATEGY_NAME = "多雨定制事件伽马突破策略"
SOURCE_FACTOR_ID = "DUOYU000940"
CATEGORY = "伽马突破"
STYLE = "事件"
OPERATOR_ID = "伽马突破_v3"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    vol_compression = 1.0 - _cs_rank(_zscore(true_range, 22).abs()).abs()
    breakout = _breakout(panel, 5)
    raw = breakout * vol_compression * 0.94 * VARIANT_WEIGHT + 0.14 * VARIANT_WEIGHT * _amount_z(panel, 5)
    raw = raw * _sigmoid_gate(_amount_z(panel, 5), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 伽马突破_v3 | style: 事件 | seed: 20246512
