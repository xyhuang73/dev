...
STRATEGY_ID = "DUOYU003460"
STRATEGY_NAME = "多雨定制事件机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003460"
CATEGORY = "机构散户资金博弈"
STYLE = "事件"
OPERATOR_ID = "机构散户资金博弈_v9"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 10
FAST_WINDOW = 11
SLOW_WINDOW = 17
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    holder_change = _field(panel, "holder_count_change") if "holder_count_change" in panel.columns else _diff(_field(panel, "fund_share"), 19)
    smart_money = _inst_flow_divergence(_ret(close, 11), inst_change, 11)
    retail_pressure = -_zscore(holder_change, 19)
    raw = 0.51 * VARIANT_WEIGHT * smart_money + 0.27 * VARIANT_WEIGHT * retail_pressure + _ret(close, 19)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 机构散户资金博弈_v9 | style: 事件 | seed: 20258518
