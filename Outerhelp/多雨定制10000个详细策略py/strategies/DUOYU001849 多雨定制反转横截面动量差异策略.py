...
STRATEGY_ID = "DUOYU001849"
STRATEGY_NAME = "多雨定制反转横截面动量差异策略"
SOURCE_FACTOR_ID = "DUOYU001849"
CATEGORY = "横截面动量差异"
STYLE = "反转"
OPERATOR_ID = "横截面动量差异_v9"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.42 * VARIANT_WEIGHT * _zscore(momentum_diff, 5) + _ret(close, 23)
    raw = raw * _sigmoid_gate(_ret(close, 5), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 5)
    # instance: 横截面动量差异_v9 | style: 反转 | seed: 20249418
