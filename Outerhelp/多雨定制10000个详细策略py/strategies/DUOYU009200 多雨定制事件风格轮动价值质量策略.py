...
STRATEGY_ID = "DUOYU009200"
STRATEGY_NAME = "多雨定制事件风格轮动价值质量策略"
SOURCE_FACTOR_ID = "DUOYU009200"
CATEGORY = "风格轮动价值质量"
STYLE = "事件"
OPERATOR_ID = "风格轮动价值质量_v2"
MARKET_SCOPE = "风格/宽基"
HORIZON_DAYS = 10
FAST_WINDOW = 11
SLOW_WINDOW = 22
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.32 * VARIANT_WEIGHT * _zscore(valuation, 24) + 0.26 * VARIANT_WEIGHT * _zscore(quality, 24) + 0.26 * VARIANT_WEIGHT * _zscore(dividend, 24) + _ret(close, 11)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 风格轮动价值质量_v2 | style: 事件 | seed: 20295511
