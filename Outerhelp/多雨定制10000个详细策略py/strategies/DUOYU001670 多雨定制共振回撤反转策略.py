...
STRATEGY_ID = "DUOYU001670"
STRATEGY_NAME = "多雨定制共振回撤反转策略"
SOURCE_FACTOR_ID = "DUOYU001670"
CATEGORY = "回撤反转"
STYLE = "共振"
OPERATOR_ID = "回撤反转_v10"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    trend_gate = (_ret(close, 23) > 0).astype(float)
    raw = -_ret(close, 8) * trend_gate + 0.1 * VARIANT_WEIGHT * _amount_z(panel, 8)
    score = _risk_scale(_cs_rank(raw), panel, 5)
    # instance: 回撤反转_v10 | style: 共振 | seed: 20248519
