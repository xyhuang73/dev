...
STRATEGY_ID = "DUOYU005610"
STRATEGY_NAME = "多雨定制共振跟踪质量策略"
SOURCE_FACTOR_ID = "DUOYU005610"
CATEGORY = "跟踪质量"
STYLE = "共振"
OPERATOR_ID = "跟踪质量_v1"
MARKET_SCOPE = "宽基/行业"
...
SLOW_WINDOW = 27
VARIANT_WEIGHT = 0.91
REQUIRED_FIELDS = ["index_close", "tracking_error", "tracking_diff"]

ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "跟踪误差数据频率可能低于价格数据。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 16 个交易日。", "先计算 跟踪质量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU005610 - 多雨定制共振跟踪质量策略

策略主题：跟踪质量 / 共振
适用范围：宽基/行业
主要周期：16 个交易日
必需字段：index_close, tracking_error, tracking_diff

经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    tracking_error = _field(panel, "tracking_error")
    tracking_diff = _field(panel, "tracking_diff")
    raw = _ret(index_close, 27) - 0.46 * VARIANT_WEIGHT * _zscore(tracking_error, 27).abs() - 0.31 * VARIANT_WEIGHT * _zscore(tracking_diff, 27).abs()
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 跟踪质量_v1 | style: 共振 | seed: 20270510
