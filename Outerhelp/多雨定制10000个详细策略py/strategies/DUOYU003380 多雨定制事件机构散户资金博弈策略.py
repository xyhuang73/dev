...
STRATEGY_ID = "DUOYU003380"
STRATEGY_NAME = "多雨定制事件机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003380"
CATEGORY = "机构散户资金博弈"
STYLE = "事件"
OPERATOR_ID = "机构散户资金博弈_v5"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 14
FAST_WINDOW = 11
SLOW_WINDOW = 14
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    holder_change = _field(panel, "holder_count_change") if "holder_count_change" in panel.columns else _diff(_field(panel, "fund_share"), 16)
    smart_money = _inst_flow_divergence(_ret(close, 11), inst_change, 11)
    retail_pressure = -_zscore(holder_change, 16)
    raw = 0.37 * VARIANT_WEIGHT * smart_money + 0.23 * VARIANT_WEIGHT * retail_pressure + _ret(close, 16)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 机构散户资金博弈_v5 | style: 事件 | seed: 20258514
