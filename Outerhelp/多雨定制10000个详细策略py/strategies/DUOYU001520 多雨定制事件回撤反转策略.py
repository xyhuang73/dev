...
STRATEGY_ID = "DUOYU001520"
STRATEGY_NAME = "多雨定制事件回撤反转策略"
SOURCE_FACTOR_ID = "DUOYU001520"
CATEGORY = "回撤反转"
STYLE = "事件"
OPERATOR_ID = "回撤反转_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    trend_gate = (_ret(close, 20) > 0).astype(float)
    raw = -_ret(close, 9) * trend_gate + 0.15 * VARIANT_WEIGHT * _amount_z(panel, 9)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 回撤反转_v2 | style: 事件 | seed: 20249511
