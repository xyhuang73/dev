...
STRATEGY_ID = "DUOYU004594"
STRATEGY_NAME = "多雨定制集中买卖盘深度失衡策略"
SOURCE_FACTOR_ID = "DUOYU004594"
CATEGORY = "买卖盘深度失衡"
STYLE = "集中"
OPERATOR_ID = "买卖盘深度失衡_v3"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 10
FAST_WINDOW = 8
SLOW_WINDOW = 20
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    depth_imbalance = (_zscore(turnover, 7) - _zscore(spread, 7).abs()) / (1.0 + _zscore(spread, 25).abs())
    raw = 0.4 * VARIANT_WEIGHT * _zscore(depth_imbalance, 7) + _ret(close, 25)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 买卖盘深度失衡_v3 | style: 集中 | seed: 20264912
