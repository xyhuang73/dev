...
STRATEGY_ID = "DUOYU006227"
STRATEGY_NAME = "多雨定制加速尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006227"
CATEGORY = "尾部风险阈值"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 19 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 19 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 尾部风险阈值_v8 | style: 加速 | seed: 20273217
