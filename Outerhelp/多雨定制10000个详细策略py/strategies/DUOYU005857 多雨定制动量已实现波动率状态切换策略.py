...
STRATEGY_ID = "DUOYU005857"
STRATEGY_NAME = "多雨定制动量已实现波动率状态切换策略"
SOURCE_FACTOR_ID = "DUOYU005857"
CATEGORY = "已实现波动率状态切换"
STYLE = "动量"
OPERATOR_ID = "已实现波动率状态切换_v5"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 18
FAST_WINDOW = 11
SLOW_WINDOW = 16
VARIANT_WEIGHT = 1.02
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "状态判断滞后可能导致切换在错误时机。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 18 个交易日。", "先计算 已实现波动率状态切换 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU005857 - 多雨定制动量已实现波动率状态切换策略
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _amount_z(panel: pd.DataFrame, window: int) -> pd.Series:
    amount = _field(panel, "amount")
    return _zscore(np.log1p(amount.where(amount >= 0)), window)
...
def _vol_regime(close: pd.Series, fast_window: int, slow_window: int) -> pd.Series:
    """检测波动率状态：压缩(-1)、平稳(0)、扩张(+1)。"""
    r = close.pct_change()
    fast_vol = _by_etf(r.abs(), lambda x: x.rolling(max(5, int(fast_window)), min_periods=3).std())
    slow_vol = _by_etf(r.abs(), lambda x: x.rolling(max(10, int(slow_window)), min_periods=5).std())
    ratio = fast_vol / (slow_vol.abs() + EPS)
    return np.tanh(ratio - 1.0)
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    vol_regime = _vol_regime(close, 10, 20)
    trend_term = _ret(close, 20) * _sigmoid_gate(vol_regime, 0, 3)
    rev_term = -_ret(close, 10) * (1.0 - _sigmoid_gate(vol_regime, 0, 3))
    raw = np.where(vol_regime > 0, trend_term + 0.14 * VARIANT_WEIGHT * _amount_z(panel, 10),
                   rev_term + 0.14 * VARIANT_WEIGHT * _amount_z(panel, 10))
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 已实现波动率状态切换_v5 | style: 动量 | seed: 20272214
