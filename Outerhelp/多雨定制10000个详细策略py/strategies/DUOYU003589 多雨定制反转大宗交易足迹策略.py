...
STRATEGY_ID = "DUOYU003589"
STRATEGY_NAME = "多雨定制反转大宗交易足迹策略"
SOURCE_FACTOR_ID = "DUOYU003589"
CATEGORY = "大宗交易足迹"
STYLE = "反转"
OPERATOR_ID = "大宗交易足迹_v7"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 13
FAST_WINDOW = 13
SLOW_WINDOW = 18
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    block_premium = _field(panel, "block_trade_premium") if "block_trade_premium" in panel.columns else _zscore(_amount_z(panel, 12) - _amount_z(panel, 20), 12)
    raw = 0.3 * VARIANT_WEIGHT * _zscore(block_premium, 12) + _ret(close, 20)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 大宗交易足迹_v7 | style: 反转 | seed: 20258416
