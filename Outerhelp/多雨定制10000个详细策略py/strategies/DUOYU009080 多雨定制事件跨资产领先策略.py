...
STRATEGY_ID = "DUOYU009080"
STRATEGY_NAME = "多雨定制事件跨资产领先策略"
SOURCE_FACTOR_ID = "DUOYU009080"
CATEGORY = "跨资产领先"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 跨资产领先_v2 | style: 事件 | seed: 20294511
