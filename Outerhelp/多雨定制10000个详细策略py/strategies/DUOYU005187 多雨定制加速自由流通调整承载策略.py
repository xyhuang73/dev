...
STRATEGY_ID = "DUOYU005187"
STRATEGY_NAME = "多雨定制加速自由流通调整承载策略"
SOURCE_FACTOR_ID = "DUOYU005187"
CATEGORY = "自由流通调整承载"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 自由流通调整承载_v6 | style: 加速 | seed: 20267215
