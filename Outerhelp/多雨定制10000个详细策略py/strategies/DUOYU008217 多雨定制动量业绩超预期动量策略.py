...
STRATEGY_ID = "DUOYU008217"
STRATEGY_NAME = "多雨定制动量业绩超预期动量策略"
SOURCE_FACTOR_ID = "DUOYU008217"
CATEGORY = "业绩超预期动量"
STYLE = "动量"
OPERATOR_ID = "业绩超预期动量_v1"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 15
FAST_WINDOW = 8
SLOW_WINDOW = 21
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "分析师预测存在系统性乐观偏差。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 15 个交易日。", "先计算 业绩超预期动量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008217 - 多雨定制动量业绩超预期动量策略
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    surprise = _zscore(earnings_revision, 7) - _zscore(earnings_revision, 25)
    raw = 0.37 * VARIANT_WEIGHT * surprise + 0.21 * VARIANT_WEIGHT * _ret(industry_momentum, 7) + _ret(close, 7)
    raw = raw * _sigmoid_gate(surprise, 0, 4)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 业绩超预期动量_v1 | style: 动量 | seed: 20288210
