...
STRATEGY_ID = "DUOYU004740"
STRATEGY_NAME = "多雨定制事件流动性冲击吸收策略"
SOURCE_FACTOR_ID = "DUOYU004740"
CATEGORY = "流动性冲击吸收"
STYLE = "事件"
OPERATOR_ID = "流动性冲击吸收_v1"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    shock = _zscore(turnover, 9) - _zscore(turnover, 24)
    absorption = _ret(close, 9) / (_zscore(turnover, 9).abs() + EPS)
    raw = 0.32 * VARIANT_WEIGHT * shock + 0.27 * VARIANT_WEIGHT * _zscore(absorption, 9)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 流动性冲击吸收_v1 | style: 事件 | seed: 20266510
