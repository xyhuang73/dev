...
STRATEGY_ID = "DUOYU009137"
STRATEGY_NAME = "多雨定制动量跨资产领先策略"
SOURCE_FACTOR_ID = "DUOYU009137"
CATEGORY = "跨资产领先"
STYLE = "动量"
OPERATOR_ID = "跨资产领先_v5"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 18
FAST_WINDOW = 11
SLOW_WINDOW = 20
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["close", "lead_asset_return", "market_risk_appetite"]

ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "跨资产领先关系并非稳定不变。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 18 个交易日。", "先计算 跨资产领先 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009137 - 多雨定制动量跨资产领先策略

策略主题：跨资产领先 / 动量
适用范围：宽基/行业/主题
主要周期：18 个交易日
必需字段：close, lead_asset_return, market_risk_appetite

经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    lead = _field(panel, "lead_asset_return")
    appetite = _field(panel, "market_risk_appetite")
    raw = lead + 0.39 * VARIANT_WEIGHT * _ret(close, 10) + 0.26 * VARIANT_WEIGHT * _zscore(appetite, 24)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 跨资产领先_v5 | style: 动量 | seed: 20294214
