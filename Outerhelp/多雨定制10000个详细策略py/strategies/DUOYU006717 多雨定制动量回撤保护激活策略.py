...
STRATEGY_ID = "DUOYU006717"
STRATEGY_NAME = "多雨定制动量回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006717"
CATEGORY = "回撤保护激活"
STYLE = "动量"
OPERATOR_ID = "回撤保护激活_v8"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    if "drawdown" in panel.columns:
        ...
        rolling_peak = _rolling_max(close, 14)
        drawdown = close / (rolling_peak.abs() + EPS) - 1.0
    dd_guard = _max_dd_guard(drawdown, -0.16)
    raw = -_zscore(drawdown, 14) * dd_guard + 0.32 * VARIANT_WEIGHT * _zscore(net_flow, 6) + _repair_slope(close, 6)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 回撤保护激活_v8 | style: 动量 | seed: 20277217
