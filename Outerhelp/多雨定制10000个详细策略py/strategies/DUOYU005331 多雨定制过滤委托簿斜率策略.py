...
STRATEGY_ID = "DUOYU005331"
STRATEGY_NAME = "多雨定制过滤委托簿斜率策略"
SOURCE_FACTOR_ID = "DUOYU005331"
CATEGORY = "委托簿斜率"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 委托簿斜率_v4 | style: 过滤 | seed: 20268613
