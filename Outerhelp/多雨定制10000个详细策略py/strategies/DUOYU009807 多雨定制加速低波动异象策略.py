...
STRATEGY_ID = "DUOYU009807"
STRATEGY_NAME = "多雨定制加速低波动异象策略"
SOURCE_FACTOR_ID = "DUOYU009807"
CATEGORY = "低波动异象"
STYLE = "加速"
OPERATOR_ID = "低波动异象_v3"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    quality = _field(panel, "quality_score") if "quality_score" in panel.columns else _zscore(_field(panel, "dividend_yield"), 17)
    raw = -0.65 * VARIANT_WEIGHT * _downside_vol(close, 17) + 0.33 * VARIANT_WEIGHT * _zscore(quality, 17) + _ret(close, 17)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 低波动异象_v3 | style: 加速 | seed: 20299212
