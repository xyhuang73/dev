...
STRATEGY_ID = "DUOYU003948"
STRATEGY_NAME = "多雨定制减速北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003948"
CATEGORY = "北向资金泄漏"
STYLE = "减速"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 北向资金泄漏_v7 | style: 减速 | seed: 20260316
