...
STRATEGY_ID = "DUOYU006793"
STRATEGY_NAME = "多雨定制分散保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006793"
CATEGORY = "保证金追缴级联代理"
STYLE = "分散"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。"
RISK_NOTE_CN = "保证金数据频率低，代理变量精度有限。 分散风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 保证金追缴级联代理_v4 | style: 分散 | seed: 20277813
