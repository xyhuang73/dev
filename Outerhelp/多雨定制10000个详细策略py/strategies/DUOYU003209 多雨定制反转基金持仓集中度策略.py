...
STRATEGY_ID = "DUOYU003209"
STRATEGY_NAME = "多雨定制反转基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003209"
CATEGORY = "基金持仓集中度"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 20 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "机构持仓数据季度披露存在严重滞后。 反转风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 20 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 基金持仓集中度_v7 | style: 反转 | seed: 20256416
