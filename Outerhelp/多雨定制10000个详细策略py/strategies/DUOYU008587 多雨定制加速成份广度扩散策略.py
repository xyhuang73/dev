...
STRATEGY_ID = "DUOYU008587"
STRATEGY_NAME = "多雨定制加速成份广度扩散策略"
SOURCE_FACTOR_ID = "DUOYU008587"
CATEGORY = "成份广度扩散"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 成份广度扩散_v6 | style: 加速 | seed: 20289215
