...
STRATEGY_ID = "DUOYU000454"
STRATEGY_NAME = "多雨定制集中缺口回补修复策略"
SOURCE_FACTOR_ID = "DUOYU000454"
CATEGORY = "缺口回补修复"
STYLE = "集中"
OPERATOR_ID = "缺口回补修复_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    gap_fill = -_zscore(overnight_gap, 11)
    trend = _ret(close, 21)
    raw = gap_fill * _sigmoid_gate(trend, 0, 3) + 0.1 * VARIANT_WEIGHT * _amount_z(panel, 11)
    raw = raw * _max_dd_guard(drawdown, -0.12)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 缺口回补修复_v1 | style: 集中 | seed: 20243910
