...
STRATEGY_ID = "DUOYU006210"
STRATEGY_NAME = "多雨定制共振尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006210"
CATEGORY = "尾部风险阈值"
STYLE = "共振"
OPERATOR_ID = "尾部风险阈值_v7"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "尾部风险模型依赖历史分布假设。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 11 个交易日。", "先计算 尾部风险阈值 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU006210 - 多雨定制共振尾部风险阈值策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    tail_z = _tail_risk_z(close, 27, 0.05)
    raw = 0.33 * VARIANT_WEIGHT * _zscore(tail_z, 5) + _ret(close, 27)
    raw = raw * _max_dd_guard(drawdown, -0.10)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 尾部风险阈值_v7 | style: 共振 | seed: 20273516
