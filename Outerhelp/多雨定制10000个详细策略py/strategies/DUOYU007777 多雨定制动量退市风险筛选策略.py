...
STRATEGY_ID = "DUOYU007777"
STRATEGY_NAME = "多雨定制动量退市风险筛选策略"
SOURCE_FACTOR_ID = "DUOYU007777"
CATEGORY = "退市风险筛选"
STYLE = "动量"
OPERATOR_ID = "退市风险筛选_v7"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 17
FAST_WINDOW = 16
SLOW_WINDOW = 22
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：退市风险低+审计质量高+价格趋势好=安全边际充足，离退市越远越安全。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：退市风险低+审计质量高+价格趋势好=安全边际充足，离退市越远越安全。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    st_flag = _field(panel, "st_risk_flag") if "st_risk_flag" in panel.columns else _zscore(delist_score, 27)
    ...
    raw = 0.54 * VARIANT_WEIGHT * _zscore(risk_dist, 14) + 0.22 * VARIANT_WEIGHT * _zscore(audit, 27) + _ret(close, 27)
    raw = raw * _max_dd_guard(drawdown, -0.08)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 退市风险筛选_v7 | style: 动量 | seed: 20284216
