...
STRATEGY_ID = "DUOYU004938"
STRATEGY_NAME = "多雨定制质量Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU004938"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
"""
DUOYU004938 - 多雨定制质量Amihud 非流动性策略
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: amihud_非流动性_v2 | style: 质量 | seed: 20267311
