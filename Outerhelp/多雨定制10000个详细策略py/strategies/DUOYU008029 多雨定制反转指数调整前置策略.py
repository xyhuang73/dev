...
STRATEGY_ID = "DUOYU008029"
STRATEGY_NAME = "多雨定制反转指数调整前置策略"
SOURCE_FACTOR_ID = "DUOYU008029"
CATEGORY = "指数调整前置"
STYLE = "反转"
OPERATOR_ID = "指数调整前置_v6"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 15
FAST_WINDOW = 14
SLOW_WINDOW = 20
VARIANT_WEIGHT = 0.95
REQUIRED_FIELDS = ["close", "amount", "event_calendar_distance"]

ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "指数调整规则公开后前置收益衰减。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 15 个交易日。", "先计算 指数调整前置 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008029 - 多雨定制反转指数调整前置策略

策略主题：指数调整前置 / 反转
适用范围：宽基/行业
主要周期：15 个交易日
必需字段：close, amount, event_calendar_distance

经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    close = _field(panel, "close")
    amount = _field(panel, "amount")
    rebalance_dist = _field(panel, "event_calendar_distance")
    frontrun_score = _event_proximity(rebalance_dist, 14) * _amount_z(panel, 13)
    raw = 0.26 * VARIANT_WEIGHT * _zscore(frontrun_score, 13) + _ret(close, 23)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 指数调整前置_v6 | style: 反转 | seed: 20285415
