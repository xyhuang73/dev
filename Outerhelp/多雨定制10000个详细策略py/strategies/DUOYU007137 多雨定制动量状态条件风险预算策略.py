...
STRATEGY_ID = "DUOYU007137"
STRATEGY_NAME = "多雨定制动量状态条件风险预算策略"
SOURCE_FACTOR_ID = "DUOYU007137"
CATEGORY = "状态条件风险预算"
STYLE = "动量"
OPERATOR_ID = "状态条件风险预算_v5"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 10
FAST_WINDOW = 10
SLOW_WINDOW = 15
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _ret(close, 18) - 0.24 * VARIANT_WEIGHT * _zscore(beta, 18).abs() - 0.17 * VARIANT_WEIGHT * _zscore(linkage, 18).abs() - 0.3 * VARIANT_WEIGHT * _zscore(downside, 18).abs()
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 状态条件风险预算_v5 | style: 动量 | seed: 20280214
