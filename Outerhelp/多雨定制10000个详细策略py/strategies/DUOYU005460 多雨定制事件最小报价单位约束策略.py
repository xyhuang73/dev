...
STRATEGY_ID = "DUOYU005460"
STRATEGY_NAME = "多雨定制事件最小报价单位约束策略"
SOURCE_FACTOR_ID = "DUOYU005460"
CATEGORY = "最小报价单位约束"
STYLE = "事件"
OPERATOR_ID = "最小报价单位约束_v1"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 17
FAST_WINDOW = 10
SLOW_WINDOW = 18
VARIANT_WEIGHT = 0.98
REQUIRED_FIELDS = ["close", "bid_ask_spread", "amount"]

ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "标的拆分/合并事件影响 tick 约束计算。 事件风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 17 个交易日。", "先计算 最小报价单位约束 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU005460 - 多雨定制事件最小报价单位约束策略

策略主题：最小报价单位约束 / 事件
适用范围：全市场
主要周期：17 个交易日
必需字段：close, bid_ask_spread, amount

经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    tick_constraint = (spread / (close.abs() + EPS)).clip(0, 0.05)
    raw = -0.28 * VARIANT_WEIGHT * _zscore(tick_constraint, 20) + 0.25 * VARIANT_WEIGHT * _amount_z(panel, 10)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 最小报价单位约束_v1 | style: 事件 | seed: 20270510
