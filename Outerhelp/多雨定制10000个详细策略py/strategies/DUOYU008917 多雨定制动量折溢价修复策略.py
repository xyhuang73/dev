...
STRATEGY_ID = "DUOYU008917"
STRATEGY_NAME = "多雨定制动量折溢价修复策略"
SOURCE_FACTOR_ID = "DUOYU008917"
CATEGORY = "折溢价修复"
STYLE = "动量"
OPERATOR_ID = "折溢价修复_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -_zscore(premium, 22) + 0.32 * VARIANT_WEIGHT * _ret(nav, 22) + 0.15 * VARIANT_WEIGHT * _ret(close, 11)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 折溢价修复_v1 | style: 动量 | seed: 20293210
