...
STRATEGY_ID = "DUOYU003085"
STRATEGY_NAME = "多雨定制确认基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003085"
CATEGORY = "基金持仓集中度"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 基金持仓集中度_v1 | style: 确认 | seed: 20256010
