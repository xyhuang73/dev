...
STRATEGY_ID = "DUOYU003315"
STRATEGY_NAME = "多雨定制轮动机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003315"
CATEGORY = "机构散户资金博弈"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 机构散户资金博弈_v2 | style: 轮动 | seed: 20258011
