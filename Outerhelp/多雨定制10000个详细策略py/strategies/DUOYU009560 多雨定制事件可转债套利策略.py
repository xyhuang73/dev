...
STRATEGY_ID = "DUOYU009560"
STRATEGY_NAME = "多雨定制事件可转债套利策略"
SOURCE_FACTOR_ID = "DUOYU009560"
CATEGORY = "可转债套利"
STYLE = "事件"
OPERATOR_ID = "可转债套利_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.33 * VARIANT_WEIGHT * arb_signal + _ret(close, 23)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 可转债套利_v2 | style: 事件 | seed: 20298511
