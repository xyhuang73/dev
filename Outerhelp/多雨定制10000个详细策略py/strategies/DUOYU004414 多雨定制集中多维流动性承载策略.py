...
STRATEGY_ID = "DUOYU004414"
STRATEGY_NAME = "多雨定制集中多维流动性承载策略"
SOURCE_FACTOR_ID = "DUOYU004414"
CATEGORY = "多维流动性承载"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 多维流动性承载_v3 | style: 集中 | seed: 20263912
