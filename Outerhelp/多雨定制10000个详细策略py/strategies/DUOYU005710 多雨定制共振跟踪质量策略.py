...
STRATEGY_ID = "DUOYU005710"
STRATEGY_NAME = "多雨定制共振跟踪质量策略"
SOURCE_FACTOR_ID = "DUOYU005710"
CATEGORY = "跟踪质量"
STYLE = "共振"
OPERATOR_ID = "跟踪质量_v6"
MARKET_SCOPE = "宽基/行业"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _ret(index_close, 15) - 0.47 * VARIANT_WEIGHT * _zscore(tracking_error, 15).abs() - 0.17 * VARIANT_WEIGHT * _zscore(tracking_diff, 15).abs()
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 跟踪质量_v6 | style: 共振 | seed: 20270515
