...
STRATEGY_ID = "DUOYU004117"
STRATEGY_NAME = "多雨定制动量行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004117"
CATEGORY = "行业资金轮动动量"
STYLE = "动量"
OPERATOR_ID = "行业资金轮动动量_v6"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 12
FAST_WINDOW = 10
SLOW_WINDOW = 20
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "行业分类标准变化影响信号连续性。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 12 个交易日。", "先计算 行业资金轮动动量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU004117 - 多雨定制动量行业资金轮动动量策略
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    sector_relative = _cs_rank(_zscore(net_flow, 9)) - _cs_rank(_zscore(net_flow, 24))
    raw = 0.32 * VARIANT_WEIGHT * sector_relative + _ret(close, 9)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 行业资金轮动动量_v6 | style: 动量 | seed: 20262215
