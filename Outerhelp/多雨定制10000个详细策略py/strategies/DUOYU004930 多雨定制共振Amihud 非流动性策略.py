...
STRATEGY_ID = "DUOYU004930"
STRATEGY_NAME = "多雨定制共振Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU004930"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
"""
DUOYU004930 - 多雨定制共振Amihud 非流动性策略
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: amihud_非流动性_v2 | style: 共振 | seed: 20266511
