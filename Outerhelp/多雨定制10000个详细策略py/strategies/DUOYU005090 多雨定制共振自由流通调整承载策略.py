...
STRATEGY_ID = "DUOYU005090"
STRATEGY_NAME = "多雨定制共振自由流通调整承载策略"
SOURCE_FACTOR_ID = "DUOYU005090"
CATEGORY = "自由流通调整承载"
STYLE = "共振"
OPERATOR_ID = "自由流通调整承载_v1"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 16
FAST_WINDOW = 10
SLOW_WINDOW = 27
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    free_float_adj = _zscore(turnover, 27) / (1.0 + _zscore(spread, 27).abs())
    raw = 0.3 * VARIANT_WEIGHT * _zscore(free_float_adj, 10) + 0.23 * VARIANT_WEIGHT * _amount_z(panel, 10)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 自由流通调整承载_v1 | style: 共振 | seed: 20267510
