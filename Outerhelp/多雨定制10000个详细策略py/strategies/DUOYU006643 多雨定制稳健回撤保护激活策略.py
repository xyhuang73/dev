...
STRATEGY_ID = "DUOYU006643"
STRATEGY_NAME = "多雨定制稳健回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006643"
CATEGORY = "回撤保护激活"
STYLE = "稳健"
...
ECONOMIC_LOGIC_CN = "该策略以约 4 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。"
...
经济逻辑：
该策略以约 4 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤保护激活_v5 | style: 稳健 | seed: 20275814
