...
STRATEGY_ID = "DUOYU000337"
STRATEGY_NAME = "多雨定制动量多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000337"
CATEGORY = "多时间框架动量"
STYLE = "动量"
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 多时间框架动量_v6 | style: 动量 | seed: 20243215
