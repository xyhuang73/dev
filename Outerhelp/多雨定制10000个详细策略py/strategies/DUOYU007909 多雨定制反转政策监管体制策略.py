...
STRATEGY_ID = "DUOYU007909"
STRATEGY_NAME = "多雨定制反转政策监管体制策略"
SOURCE_FACTOR_ID = "DUOYU007909"
CATEGORY = "政策监管体制"
STYLE = "反转"
OPERATOR_ID = "政策监管体制_v7"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 14
FAST_WINDOW = 9
SLOW_WINDOW = 27
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    policy_cycle = _field(panel, "industry_policy_cycle") if "industry_policy_cycle" in panel.columns else _zscore(policy_sent, 30)
    raw = -0.34 * VARIANT_WEIGHT * _zscore(reg_risk, 30) + 0.24 * VARIANT_WEIGHT * _zscore(policy_sent, 9) + 0.3 * VARIANT_WEIGHT * _zscore(policy_cycle, 30)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 政策监管体制_v7 | style: 反转 | seed: 20284416
