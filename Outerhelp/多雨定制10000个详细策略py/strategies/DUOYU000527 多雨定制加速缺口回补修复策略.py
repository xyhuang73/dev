...
STRATEGY_ID = "DUOYU000527"
STRATEGY_NAME = "多雨定制加速缺口回补修复策略"
SOURCE_FACTOR_ID = "DUOYU000527"
CATEGORY = "缺口回补修复"
STYLE = "加速"
OPERATOR_ID = "缺口回补修复_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    trend = _ret(close, 23)
    raw = gap_fill * _sigmoid_gate(trend, 0, 3) + 0.2 * VARIANT_WEIGHT * _amount_z(panel, 12)
    raw = raw * _max_dd_guard(drawdown, -0.12)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 缺口回补修复_v5 | style: 加速 | seed: 20243214
