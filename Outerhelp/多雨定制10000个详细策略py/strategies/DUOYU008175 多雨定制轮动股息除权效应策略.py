...
STRATEGY_ID = "DUOYU008175"
STRATEGY_NAME = "多雨定制轮动股息除权效应策略"
SOURCE_FACTOR_ID = "DUOYU008175"
CATEGORY = "股息除权效应"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "红利税和除权价格调整需考虑。 轮动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 股息除权效应_v6 | style: 轮动 | seed: 20287015
