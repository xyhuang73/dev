...
STRATEGY_ID = "DUOYU008107"
STRATEGY_NAME = "多雨定制加速股息除权效应策略"
SOURCE_FACTOR_ID = "DUOYU008107"
CATEGORY = "股息除权效应"
STYLE = "加速"
OPERATOR_ID = "股息除权效应_v3"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    div_score = _zscore(dividend_yield, 27) * _event_proximity(div_date_dist, 8)
    raw = 0.21 * VARIANT_WEIGHT * div_score + _ret(close, 9)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 股息除权效应_v3 | style: 加速 | seed: 20286212
