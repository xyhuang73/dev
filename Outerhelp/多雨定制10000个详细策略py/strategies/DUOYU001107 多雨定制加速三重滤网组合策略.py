...
STRATEGY_ID = "DUOYU001107"
STRATEGY_NAME = "多雨定制加速三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001107"
CATEGORY = "三重滤网组合"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 三重滤网组合_v2 | style: 加速 | seed: 20246211
