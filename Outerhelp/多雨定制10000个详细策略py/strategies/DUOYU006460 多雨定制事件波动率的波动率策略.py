...
STRATEGY_ID = "DUOYU006460"
STRATEGY_NAME = "多雨定制事件波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006460"
CATEGORY = "波动率的波动率"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v3 | style: 事件 | seed: 20276512
