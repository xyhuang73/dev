...
STRATEGY_ID = "DUOYU006725"
STRATEGY_NAME = "多雨定制确认保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006725"
CATEGORY = "保证金追缴级联代理"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 保证金追缴级联代理_v1 | style: 确认 | seed: 20277010
