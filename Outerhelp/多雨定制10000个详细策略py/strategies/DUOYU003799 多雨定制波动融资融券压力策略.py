...
STRATEGY_ID = "DUOYU003799"
STRATEGY_NAME = "多雨定制波动融资融券压力策略"
SOURCE_FACTOR_ID = "DUOYU003799"
CATEGORY = "融资融券压力"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高融资比例+价格上涨=杠杆驱动行情（需警惕），高融券+价格下跌=空头挤压机会。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
RISK_NOTE_CN = "两融数据有延迟且受政策影响。 波动风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 11 个交易日。", "先计算 融资融券压力 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU003799 - 多雨定制波动融资融券压力策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高融资比例+价格上涨=杠杆驱动行情（需警惕），高融券+价格下跌=空头挤压机会。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 融资融券压力_v8 | style: 波动 | seed: 20260417
