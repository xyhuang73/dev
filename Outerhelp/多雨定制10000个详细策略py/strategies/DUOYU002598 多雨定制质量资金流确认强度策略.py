...
STRATEGY_ID = "DUOYU002598"
STRATEGY_NAME = "多雨定制质量资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002598"
CATEGORY = "资金流确认强度"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 资金流确认强度_v6 | style: 质量 | seed: 20254315
