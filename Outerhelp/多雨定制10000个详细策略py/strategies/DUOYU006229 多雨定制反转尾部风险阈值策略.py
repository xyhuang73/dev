...
STRATEGY_ID = "DUOYU006229"
STRATEGY_NAME = "多雨定制反转尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006229"
CATEGORY = "尾部风险阈值"
STYLE = "反转"
OPERATOR_ID = "尾部风险阈值_v8"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    tail_z = _tail_risk_z(close, 18, 0.05)
    raw = 0.35 * VARIANT_WEIGHT * _zscore(tail_z, 10) + _ret(close, 18)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 尾部风险阈值_v8 | style: 反转 | seed: 20273417
