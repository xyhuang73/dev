...
STRATEGY_ID = "DUOYU003014"
STRATEGY_NAME = "多雨定制集中拥挤度退潮策略"
SOURCE_FACTOR_ID = "DUOYU003014"
CATEGORY = "拥挤度退潮"
STYLE = "集中"
OPERATOR_ID = "拥挤度退潮_v7"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    weakening = (_ret(close, 11) < 0).astype(float)
    raw = -_zscore(attention, 11) * weakening - 0.19 * VARIANT_WEIGHT * _zscore(turnover, 11)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 拥挤度退潮_v7 | style: 集中 | seed: 20255916
