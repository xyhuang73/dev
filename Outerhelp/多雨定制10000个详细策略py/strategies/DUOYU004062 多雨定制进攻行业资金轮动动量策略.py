...
STRATEGY_ID = "DUOYU004062"
STRATEGY_NAME = "多雨定制进攻行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004062"
CATEGORY = "行业资金轮动动量"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 行业资金轮动动量_v4 | style: 进攻 | seed: 20260713
