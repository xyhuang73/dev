...
STRATEGY_ID = "DUOYU002789"
STRATEGY_NAME = "多雨定制反转份额扩张吸收策略"
SOURCE_FACTOR_ID = "DUOYU002789"
CATEGORY = "份额扩张吸收"
STYLE = "反转"
OPERATOR_ID = "份额扩张吸收_v6"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 19
FAST_WINDOW = 11
SLOW_WINDOW = 20
VARIANT_WEIGHT = 0.95
REQUIRED_FIELDS = ["close", "fund_share", "amount"]

ECONOMIC_LOGIC_CN = "该策略以约 19 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "份额变化数据存在 T+1 以上滞后。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 19 个交易日。", "先计算 份额扩张吸收 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU002789 - 多雨定制反转份额扩张吸收策略

策略主题：份额扩张吸收 / 反转
适用范围：宽基/行业/主题
主要周期：19 个交易日
必需字段：close, fund_share, amount

经济逻辑：
该策略以约 19 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    resilience = _ret(close, 10) - _downside_vol(close, 10)
    raw = _diff(fund_share, 22) * resilience + 0.27 * VARIANT_WEIGHT * _amount_z(panel, 22)
    score = _risk_scale(_cs_rank(raw), panel, 18)
    # instance: 份额扩张吸收_v6 | style: 反转 | seed: 20254415
