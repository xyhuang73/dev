...
STRATEGY_ID = "DUOYU002655"
STRATEGY_NAME = "多雨定制轮动资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002655"
CATEGORY = "资金流确认强度"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "大额申赎可能导致资金流指标短期失真。 轮动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 资金流确认强度_v9 | style: 轮动 | seed: 20254018
