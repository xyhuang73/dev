...
STRATEGY_ID = "DUOYU007897"
STRATEGY_NAME = "多雨定制动量政策监管体制策略"
SOURCE_FACTOR_ID = "DUOYU007897"
CATEGORY = "政策监管体制"
STYLE = "动量"
OPERATOR_ID = "政策监管体制_v6"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 15
FAST_WINDOW = 9
SLOW_WINDOW = 23
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    policy_cycle = _field(panel, "industry_policy_cycle") if "industry_policy_cycle" in panel.columns else _zscore(policy_sent, 28)
    raw = -0.3 * VARIANT_WEIGHT * _zscore(reg_risk, 28) + 0.25 * VARIANT_WEIGHT * _zscore(policy_sent, 8) + 0.24 * VARIANT_WEIGHT * _zscore(policy_cycle, 28)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 政策监管体制_v6 | style: 动量 | seed: 20285215
