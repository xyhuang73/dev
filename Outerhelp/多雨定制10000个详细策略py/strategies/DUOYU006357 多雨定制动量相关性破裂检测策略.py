...
STRATEGY_ID = "DUOYU006357"
STRATEGY_NAME = "多雨定制动量相关性破裂检测策略"
SOURCE_FACTOR_ID = "DUOYU006357"
CATEGORY = "相关性破裂检测"
STYLE = "动量"
OPERATOR_ID = "相关性破裂检测_v6"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 16
FAST_WINDOW = 14
SLOW_WINDOW = 15
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "相关性估计在短窗口内不稳定。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 16 个交易日。", "先计算 相关性破裂检测 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU006357 - 多雨定制动量相关性破裂检测策略
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _ret(close, 18) - 0.2 * VARIANT_WEIGHT * _zscore(beta, 18).abs() + 0.28 * VARIANT_WEIGHT * corr_break
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 相关性破裂检测_v6 | style: 动量 | seed: 20275215
