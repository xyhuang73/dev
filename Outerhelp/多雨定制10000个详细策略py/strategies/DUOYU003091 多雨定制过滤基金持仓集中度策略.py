...
STRATEGY_ID = "DUOYU003091"
STRATEGY_NAME = "多雨定制过滤基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003091"
CATEGORY = "基金持仓集中度"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
RISK_NOTE_CN = "机构持仓数据季度披露存在严重滞后。 过滤风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 11 个交易日。", "先计算 基金持仓集中度 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU003091 - 多雨定制过滤基金持仓集中度策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 基金持仓集中度_v1 | style: 过滤 | seed: 20256610
