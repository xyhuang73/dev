...
STRATEGY_ID = "DUOYU004854"
STRATEGY_NAME = "多雨定制集中流动性冲击吸收策略"
SOURCE_FACTOR_ID = "DUOYU004854"
CATEGORY = "流动性冲击吸收"
STYLE = "集中"
OPERATOR_ID = "流动性冲击吸收_v7"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    shock = _zscore(turnover, 5) - _zscore(turnover, 24)
    absorption = _ret(close, 5) / (_zscore(turnover, 5).abs() + EPS)
    raw = 0.31 * VARIANT_WEIGHT * shock + 0.42 * VARIANT_WEIGHT * _zscore(absorption, 5)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 流动性冲击吸收_v7 | style: 集中 | seed: 20265916
