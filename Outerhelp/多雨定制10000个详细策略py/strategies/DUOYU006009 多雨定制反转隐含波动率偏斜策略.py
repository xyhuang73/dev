...
STRATEGY_ID = "DUOYU006009"
STRATEGY_NAME = "多雨定制反转隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU006009"
CATEGORY = "隐含波动率偏斜"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 隐含波动率偏斜_v5 | style: 反转 | seed: 20272414
