...
STRATEGY_ID = "DUOYU004540"
STRATEGY_NAME = "多雨定制事件多维流动性承载策略"
SOURCE_FACTOR_ID = "DUOYU004540"
CATEGORY = "多维流动性承载"
STYLE = "事件"
OPERATOR_ID = "多维流动性承载_v9"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 10
FAST_WINDOW = 8
SLOW_WINDOW = 24
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.44 * VARIANT_WEIGHT * _amount_z(panel, 26) + 0.3 * VARIANT_WEIGHT * _zscore(turnover, 8) - _zscore(spread, 8).abs()
    score = _cs_rank(raw)
    # instance: 多维流动性承载_v9 | style: 事件 | seed: 20264518
