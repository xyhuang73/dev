...
STRATEGY_ID = "DUOYU005320"
STRATEGY_NAME = "多雨定制事件委托簿斜率策略"
SOURCE_FACTOR_ID = "DUOYU005320"
CATEGORY = "委托簿斜率"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "委托簿数据需要 Level 2 行情支持。 事件风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 委托簿斜率_v3 | style: 事件 | seed: 20269512
