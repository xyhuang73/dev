...
STRATEGY_ID = "DUOYU007997"
STRATEGY_NAME = "多雨定制动量指数调整前置策略"
SOURCE_FACTOR_ID = "DUOYU007997"
CATEGORY = "指数调整前置"
STYLE = "动量"
OPERATOR_ID = "指数调整前置_v4"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 10
SLOW_WINDOW = 22
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    frontrun_score = _event_proximity(rebalance_dist, 10) * _amount_z(panel, 9)
    raw = 0.4 * VARIANT_WEIGHT * _zscore(frontrun_score, 9) + _ret(close, 26)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 指数调整前置_v4 | style: 动量 | seed: 20286213
