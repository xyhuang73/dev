...
STRATEGY_ID = "DUOYU003180"
STRATEGY_NAME = "多雨定制事件基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003180"
CATEGORY = "基金持仓集中度"
STYLE = "事件"
OPERATOR_ID = "基金持仓集中度_v5"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 15
FAST_WINDOW = 15
SLOW_WINDOW = 20
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.38 * VARIANT_WEIGHT * _zscore(inst_holding, 22) + 0.4 * VARIANT_WEIGHT * divergence - 0.27 * VARIANT_WEIGHT * _concentration_risk(top10_conc, 22)
    raw = raw * _sigmoid_gate(_inst_flow_divergence(_ret(close, 14), inst_change, 14), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 基金持仓集中度_v5 | style: 事件 | seed: 20257514
