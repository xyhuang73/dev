...
STRATEGY_ID = "DUOYU002497"
STRATEGY_NAME = "多雨定制动量资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002497"
CATEGORY = "资金流确认强度"
STYLE = "动量"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "大额申赎可能导致资金流指标短期失真。 动量风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 资金流确认强度_v1 | style: 动量 | seed: 20254210
