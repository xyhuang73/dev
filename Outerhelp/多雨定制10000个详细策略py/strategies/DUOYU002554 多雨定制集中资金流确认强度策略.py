...
STRATEGY_ID = "DUOYU002554"
STRATEGY_NAME = "多雨定制集中资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002554"
CATEGORY = "资金流确认强度"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 资金流确认强度_v4 | style: 集中 | seed: 20253913
