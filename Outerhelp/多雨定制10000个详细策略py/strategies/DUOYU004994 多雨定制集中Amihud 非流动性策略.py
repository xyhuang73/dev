...
STRATEGY_ID = "DUOYU004994"
STRATEGY_NAME = "多雨定制集中Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU004994"
CATEGORY = "Amihud 非流动性"
STYLE = "集中"
OPERATOR_ID = "amihud_非流动性_v5"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 10
FAST_WINDOW = 8
SLOW_WINDOW = 12
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
"""
DUOYU004994 - 多雨定制集中Amihud 非流动性策略
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    amihud = _zscore(ret_abs / (np.log1p(amount.clip(lower=0)) + EPS), 15)
    raw = -amihud + 0.23 * VARIANT_WEIGHT * _amount_z(panel, 10)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: amihud_非流动性_v5 | style: 集中 | seed: 20266914
