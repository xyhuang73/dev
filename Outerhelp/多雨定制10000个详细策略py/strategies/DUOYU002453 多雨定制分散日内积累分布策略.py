...
STRATEGY_ID = "DUOYU002453"
STRATEGY_NAME = "多雨定制分散日内积累分布策略"
SOURCE_FACTOR_ID = "DUOYU002453"
CATEGORY = "日内积累分布"
STYLE = "分散"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。"
RISK_NOTE_CN = "极端行情下日内结构信号可能失效。 分散风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 日内积累分布_v9 | style: 分散 | seed: 20252818
