...
STRATEGY_ID = "DUOYU007249"
STRATEGY_NAME = "多雨定制反转波动率压缩预警策略"
SOURCE_FACTOR_ID = "DUOYU007249"
CATEGORY = "波动率压缩预警"
STYLE = "反转"
OPERATOR_ID = "波动率压缩预警_v3"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    vol_z = _zscore(tr, 15).abs()
    compression = -_cs_rank(vol_z)
    breakout_potential = _breakout(panel, 8)
    raw = compression * 0.41 * VARIANT_WEIGHT + breakout_potential * 0.4 * VARIANT_WEIGHT
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 波动率压缩预警_v3 | style: 反转 | seed: 20280412
