...
STRATEGY_ID = "DUOYU003290"
STRATEGY_NAME = "多雨定制共振机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003290"
CATEGORY = "机构散户资金博弈"
STYLE = "共振"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "持有人数据频率低，信号更新不及时。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 机构散户资金博弈 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU003290 - 多雨定制共振机构散户资金博弈策略
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 机构散户资金博弈_v1 | style: 共振 | seed: 20257510
