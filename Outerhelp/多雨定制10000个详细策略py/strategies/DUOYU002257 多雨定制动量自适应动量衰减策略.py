...
STRATEGY_ID = "DUOYU002257"
STRATEGY_NAME = "多雨定制动量自适应动量衰减策略"
SOURCE_FACTOR_ID = "DUOYU002257"
CATEGORY = "自适应动量衰减"
STYLE = "动量"
OPERATOR_ID = "自适应动量衰减_v9"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    close = _field(panel, "close")
    ret = _ret(close, 14)
    decay = np.exp(-3.33 * VARIANT_WEIGHT * _downside_vol(close, 14).abs())
    raw = ret * decay + 0.16 * VARIANT_WEIGHT * _amount_z(panel, 10)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 自适应动量衰减_v9 | style: 动量 | seed: 20252218
