...
STRATEGY_ID = "DUOYU007100"
STRATEGY_NAME = "多雨定制事件状态条件风险预算策略"
SOURCE_FACTOR_ID = "DUOYU007100"
CATEGORY = "状态条件风险预算"
STYLE = "事件"
OPERATOR_ID = "状态条件风险预算_v3"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 14
FAST_WINDOW = 7
SLOW_WINDOW = 15
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "风险预算参数依赖历史估计。 事件风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 14 个交易日。", "先计算 状态条件风险预算 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU007100 - 多雨定制事件状态条件风险预算策略
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _ret(close, 17) - 0.16 * VARIANT_WEIGHT * _zscore(beta, 17).abs() - 0.31 * VARIANT_WEIGHT * _zscore(linkage, 17).abs() - 0.36 * VARIANT_WEIGHT * _zscore(downside, 17).abs()
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 状态条件风险预算_v3 | style: 事件 | seed: 20280512
