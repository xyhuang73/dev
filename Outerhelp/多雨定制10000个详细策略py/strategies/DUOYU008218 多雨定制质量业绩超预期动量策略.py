...
STRATEGY_ID = "DUOYU008218"
STRATEGY_NAME = "多雨定制质量业绩超预期动量策略"
SOURCE_FACTOR_ID = "DUOYU008218"
CATEGORY = "业绩超预期动量"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
RISK_NOTE_CN = "分析师预测存在系统性乐观偏差。 质量风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 业绩超预期动量_v1 | style: 质量 | seed: 20288310
