...
STRATEGY_ID = "DUOYU004049"
STRATEGY_NAME = "多雨定制反转行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004049"
CATEGORY = "行业资金轮动动量"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 行业资金轮动动量_v3 | style: 反转 | seed: 20261412
