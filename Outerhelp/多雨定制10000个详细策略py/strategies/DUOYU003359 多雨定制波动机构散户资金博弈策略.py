...
STRATEGY_ID = "DUOYU003359"
STRATEGY_NAME = "多雨定制波动机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003359"
CATEGORY = "机构散户资金博弈"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
RISK_NOTE_CN = "持有人数据频率低，信号更新不及时。 波动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 机构散户资金博弈_v4 | style: 波动 | seed: 20258413
