...
STRATEGY_ID = "DUOYU006152"
STRATEGY_NAME = "多雨定制择时尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006152"
CATEGORY = "尾部风险阈值"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 19 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
...
经济逻辑：
该策略以约 19 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 尾部风险阈值_v4 | style: 择时 | seed: 20273713
