...
STRATEGY_ID = "DUOYU009792"
STRATEGY_NAME = "多雨定制择时低波动异象策略"
SOURCE_FACTOR_ID = "DUOYU009792"
CATEGORY = "低波动异象"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：低波动+高质量=低波异象，历史上低波动组合经风险调整后回报优于高波动。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 低波动异象_v2 | style: 择时 | seed: 20299711
