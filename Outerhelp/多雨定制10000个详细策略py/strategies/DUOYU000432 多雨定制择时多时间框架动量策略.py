...
STRATEGY_ID = "DUOYU000432"
STRATEGY_NAME = "多雨定制择时多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000432"
CATEGORY = "多时间框架动量"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
RISK_NOTE_CN = "多周期共振失效于剧烈反转行情。 择时风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 多时间框架动量_v11 | style: 择时 | seed: 20242720
