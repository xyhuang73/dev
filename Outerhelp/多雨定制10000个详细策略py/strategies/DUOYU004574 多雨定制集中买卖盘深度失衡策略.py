...
STRATEGY_ID = "DUOYU004574"
STRATEGY_NAME = "多雨定制集中买卖盘深度失衡策略"
SOURCE_FACTOR_ID = "DUOYU004574"
CATEGORY = "买卖盘深度失衡"
STYLE = "集中"
OPERATOR_ID = "买卖盘深度失衡_v2"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 10
FAST_WINDOW = 8
SLOW_WINDOW = 16
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    depth_imbalance = (_zscore(turnover, 7) - _zscore(spread, 7).abs()) / (1.0 + _zscore(spread, 20).abs())
    raw = 0.46 * VARIANT_WEIGHT * _zscore(depth_imbalance, 7) + _ret(close, 20)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 买卖盘深度失衡_v2 | style: 集中 | seed: 20264911
