...
STRATEGY_ID = "DUOYU004817"
STRATEGY_NAME = "多雨定制动量流动性冲击吸收策略"
SOURCE_FACTOR_ID = "DUOYU004817"
CATEGORY = "流动性冲击吸收"
STYLE = "动量"
OPERATOR_ID = "流动性冲击吸收_v5"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    shock = _zscore(turnover, 5) - _zscore(turnover, 20)
    absorption = _ret(close, 5) / (_zscore(turnover, 5).abs() + EPS)
    raw = 0.18 * VARIANT_WEIGHT * shock + 0.28 * VARIANT_WEIGHT * _zscore(absorption, 5)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 流动性冲击吸收_v5 | style: 动量 | seed: 20266214
