...
STRATEGY_ID = "DUOYU009377"
STRATEGY_NAME = "多雨定制动量商品联动对冲策略"
SOURCE_FACTOR_ID = "DUOYU009377"
CATEGORY = "商品联动对冲"
STYLE = "动量"
OPERATOR_ID = "商品联动对冲_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "商品 标的 受 contango/backwardation 影响。 动量风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.39 * VARIANT_WEIGHT * commodity_link + _ret(close, 24)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 商品联动对冲_v5 | style: 动量 | seed: 20296214
