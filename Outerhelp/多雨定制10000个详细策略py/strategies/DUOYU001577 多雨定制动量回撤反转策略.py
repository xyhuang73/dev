...
STRATEGY_ID = "DUOYU001577"
STRATEGY_NAME = "多雨定制动量回撤反转策略"
SOURCE_FACTOR_ID = "DUOYU001577"
CATEGORY = "回撤反转"
STYLE = "动量"
OPERATOR_ID = "回撤反转_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    trend_gate = (_ret(close, 21) > 0).astype(float)
    raw = -_ret(close, 8) * trend_gate + 0.16 * VARIANT_WEIGHT * _amount_z(panel, 8)
    raw = raw * _sigmoid_gate(_amount_z(panel, 8), -0.5, 4)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 回撤反转_v5 | style: 动量 | seed: 20249214
