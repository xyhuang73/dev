...
STRATEGY_ID = "DUOYU003674"
STRATEGY_NAME = "多雨定制集中融资融券压力策略"
SOURCE_FACTOR_ID = "DUOYU003674"
CATEGORY = "融资融券压力"
STYLE = "集中"
OPERATOR_ID = "融资融券压力_v2"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 10
FAST_WINDOW = 11
SLOW_WINDOW = 21
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高融资比例+价格上涨=杠杆驱动行情（需警惕），高融券+价格下跌=空头挤压机会。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高融资比例+价格上涨=杠杆驱动行情（需警惕），高融券+价格下跌=空头挤压机会。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.31 * VARIANT_WEIGHT * _zscore(margin_ratio, 27) + 0.31 * VARIANT_WEIGHT * _ret(close, 9) - _zscore(short_ratio, 9).abs()
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 融资融券压力_v2 | style: 集中 | seed: 20259911
