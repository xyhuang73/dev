...
STRATEGY_ID = "DUOYU006057"
STRATEGY_NAME = "多雨定制动量隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU006057"
CATEGORY = "隐含波动率偏斜"
STYLE = "动量"
OPERATOR_ID = "隐含波动率偏斜_v7"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 10
FAST_WINDOW = 7
SLOW_WINDOW = 13
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    iv_change = _diff(iv, 6)
    raw = -0.4 * VARIANT_WEIGHT * _zscore(iv_change, 6) + _ret(close, 16)
    raw = raw * _max_dd_guard(drawdown, -0.12)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 隐含波动率偏斜_v7 | style: 动量 | seed: 20273216
