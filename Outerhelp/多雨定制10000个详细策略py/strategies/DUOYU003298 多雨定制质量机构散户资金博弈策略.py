...
STRATEGY_ID = "DUOYU003298"
STRATEGY_NAME = "多雨定制质量机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003298"
CATEGORY = "机构散户资金博弈"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
RISK_NOTE_CN = "持有人数据频率低，信号更新不及时。 质量风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 机构散户资金博弈_v1 | style: 质量 | seed: 20258310
