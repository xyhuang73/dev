...
STRATEGY_ID = "DUOYU001297"
STRATEGY_NAME = "多雨定制动量相对强弱动量策略"
SOURCE_FACTOR_ID = "DUOYU001297"
CATEGORY = "相对强弱动量"
STYLE = "动量"
OPERATOR_ID = "相对强弱动量_v1"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 16
FAST_WINDOW = 10
SLOW_WINDOW = 17
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _ret(close, 21) - _ret(index_close, 21) + 0.38 * VARIANT_WEIGHT * _zscore(breadth, 9)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 相对强弱动量_v1 | style: 动量 | seed: 20248210
