...
STRATEGY_ID = "DUOYU003240"
STRATEGY_NAME = "多雨定制事件基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003240"
CATEGORY = "基金持仓集中度"
STYLE = "事件"
OPERATOR_ID = "基金持仓集中度_v8"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 15
FAST_WINDOW = 17
SLOW_WINDOW = 24
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    divergence = _inst_flow_divergence(_ret(_field(panel, "close"), 16), inst_change, 16)
    raw = 0.24 * VARIANT_WEIGHT * _zscore(inst_holding, 26) + 0.44 * VARIANT_WEIGHT * divergence - 0.19 * VARIANT_WEIGHT * _concentration_risk(top10_conc, 26)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 基金持仓集中度_v8 | style: 事件 | seed: 20257517
