...
STRATEGY_ID = "DUOYU002378"
STRATEGY_NAME = "多雨定制质量日内积累分布策略"
SOURCE_FACTOR_ID = "DUOYU002378"
CATEGORY = "日内积累分布"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 日内积累分布_v5 | style: 质量 | seed: 20253314
