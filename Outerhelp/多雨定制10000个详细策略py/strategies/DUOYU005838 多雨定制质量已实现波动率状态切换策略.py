...
STRATEGY_ID = "DUOYU005838"
STRATEGY_NAME = "多雨定制质量已实现波动率状态切换策略"
SOURCE_FACTOR_ID = "DUOYU005838"
CATEGORY = "已实现波动率状态切换"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
"""
DUOYU005838 - 多雨定制质量已实现波动率状态切换策略
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：波动扩张时跟随趋势，波动收缩时做反转，自适应状态切换减少策略与市场状态错配。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 已实现波动率状态切换_v4 | style: 质量 | seed: 20272313
