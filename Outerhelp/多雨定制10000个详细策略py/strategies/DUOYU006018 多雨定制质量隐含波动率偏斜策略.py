...
STRATEGY_ID = "DUOYU006018"
STRATEGY_NAME = "多雨定制质量隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU006018"
CATEGORY = "隐含波动率偏斜"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 隐含波动率偏斜_v5 | style: 质量 | seed: 20273314
