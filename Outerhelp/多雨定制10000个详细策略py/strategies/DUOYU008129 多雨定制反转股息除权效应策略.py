...
STRATEGY_ID = "DUOYU008129"
STRATEGY_NAME = "多雨定制反转股息除权效应策略"
SOURCE_FACTOR_ID = "DUOYU008129"
CATEGORY = "股息除权效应"
STYLE = "反转"
OPERATOR_ID = "股息除权效应_v4"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    div_score = _zscore(dividend_yield, 16) * _event_proximity(div_date_dist, 11)
    raw = 0.26 * VARIANT_WEIGHT * div_score + _ret(close, 8)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 股息除权效应_v4 | style: 反转 | seed: 20286413
