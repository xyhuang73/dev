...
STRATEGY_ID = "DUOYU001813"
STRATEGY_NAME = "多雨定制分散横截面动量差异策略"
SOURCE_FACTOR_ID = "DUOYU001813"
CATEGORY = "横截面动量差异"
STYLE = "分散"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。"
RISK_NOTE_CN = "高波动市场中动量差异噪声增大。 分散风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 横截面动量差异_v7 | style: 分散 | seed: 20249816
