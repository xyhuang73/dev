...
STRATEGY_ID = "DUOYU002000"
STRATEGY_NAME = "多雨定制事件路径效率动量策略"
SOURCE_FACTOR_ID = "DUOYU002000"
CATEGORY = "路径效率动量"
STYLE = "事件"
OPERATOR_ID = "路径效率动量_v6"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    eff = _rolling_efficiency_ratio(close, 27)
    raw = 0.41 * VARIANT_WEIGHT * _zscore(eff, 5) + _ret(close, 27)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 路径效率动量_v6 | style: 事件 | seed: 20251515
