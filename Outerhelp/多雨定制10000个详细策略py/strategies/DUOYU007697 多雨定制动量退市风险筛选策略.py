...
STRATEGY_ID = "DUOYU007697"
STRATEGY_NAME = "多雨定制动量退市风险筛选策略"
SOURCE_FACTOR_ID = "DUOYU007697"
CATEGORY = "退市风险筛选"
STYLE = "动量"
OPERATOR_ID = "退市风险筛选_v3"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 20
FAST_WINDOW = 16
SLOW_WINDOW = 23
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["close", "delisting_risk_score", "st_risk_flag", "audit_opinion_score"]

ECONOMIC_LOGIC_CN = "该策略以约 20 个交易日为主要持有观察期，核心逻辑是：退市风险低+审计质量高+价格趋势好=安全边际充足，离退市越远越安全。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "退市风险评估模型可能遗漏灰犀牛事件。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 20 个交易日。", "先计算 退市风险筛选 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU007697 - 多雨定制动量退市风险筛选策略

策略主题：退市风险筛选 / 动量
适用范围：全市场
主要周期：20 个交易日
必需字段：close, delisting_risk_score, st_risk_flag, audit_opinion_score

经济逻辑：
该策略以约 20 个交易日为主要持有观察期，核心逻辑是：退市风险低+审计质量高+价格趋势好=安全边际充足，离退市越远越安全。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _delisting_distance(risk_score: pd.Series, st_flag: pd.Series, min_days: int = 60) -> pd.Series:
    """退市风险距离：低风险+无ST=安全距离大。"""
    risk = pd.to_numeric(risk_score, errors="coerce")
    st = pd.to_numeric(st_flag, errors="coerce").fillna(0)
    raw = risk + 0.5 * st
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    delist_score = _field(panel, "delisting_risk_score")
    st_flag = _field(panel, "st_risk_flag") if "st_risk_flag" in panel.columns else _zscore(delist_score, 28)
    audit = _field(panel, "audit_opinion_score") if "audit_opinion_score" in panel.columns else (1.0 - delist_score)
    risk_dist = _delisting_distance(delist_score, st_flag, 60)
    raw = 0.46 * VARIANT_WEIGHT * _zscore(risk_dist, 14) + 0.21 * VARIANT_WEIGHT * _zscore(audit, 28) + _ret(close, 28)
    raw = raw * _max_dd_guard(drawdown, -0.08)
    score = _risk_scale(_cs_rank(raw), panel, 19)
    # instance: 退市风险筛选_v3 | style: 动量 | seed: 20284212
