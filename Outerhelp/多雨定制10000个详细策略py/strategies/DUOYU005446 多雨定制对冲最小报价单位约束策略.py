...
STRATEGY_ID = "DUOYU005446"
STRATEGY_NAME = "多雨定制对冲最小报价单位约束策略"
SOURCE_FACTOR_ID = "DUOYU005446"
CATEGORY = "最小报价单位约束"
STYLE = "对冲"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。"
RISK_NOTE_CN = "标的拆分/合并事件影响 tick 约束计算。 对冲风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 11 个交易日。", "先计算 最小报价单位约束 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU005446 - 多雨定制对冲最小报价单位约束策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 最小报价单位约束_v1 | style: 对冲 | seed: 20269110
