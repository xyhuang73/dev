...
STRATEGY_ID = "DUOYU001484"
STRATEGY_NAME = "多雨定制修复回撤反转策略"
SOURCE_FACTOR_ID = "DUOYU001484"
CATEGORY = "回撤反转"
STYLE = "修复"
OPERATOR_ID = "回撤反转_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤反转_v1 | style: 修复 | seed: 20247910
