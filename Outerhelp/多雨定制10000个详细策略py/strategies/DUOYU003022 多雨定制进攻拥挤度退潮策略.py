...
STRATEGY_ID = "DUOYU003022"
STRATEGY_NAME = "多雨定制进攻拥挤度退潮策略"
SOURCE_FACTOR_ID = "DUOYU003022"
CATEGORY = "拥挤度退潮"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 拥挤度退潮_v8 | style: 进攻 | seed: 20254717
