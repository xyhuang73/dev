...
STRATEGY_ID = "DUOYU004275"
STRATEGY_NAME = "多雨定制轮动申赎套利流策略"
SOURCE_FACTOR_ID = "DUOYU004275"
CATEGORY = "申赎套利流"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折溢价套利资金流：溢价回归+净值上涨=套利与趋势共振，折价扩大+净值下跌=压力。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "跨境 标的 申赎机制不同于境内标的。 轮动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折溢价套利资金流：溢价回归+净值上涨=套利与趋势共振，折价扩大+净值下跌=压力。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 申赎套利流_v5 | style: 轮动 | seed: 20263014
