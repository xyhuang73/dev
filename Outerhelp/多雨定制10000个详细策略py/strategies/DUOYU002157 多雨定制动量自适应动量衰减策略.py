...
STRATEGY_ID = "DUOYU002157"
STRATEGY_NAME = "多雨定制动量自适应动量衰减策略"
SOURCE_FACTOR_ID = "DUOYU002157"
CATEGORY = "自适应动量衰减"
STYLE = "动量"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 自适应动量衰减_v4 | style: 动量 | seed: 20252213
