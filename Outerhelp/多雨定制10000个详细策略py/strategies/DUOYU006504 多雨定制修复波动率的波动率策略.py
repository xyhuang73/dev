...
STRATEGY_ID = "DUOYU006504"
STRATEGY_NAME = "多雨定制修复波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006504"
CATEGORY = "波动率的波动率"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v6 | style: 修复 | seed: 20274915
