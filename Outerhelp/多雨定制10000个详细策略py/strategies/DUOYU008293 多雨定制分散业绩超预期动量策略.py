...
STRATEGY_ID = "DUOYU008293"
STRATEGY_NAME = "多雨定制分散业绩超预期动量策略"
SOURCE_FACTOR_ID = "DUOYU008293"
CATEGORY = "业绩超预期动量"
STYLE = "分散"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 业绩超预期动量_v5 | style: 分散 | seed: 20287814
