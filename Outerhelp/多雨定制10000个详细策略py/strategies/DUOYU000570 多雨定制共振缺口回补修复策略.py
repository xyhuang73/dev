...
STRATEGY_ID = "DUOYU000570"
STRATEGY_NAME = "多雨定制共振缺口回补修复策略"
SOURCE_FACTOR_ID = "DUOYU000570"
CATEGORY = "缺口回补修复"
STYLE = "共振"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "重大事件驱动缺口可能长期不回补。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 缺口回补修复 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU000570 - 多雨定制共振缺口回补修复策略
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：跳空缺口回补信号叠加趋势过滤，识别缺口修复过程中的承接力度。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 缺口回补修复_v7 | style: 共振 | seed: 20243516
