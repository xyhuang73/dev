...
STRATEGY_ID = "DUOYU003611"
STRATEGY_NAME = "多雨定制过滤大宗交易足迹策略"
SOURCE_FACTOR_ID = "DUOYU003611"
CATEGORY = "大宗交易足迹"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：大宗交易溢价/折价反映机构投资者对当前价格的判断，溢价=看好、折价=看淡。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：大宗交易溢价/折价反映机构投资者对当前价格的判断，溢价=看好、折价=看淡。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 大宗交易足迹_v8 | style: 过滤 | seed: 20258617
