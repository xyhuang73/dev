...
STRATEGY_ID = "DUOYU008703"
STRATEGY_NAME = "多雨定制稳健日历效应季节策略"
SOURCE_FACTOR_ID = "DUOYU008703"
CATEGORY = "日历效应季节"
STYLE = "稳健"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：日历效应（季节性规律）+成交确认，捕捉每年固定时间窗口的统计规律。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。"
RISK_NOTE_CN = "日历效应可能随市场结构变化而漂移。 稳健风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 12 个交易日。", "先计算 日历效应季节 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008703 - 多雨定制稳健日历效应季节策略
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：日历效应（季节性规律）+成交确认，捕捉每年固定时间窗口的统计规律。 当前风格为[稳健]（权重系数=0.73，DD敏感度=1.5）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 日历效应季节_v5 | style: 稳健 | seed: 20289814
