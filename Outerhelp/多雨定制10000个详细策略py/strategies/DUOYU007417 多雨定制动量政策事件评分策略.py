...
STRATEGY_ID = "DUOYU007417"
STRATEGY_NAME = "多雨定制动量政策事件评分策略"
SOURCE_FACTOR_ID = "DUOYU007417"
CATEGORY = "政策事件评分"
STYLE = "动量"
OPERATOR_ID = "政策事件评分_v3"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 17
FAST_WINDOW = 14
SLOW_WINDOW = 17
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _zscore(policy, 12) * _event_proximity(distance, 15) + 0.25 * VARIANT_WEIGHT * _amount_z(panel, 12)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 政策事件评分_v3 | style: 动量 | seed: 20282212
