...
STRATEGY_ID = "DUOYU001681"
STRATEGY_NAME = "多雨定制核心横截面动量差异策略"
SOURCE_FACTOR_ID = "DUOYU001681"
CATEGORY = "横截面动量差异"
STYLE = "核心"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 横截面动量差异_v1 | style: 核心 | seed: 20248610
