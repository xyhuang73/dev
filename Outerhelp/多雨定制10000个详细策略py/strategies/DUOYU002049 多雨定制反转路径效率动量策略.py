...
STRATEGY_ID = "DUOYU002049"
STRATEGY_NAME = "多雨定制反转路径效率动量策略"
SOURCE_FACTOR_ID = "DUOYU002049"
CATEGORY = "路径效率动量"
STYLE = "反转"
OPERATOR_ID = "路径效率动量_v9"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.51 * VARIANT_WEIGHT * _zscore(eff, 7) + _ret(close, 21)
    score = _risk_scale(_cs_rank(raw), panel, 7)
    # instance: 路径效率动量_v9 | style: 反转 | seed: 20250418
