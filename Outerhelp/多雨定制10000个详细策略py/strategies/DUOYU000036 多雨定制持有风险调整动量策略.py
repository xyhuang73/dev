...
STRATEGY_ID = "DUOYU000036"
STRATEGY_NAME = "多雨定制持有风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000036"
CATEGORY = "风险调整动量"
STYLE = "持有"
OPERATOR_ID = "风险调整动量_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    raw = _ret(close, 24) - 0.44 * VARIANT_WEIGHT * _downside_vol(close, 24) + 0.19 * VARIANT_WEIGHT * _amount_z(panel, 6)
    score = _risk_scale(_cs_rank(raw), panel, 18)
    # instance: 风险调整动量_v2 | style: 持有 | seed: 20242111
