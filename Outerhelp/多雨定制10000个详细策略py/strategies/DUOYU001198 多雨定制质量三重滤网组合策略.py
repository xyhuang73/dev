...
STRATEGY_ID = "DUOYU001198"
STRATEGY_NAME = "多雨定制质量三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001198"
CATEGORY = "三重滤网组合"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 三重滤网组合_v6 | style: 质量 | seed: 20247315
