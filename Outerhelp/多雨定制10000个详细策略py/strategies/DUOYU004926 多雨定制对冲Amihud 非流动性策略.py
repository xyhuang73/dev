...
STRATEGY_ID = "DUOYU004926"
STRATEGY_NAME = "多雨定制对冲Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU004926"
...
ECONOMIC_LOGIC_CN = "该策略以约 4 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。"
...
"""
DUOYU004926 - 多雨定制对冲Amihud 非流动性策略
...
经济逻辑：
该策略以约 4 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[对冲]（权重系数=0.68，DD敏感度=1.6）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: amihud_非流动性_v2 | style: 对冲 | seed: 20266111
