...
STRATEGY_ID = "DUOYU002777"
STRATEGY_NAME = "多雨定制动量份额扩张吸收策略"
SOURCE_FACTOR_ID = "DUOYU002777"
CATEGORY = "份额扩张吸收"
STYLE = "动量"
OPERATOR_ID = "份额扩张吸收_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "份额变化数据存在 T+1 以上滞后。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 10 个交易日。", "先计算 份额扩张吸收 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU002777 - 多雨定制动量份额扩张吸收策略
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    resilience = _ret(close, 8) - _downside_vol(close, 8)
    raw = _diff(fund_share, 22) * resilience + 0.25 * VARIANT_WEIGHT * _amount_z(panel, 22)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 份额扩张吸收_v5 | style: 动量 | seed: 20255214
