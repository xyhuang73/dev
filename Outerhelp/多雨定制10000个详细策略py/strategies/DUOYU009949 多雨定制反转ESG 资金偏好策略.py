...
STRATEGY_ID = "DUOYU009949"
STRATEGY_NAME = "多雨定制反转ESG 资金偏好策略"
SOURCE_FACTOR_ID = "DUOYU009949"
CATEGORY = "ESG 资金偏好"
STYLE = "反转"
OPERATOR_ID = "esg_资金偏好_v4"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 16
FAST_WINDOW = 11
SLOW_WINDOW = 24
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高环保合规+资金流入=ESG 主题资金偏好，捕捉可持续投资趋势。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高环保合规+资金流入=ESG 主题资金偏好，捕捉可持续投资趋势。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    compliance = _field(panel, "environmental_compliance") if "environmental_compliance" in panel.columns else _zscore(_field(panel, "quality_score"), 27)
    net_flow = _field(panel, "net_flow")
    esg_flow = _zscore(compliance, 27) * _zscore(net_flow, 10)
    raw = 0.4 * VARIANT_WEIGHT * _zscore(esg_flow, 10) + _ret(close, 27)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: esg_资金偏好_v4 | style: 反转 | seed: 20300413
