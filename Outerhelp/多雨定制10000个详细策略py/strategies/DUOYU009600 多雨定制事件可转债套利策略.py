...
STRATEGY_ID = "DUOYU009600"
STRATEGY_NAME = "多雨定制事件可转债套利策略"
SOURCE_FACTOR_ID = "DUOYU009600"
CATEGORY = "可转债套利"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "套利机制依赖 标的 申赎效率。 事件风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 15 个交易日。", "先计算 可转债套利 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009600 - 多雨定制事件可转债套利策略
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 可转债套利_v4 | style: 事件 | seed: 20298513
