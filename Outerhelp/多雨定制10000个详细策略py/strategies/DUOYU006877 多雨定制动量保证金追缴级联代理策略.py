...
STRATEGY_ID = "DUOYU006877"
STRATEGY_NAME = "多雨定制动量保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006877"
CATEGORY = "保证金追缴级联代理"
STYLE = "动量"
OPERATOR_ID = "保证金追缴级联代理_v8"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 10
FAST_WINDOW = 10
SLOW_WINDOW = 22
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    margin_ratio = _field(panel, "margin_ratio") if "margin_ratio" in panel.columns else _zscore(_amount_z(panel, 9), 26)
    downside = _downside_vol(close, 26)
    margin_stress = _zscore(margin_ratio, 9) * downside
    raw = -0.43 * VARIANT_WEIGHT * margin_stress + _ret(close, 26)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 保证金追缴级联代理_v8 | style: 动量 | seed: 20278217
