...
STRATEGY_ID = "DUOYU000417"
STRATEGY_NAME = "多雨定制动量多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000417"
CATEGORY = "多时间框架动量"
STYLE = "动量"
OPERATOR_ID = "多时间框架动量_v10"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    ret_m = _ret(close, 14)
    ret_l = _ret(close, 20)
    raw = 0.26 * VARIANT_WEIGHT * ret_s + 0.35 * VARIANT_WEIGHT * ret_m + 0.18 * VARIANT_WEIGHT * ret_l - 0.31 * _downside_vol(close, 20)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 多时间框架动量_v10 | style: 动量 | seed: 20243219
