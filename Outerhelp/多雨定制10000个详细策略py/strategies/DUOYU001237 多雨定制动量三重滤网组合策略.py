...
STRATEGY_ID = "DUOYU001237"
STRATEGY_NAME = "多雨定制动量三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001237"
CATEGORY = "三重滤网组合"
STYLE = "动量"
OPERATOR_ID = "三重滤网组合_v8"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    vol_score = -_downside_vol(close, 16)
    flow_confirm = _zscore(_field(panel, "net_flow"), 10) if "net_flow" in panel.columns else amount_z
    raw = 0.49 * VARIANT_WEIGHT * trend_score + 0.32 * VARIANT_WEIGHT * vol_score + 0.3 * VARIANT_WEIGHT * flow_confirm
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 三重滤网组合_v8 | style: 动量 | seed: 20247217
