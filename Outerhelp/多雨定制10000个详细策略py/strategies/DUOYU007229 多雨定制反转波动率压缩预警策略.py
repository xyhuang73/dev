...
STRATEGY_ID = "DUOYU007229"
STRATEGY_NAME = "多雨定制反转波动率压缩预警策略"
SOURCE_FACTOR_ID = "DUOYU007229"
CATEGORY = "波动率压缩预警"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：波动率极度压缩后往往跟随大行情，压缩状态+潜在突破=高期望信号。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率压缩预警_v2 | style: 反转 | seed: 20280411
