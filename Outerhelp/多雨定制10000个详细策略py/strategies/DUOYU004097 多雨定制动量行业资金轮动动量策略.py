...
STRATEGY_ID = "DUOYU004097"
STRATEGY_NAME = "多雨定制动量行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004097"
CATEGORY = "行业资金轮动动量"
STYLE = "动量"
OPERATOR_ID = "行业资金轮动动量_v5"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 7
FAST_WINDOW = 11
SLOW_WINDOW = 13
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    sector_relative = _cs_rank(_zscore(net_flow, 10)) - _cs_rank(_zscore(net_flow, 16))
    raw = 0.3 * VARIANT_WEIGHT * sector_relative + _ret(close, 10)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 行业资金轮动动量_v5 | style: 动量 | seed: 20262214
