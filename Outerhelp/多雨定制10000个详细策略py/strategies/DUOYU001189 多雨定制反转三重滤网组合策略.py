...
STRATEGY_ID = "DUOYU001189"
STRATEGY_NAME = "多雨定制反转三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001189"
CATEGORY = "三重滤网组合"
STYLE = "反转"
OPERATOR_ID = "三重滤网组合_v6"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    amount_z = _amount_z(panel, 10)
    trend_score = _ret(close, 18)
    vol_score = -_downside_vol(close, 14)
    flow_confirm = _zscore(_field(panel, "net_flow"), 10) if "net_flow" in panel.columns else amount_z
    raw = 0.51 * VARIANT_WEIGHT * trend_score + 0.22 * VARIANT_WEIGHT * vol_score + 0.22 * VARIANT_WEIGHT * flow_confirm
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 三重滤网组合_v6 | style: 反转 | seed: 20246415
