...
STRATEGY_ID = "DUOYU002105"
STRATEGY_NAME = "多雨定制确认自适应动量衰减策略"
SOURCE_FACTOR_ID = "DUOYU002105"
CATEGORY = "自适应动量衰减"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：动量信号根据波动率自适应衰减，高波动时动量可信度降低。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 自适应动量衰减_v2 | style: 确认 | seed: 20251011
