...
STRATEGY_ID = "DUOYU009410"
STRATEGY_NAME = "多雨定制共振汇率风险轮动策略"
SOURCE_FACTOR_ID = "DUOYU009410"
CATEGORY = "汇率风险轮动"
STYLE = "共振"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "汇率波动受宏观政策影响，可预测性低。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 12 个交易日。", "先计算 汇率风险轮动 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009410 - 多雨定制共振汇率风险轮动策略
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 汇率风险轮动_v1 | style: 共振 | seed: 20296510
