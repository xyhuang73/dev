...
STRATEGY_ID = "DUOYU002374"
STRATEGY_NAME = "多雨定制集中日内积累分布策略"
SOURCE_FACTOR_ID = "DUOYU002374"
CATEGORY = "日内积累分布"
STYLE = "集中"
OPERATOR_ID = "日内积累分布_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    raw = _close_location(panel) + 0.35 * VARIANT_WEIGHT * _vwap_support(panel, 6) + 0.22 * VARIANT_WEIGHT * _amount_z(panel, 6)
    raw = raw * _sigmoid_gate(_close_location(panel), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 日内积累分布_v5 | style: 集中 | seed: 20252914
