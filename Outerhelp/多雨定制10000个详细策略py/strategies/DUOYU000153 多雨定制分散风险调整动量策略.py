...
STRATEGY_ID = "DUOYU000153"
STRATEGY_NAME = "多雨定制分散风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000153"
CATEGORY = "风险调整动量"
STYLE = "分散"
...
ECONOMIC_LOGIC_CN = "该策略以约 5 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。"
...
经济逻辑：
该策略以约 5 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风险调整动量_v8 | style: 分散 | seed: 20241817
