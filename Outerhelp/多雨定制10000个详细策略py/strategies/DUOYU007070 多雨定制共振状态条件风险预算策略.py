...
STRATEGY_ID = "DUOYU007070"
STRATEGY_NAME = "多雨定制共振状态条件风险预算策略"
SOURCE_FACTOR_ID = "DUOYU007070"
CATEGORY = "状态条件风险预算"
STYLE = "共振"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 状态条件风险预算_v2 | style: 共振 | seed: 20279511
