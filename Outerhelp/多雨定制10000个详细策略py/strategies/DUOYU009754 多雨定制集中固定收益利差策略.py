...
STRATEGY_ID = "DUOYU009754"
STRATEGY_NAME = "多雨定制集中固定收益利差策略"
SOURCE_FACTOR_ID = "DUOYU009754"
CATEGORY = "固定收益利差"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 20 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
RISK_NOTE_CN = "利率变化对 carry 策略影响重大。 集中风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 20 个交易日。", "先计算 固定收益利差 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009754 - 多雨定制集中固定收益利差策略
...
经济逻辑：
该策略以约 20 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 固定收益利差_v6 | style: 集中 | seed: 20298915
