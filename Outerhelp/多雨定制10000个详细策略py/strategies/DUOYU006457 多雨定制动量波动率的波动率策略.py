...
STRATEGY_ID = "DUOYU006457"
STRATEGY_NAME = "多雨定制动量波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006457"
CATEGORY = "波动率的波动率"
STYLE = "动量"
OPERATOR_ID = "波动率的波动率_v3"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    close = _field(panel, "close")
    ret_1d = _ret(close, 1)
    vol = _by_etf(ret_1d.abs(), lambda x: x.rolling(18, min_periods=5).std())
    vol_of_vol = _by_etf(vol, lambda x: x.rolling(18, min_periods=5).std())
    raw = -0.41 * VARIANT_WEIGHT * _zscore(vol_of_vol, 11) + _ret(close, 18)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 波动率的波动率_v3 | style: 动量 | seed: 20276212
