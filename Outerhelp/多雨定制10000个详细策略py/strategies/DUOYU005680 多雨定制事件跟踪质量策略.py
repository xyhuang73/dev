...
STRATEGY_ID = "DUOYU005680"
STRATEGY_NAME = "多雨定制事件跟踪质量策略"
SOURCE_FACTOR_ID = "DUOYU005680"
CATEGORY = "跟踪质量"
STYLE = "事件"
OPERATOR_ID = "跟踪质量_v4"
MARKET_SCOPE = "宽基/行业"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _ret(index_close, 20) - 0.43 * VARIANT_WEIGHT * _zscore(tracking_error, 20).abs() - 0.29 * VARIANT_WEIGHT * _zscore(tracking_diff, 20).abs()
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 跟踪质量_v4 | style: 事件 | seed: 20271513
