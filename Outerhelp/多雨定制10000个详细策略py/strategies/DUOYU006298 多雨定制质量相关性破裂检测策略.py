...
STRATEGY_ID = "DUOYU006298"
STRATEGY_NAME = "多雨定制质量相关性破裂检测策略"
SOURCE_FACTOR_ID = "DUOYU006298"
CATEGORY = "相关性破裂检测"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
RISK_NOTE_CN = "相关性估计在短窗口内不稳定。 质量风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的与基准的相关性破裂意味着特质风险上升或跟踪失效，需警惕。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 相关性破裂检测_v3 | style: 质量 | seed: 20275312
