...
STRATEGY_ID = "DUOYU004536"
STRATEGY_NAME = "多雨定制持有多维流动性承载策略"
SOURCE_FACTOR_ID = "DUOYU004536"
CATEGORY = "多维流动性承载"
STYLE = "持有"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
RISK_NOTE_CN = "流动性指标在极端行情中可能突变。 持有风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 6 个交易日。", "先计算 多维流动性承载 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU004536 - 多雨定制持有多维流动性承载策略
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 多维流动性承载_v9 | style: 持有 | seed: 20264118
