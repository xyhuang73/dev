...
STRATEGY_ID = "DUOYU005114"
STRATEGY_NAME = "多雨定制集中自由流通调整承载策略"
SOURCE_FACTOR_ID = "DUOYU005114"
CATEGORY = "自由流通调整承载"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 自由流通调整承载_v2 | style: 集中 | seed: 20267911
