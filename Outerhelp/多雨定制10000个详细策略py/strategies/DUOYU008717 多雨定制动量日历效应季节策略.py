...
STRATEGY_ID = "DUOYU008717"
STRATEGY_NAME = "多雨定制动量日历效应季节策略"
SOURCE_FACTOR_ID = "DUOYU008717"
CATEGORY = "日历效应季节"
STYLE = "动量"
OPERATOR_ID = "日历效应季节_v5"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 18
FAST_WINDOW = 12
SLOW_WINDOW = 19
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["seasonality_score", "event_calendar_distance", "amount"]

ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：日历效应（季节性规律）+成交确认，捕捉每年固定时间窗口的统计规律。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "日历效应可能随市场结构变化而漂移。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 18 个交易日。", "先计算 日历效应季节 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008717 - 多雨定制动量日历效应季节策略

策略主题：日历效应季节 / 动量
适用范围：宽基/行业/主题
主要周期：18 个交易日
必需字段：seasonality_score, event_calendar_distance, amount

经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：日历效应（季节性规律）+成交确认，捕捉每年固定时间窗口的统计规律。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _event_proximity(distance: pd.Series, horizon_days: int) -> pd.Series:
    d = pd.to_numeric(distance, errors="coerce").abs()
    return (1.0 / (1.0 + d.clip(lower=0))) * (1.0 + min(30, int(horizon_days)) / 60.0)
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    seasonality = _field(panel, "seasonality_score")
    distance = _field(panel, "event_calendar_distance")
    raw = 0.53 * VARIANT_WEIGHT * _zscore(seasonality, 23) + _event_proximity(distance, 16) + 0.12 * VARIANT_WEIGHT * _amount_z(panel, 11)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 日历效应季节_v5 | style: 动量 | seed: 20291214
