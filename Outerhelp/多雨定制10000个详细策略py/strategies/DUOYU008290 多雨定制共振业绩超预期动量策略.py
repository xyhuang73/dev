...
STRATEGY_ID = "DUOYU008290"
STRATEGY_NAME = "多雨定制共振业绩超预期动量策略"
SOURCE_FACTOR_ID = "DUOYU008290"
CATEGORY = "业绩超预期动量"
STYLE = "共振"
OPERATOR_ID = "业绩超预期动量_v5"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 10
FAST_WINDOW = 11
SLOW_WINDOW = 28
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    surprise = _zscore(earnings_revision, 11) - _zscore(earnings_revision, 28)
    raw = 0.45 * VARIANT_WEIGHT * surprise + 0.21 * VARIANT_WEIGHT * _ret(industry_momentum, 11) + _ret(close, 11)
    raw = raw * _sigmoid_gate(surprise, 0, 4)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 业绩超预期动量_v5 | style: 共振 | seed: 20287514
