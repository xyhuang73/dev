...
STRATEGY_ID = "DUOYU004709"
STRATEGY_NAME = "多雨定制反转买卖盘深度失衡策略"
SOURCE_FACTOR_ID = "DUOYU004709"
CATEGORY = "买卖盘深度失衡"
STYLE = "反转"
OPERATOR_ID = "买卖盘深度失衡_v9"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 11
FAST_WINDOW = 11
SLOW_WINDOW = 20
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高换手+窄价差=深度买方市场，流动性充裕信号。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高换手+窄价差=深度买方市场，流动性充裕信号。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    depth_imbalance = (_zscore(turnover, 10) - _zscore(spread, 10).abs()) / (1.0 + _zscore(spread, 23).abs())
    raw = 0.45 * VARIANT_WEIGHT * _zscore(depth_imbalance, 10) + _ret(close, 23)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 买卖盘深度失衡_v9 | style: 反转 | seed: 20264418
