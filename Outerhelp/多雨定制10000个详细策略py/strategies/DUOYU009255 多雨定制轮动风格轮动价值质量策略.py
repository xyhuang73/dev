...
STRATEGY_ID = "DUOYU009255"
STRATEGY_NAME = "多雨定制轮动风格轮动价值质量策略"
SOURCE_FACTOR_ID = "DUOYU009255"
CATEGORY = "风格轮动价值质量"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "风格因子在不同市场周期表现差异大。 轮动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 风格轮动价值质量_v5 | style: 轮动 | seed: 20295014
