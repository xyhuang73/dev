...
STRATEGY_ID = "DUOYU003934"
STRATEGY_NAME = "多雨定制集中北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003934"
CATEGORY = "北向资金泄漏"
STYLE = "集中"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 北向资金泄漏_v6 | style: 集中 | seed: 20260915
