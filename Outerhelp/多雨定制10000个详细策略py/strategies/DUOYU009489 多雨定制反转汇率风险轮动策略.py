...
STRATEGY_ID = "DUOYU009489"
STRATEGY_NAME = "多雨定制反转汇率风险轮动策略"
SOURCE_FACTOR_ID = "DUOYU009489"
CATEGORY = "汇率风险轮动"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "汇率波动受宏观政策影响，可预测性低。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 14 个交易日。", "先计算 汇率风险轮动 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009489 - 多雨定制反转汇率风险轮动策略
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 汇率风险轮动_v5 | style: 反转 | seed: 20296414
