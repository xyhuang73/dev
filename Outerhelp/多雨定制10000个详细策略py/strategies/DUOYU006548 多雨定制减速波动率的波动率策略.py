...
STRATEGY_ID = "DUOYU006548"
STRATEGY_NAME = "多雨定制减速波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006548"
CATEGORY = "波动率的波动率"
STYLE = "减速"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v8 | style: 减速 | seed: 20275317
