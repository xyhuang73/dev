...
STRATEGY_ID = "DUOYU009989"
STRATEGY_NAME = "多雨定制反转ESG 资金偏好策略"
SOURCE_FACTOR_ID = "DUOYU009989"
CATEGORY = "ESG 资金偏好"
STYLE = "反转"
OPERATOR_ID = "esg_资金偏好_v6"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 14
FAST_WINDOW = 8
SLOW_WINDOW = 22
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    compliance = _field(panel, "environmental_compliance") if "environmental_compliance" in panel.columns else _zscore(_field(panel, "quality_score"), 25)
    net_flow = _field(panel, "net_flow")
    esg_flow = _zscore(compliance, 25) * _zscore(net_flow, 8)
    raw = 0.31 * VARIANT_WEIGHT * _zscore(esg_flow, 8) + _ret(close, 25)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: esg_资金偏好_v6 | style: 反转 | seed: 20300415
