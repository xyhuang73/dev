...
STRATEGY_ID = "DUOYU003927"
STRATEGY_NAME = "多雨定制加速北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003927"
CATEGORY = "北向资金泄漏"
STYLE = "加速"
OPERATOR_ID = "北向资金泄漏_v6"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 8
SLOW_WINDOW = 12
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：北向资金（外资）流向作为聪明钱代理变量，领先于本地资金。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.42 * VARIANT_WEIGHT * _zscore(northbound, 10) + _ret(close, 18)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 北向资金泄漏_v6 | style: 加速 | seed: 20260215
