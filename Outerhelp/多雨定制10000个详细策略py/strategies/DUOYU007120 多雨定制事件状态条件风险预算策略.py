...
STRATEGY_ID = "DUOYU007120"
STRATEGY_NAME = "多雨定制事件状态条件风险预算策略"
SOURCE_FACTOR_ID = "DUOYU007120"
CATEGORY = "状态条件风险预算"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 19 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 19 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 状态条件风险预算_v4 | style: 事件 | seed: 20280513
