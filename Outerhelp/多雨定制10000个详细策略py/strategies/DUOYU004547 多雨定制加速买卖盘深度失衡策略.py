...
STRATEGY_ID = "DUOYU004547"
STRATEGY_NAME = "多雨定制加速买卖盘深度失衡策略"
SOURCE_FACTOR_ID = "DUOYU004547"
CATEGORY = "买卖盘深度失衡"
STYLE = "加速"
OPERATOR_ID = "买卖盘深度失衡_v1"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 11
FAST_WINDOW = 6
SLOW_WINDOW = 10
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高换手+窄价差=深度买方市场，流动性充裕信号。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
RISK_NOTE_CN = "高频 microstructure 数据获取困难。 加速风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 11 个交易日。", "先计算 买卖盘深度失衡 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU004547 - 多雨定制加速买卖盘深度失衡策略
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高换手+窄价差=深度买方市场，流动性充裕信号。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    depth_imbalance = (_zscore(turnover, 10) - _zscore(spread, 10).abs()) / (1.0 + _zscore(spread, 15).abs())
    raw = 0.31 * VARIANT_WEIGHT * _zscore(depth_imbalance, 10) + _ret(close, 15)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 买卖盘深度失衡_v1 | style: 加速 | seed: 20264210
