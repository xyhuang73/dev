...
STRATEGY_ID = "DUOYU000820"
STRATEGY_NAME = "多雨定制事件成交量剖面支撑策略"
SOURCE_FACTOR_ID = "DUOYU000820"
CATEGORY = "成交量剖面支撑"
STYLE = "事件"
OPERATOR_ID = "成交量剖面支撑_v8"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：基于 VWAP 锚定的成交量支撑信号，衡量价格位于成交量加权均价上方的强度。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：基于 VWAP 锚定的成交量支撑信号，衡量价格位于成交量加权均价上方的强度。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.29 * VARIANT_WEIGHT * _zscore(vol_support, 13) + _ret(close, 15) + 0.12 * VARIANT_WEIGHT * _amount_z(panel, 13)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 成交量剖面支撑_v8 | style: 事件 | seed: 20245517
