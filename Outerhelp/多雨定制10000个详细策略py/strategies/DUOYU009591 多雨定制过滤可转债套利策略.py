...
STRATEGY_ID = "DUOYU009591"
STRATEGY_NAME = "多雨定制过滤可转债套利策略"
SOURCE_FACTOR_ID = "DUOYU009591"
CATEGORY = "可转债套利"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
RISK_NOTE_CN = "套利机制依赖 标的 申赎效率。 过滤风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 可转债套利_v4 | style: 过滤 | seed: 20297613
