...
STRATEGY_ID = "DUOYU009367"
STRATEGY_NAME = "多雨定制加速商品联动对冲策略"
SOURCE_FACTOR_ID = "DUOYU009367"
CATEGORY = "商品联动对冲"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
RISK_NOTE_CN = "商品 标的 受 contango/backwardation 影响。 加速风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 商品联动对冲_v5 | style: 加速 | seed: 20295214
