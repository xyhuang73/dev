...
STRATEGY_ID = "DUOYU001731"
STRATEGY_NAME = "多雨定制过滤横截面动量差异策略"
SOURCE_FACTOR_ID = "DUOYU001731"
CATEGORY = "横截面动量差异"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 横截面动量差异_v3 | style: 过滤 | seed: 20249612
