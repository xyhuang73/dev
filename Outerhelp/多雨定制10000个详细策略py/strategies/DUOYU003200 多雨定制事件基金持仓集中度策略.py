...
STRATEGY_ID = "DUOYU003200"
STRATEGY_NAME = "多雨定制事件基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003200"
CATEGORY = "基金持仓集中度"
STYLE = "事件"
OPERATOR_ID = "基金持仓集中度_v6"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 20
FAST_WINDOW = 12
SLOW_WINDOW = 17
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    divergence = _inst_flow_divergence(_ret(_field(panel, "close"), 12), inst_change, 12)
    raw = 0.23 * VARIANT_WEIGHT * _zscore(inst_holding, 19) + 0.34 * VARIANT_WEIGHT * divergence - 0.23 * VARIANT_WEIGHT * _concentration_risk(top10_conc, 19)
    score = _risk_scale(_cs_rank(raw), panel, 20)
    # instance: 基金持仓集中度_v6 | style: 事件 | seed: 20257515
