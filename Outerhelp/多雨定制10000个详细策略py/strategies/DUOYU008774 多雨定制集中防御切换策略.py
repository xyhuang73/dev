...
STRATEGY_ID = "DUOYU008774"
STRATEGY_NAME = "多雨定制集中防御切换策略"
SOURCE_FACTOR_ID = "DUOYU008774"
CATEGORY = "防御切换"
STYLE = "集中"
OPERATOR_ID = "防御切换_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -_zscore(appetite, 14) + 0.33 * VARIANT_WEIGHT * _zscore(dividend, 25) - _downside_vol(close, 25) - 0.21 * VARIANT_WEIGHT * _zscore(downside, 25).abs()
    score = _cs_rank(raw)
    # instance: 防御切换_v1 | style: 集中 | seed: 20291910
