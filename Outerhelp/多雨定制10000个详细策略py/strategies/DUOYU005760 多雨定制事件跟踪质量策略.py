...
STRATEGY_ID = "DUOYU005760"
STRATEGY_NAME = "多雨定制事件跟踪质量策略"
SOURCE_FACTOR_ID = "DUOYU005760"
CATEGORY = "跟踪质量"
STYLE = "事件"
OPERATOR_ID = "跟踪质量_v8"
MARKET_SCOPE = "宽基/行业"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 跟踪质量_v8 | style: 事件 | seed: 20271517
