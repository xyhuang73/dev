...
STRATEGY_ID = "DUOYU002834"
STRATEGY_NAME = "多雨定制集中份额扩张吸收策略"
SOURCE_FACTOR_ID = "DUOYU002834"
CATEGORY = "份额扩张吸收"
STYLE = "集中"
OPERATOR_ID = "份额扩张吸收_v8"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    resilience = _ret(close, 7) - _downside_vol(close, 7)
    raw = _diff(fund_share, 25) * resilience + 0.18 * VARIANT_WEIGHT * _amount_z(panel, 25)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 份额扩张吸收_v8 | style: 集中 | seed: 20254917
