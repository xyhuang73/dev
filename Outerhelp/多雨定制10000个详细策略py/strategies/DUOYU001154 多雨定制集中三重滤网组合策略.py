...
STRATEGY_ID = "DUOYU001154"
STRATEGY_NAME = "多雨定制集中三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001154"
CATEGORY = "三重滤网组合"
STYLE = "集中"
OPERATOR_ID = "三重滤网组合_v4"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    amount_z = _amount_z(panel, 9)
    trend_score = _ret(close, 18)
    vol_score = -_downside_vol(close, 13)
    flow_confirm = _zscore(_field(panel, "net_flow"), 9) if "net_flow" in panel.columns else amount_z
    raw = 0.47 * VARIANT_WEIGHT * trend_score + 0.34 * VARIANT_WEIGHT * vol_score + 0.16 * VARIANT_WEIGHT * flow_confirm
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 三重滤网组合_v4 | style: 集中 | seed: 20246913
