...
STRATEGY_ID = "DUOYU006996"
STRATEGY_NAME = "多雨定制持有最大回撤守护策略"
SOURCE_FACTOR_ID = "DUOYU006996"
CATEGORY = "最大回撤守护"
STYLE = "持有"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 最大回撤守护_v6 | style: 持有 | seed: 20279115
