...
STRATEGY_ID = "DUOYU003031"
STRATEGY_NAME = "多雨定制过滤拥挤度退潮策略"
SOURCE_FACTOR_ID = "DUOYU003031"
CATEGORY = "拥挤度退潮"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 4 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 4 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 拥挤度退潮_v8 | style: 过滤 | seed: 20255617
