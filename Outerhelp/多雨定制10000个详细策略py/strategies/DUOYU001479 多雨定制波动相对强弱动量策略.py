...
STRATEGY_ID = "DUOYU001479"
STRATEGY_NAME = "多雨定制波动相对强弱动量策略"
SOURCE_FACTOR_ID = "DUOYU001479"
CATEGORY = "相对强弱动量"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
RISK_NOTE_CN = "基准指数选择偏差影响信号有效性。 波动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 相对强弱动量_v10 | style: 波动 | seed: 20248419
