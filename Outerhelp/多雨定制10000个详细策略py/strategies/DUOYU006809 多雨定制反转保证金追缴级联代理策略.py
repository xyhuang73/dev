...
STRATEGY_ID = "DUOYU006809"
STRATEGY_NAME = "多雨定制反转保证金追缴级联代理策略"
SOURCE_FACTOR_ID = "DUOYU006809"
CATEGORY = "保证金追缴级联代理"
STYLE = "反转"
OPERATOR_ID = "保证金追缴级联代理_v5"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 11
SLOW_WINDOW = 17
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高杠杆+高波动=强制平仓风险，代理变量预警市场级联下跌。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    margin_ratio = _field(panel, "margin_ratio") if "margin_ratio" in panel.columns else _zscore(_amount_z(panel, 10), 19)
    downside = _downside_vol(close, 19)
    margin_stress = _zscore(margin_ratio, 10) * downside
    raw = -0.44 * VARIANT_WEIGHT * margin_stress + _ret(close, 19)
    raw = raw * _max_dd_guard(drawdown, -0.15)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 保证金追缴级联代理_v5 | style: 反转 | seed: 20277414
