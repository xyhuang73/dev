...
STRATEGY_ID = "DUOYU006549"
STRATEGY_NAME = "多雨定制反转波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006549"
CATEGORY = "波动率的波动率"
STYLE = "反转"
OPERATOR_ID = "波动率的波动率_v8"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.4 * VARIANT_WEIGHT * _zscore(vol_of_vol, 14) + _ret(close, 16)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 波动率的波动率_v8 | style: 反转 | seed: 20275417
