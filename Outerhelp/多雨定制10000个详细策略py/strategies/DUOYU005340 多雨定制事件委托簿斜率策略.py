...
STRATEGY_ID = "DUOYU005340"
STRATEGY_NAME = "多雨定制事件委托簿斜率策略"
SOURCE_FACTOR_ID = "DUOYU005340"
CATEGORY = "委托簿斜率"
STYLE = "事件"
OPERATOR_ID = "委托簿斜率_v4"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 7
FAST_WINDOW = 14
SLOW_WINDOW = 23
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：委托簿斜率：价差/换手比，低斜率=委托簿深、流动性好。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    book_slope = _zscore(spread, 13) / (_zscore(turnover, 13).abs() + EPS)
    raw = -0.41 * VARIANT_WEIGHT * _zscore(book_slope, 13) + _ret(close, 25)
    score = _risk_scale(_cs_rank(raw), panel, 7)
    # instance: 委托簿斜率_v4 | style: 事件 | seed: 20269513
