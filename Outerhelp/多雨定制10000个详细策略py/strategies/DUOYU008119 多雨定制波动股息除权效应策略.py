...
STRATEGY_ID = "DUOYU008119"
STRATEGY_NAME = "多雨定制波动股息除权效应策略"
SOURCE_FACTOR_ID = "DUOYU008119"
CATEGORY = "股息除权效应"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 股息除权效应_v3 | style: 波动 | seed: 20287412
