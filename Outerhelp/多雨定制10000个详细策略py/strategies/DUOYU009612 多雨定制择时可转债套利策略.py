...
STRATEGY_ID = "DUOYU009612"
STRATEGY_NAME = "多雨定制择时可转债套利策略"
SOURCE_FACTOR_ID = "DUOYU009612"
CATEGORY = "可转债套利"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 可转债套利_v5 | style: 择时 | seed: 20297714
