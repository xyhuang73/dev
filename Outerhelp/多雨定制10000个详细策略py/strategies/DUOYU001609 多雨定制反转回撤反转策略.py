...
STRATEGY_ID = "DUOYU001609"
STRATEGY_NAME = "多雨定制反转回撤反转策略"
SOURCE_FACTOR_ID = "DUOYU001609"
CATEGORY = "回撤反转"
STYLE = "反转"
OPERATOR_ID = "回撤反转_v7"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：中期趋势仍为正的前提下做短期回撤反转，兼顾方向过滤和逆向捕捉。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -_ret(close, 4) * trend_gate + 0.17 * VARIANT_WEIGHT * _amount_z(panel, 4)
    raw = raw * _sigmoid_gate(_amount_z(panel, 4), -0.5, 4)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 回撤反转_v7 | style: 反转 | seed: 20248416
