...
STRATEGY_ID = "DUOYU009236"
STRATEGY_NAME = "多雨定制持有风格轮动价值质量策略"
SOURCE_FACTOR_ID = "DUOYU009236"
CATEGORY = "风格轮动价值质量"
STYLE = "持有"
OPERATOR_ID = "风格轮动价值质量_v4"
MARKET_SCOPE = "风格/宽基"
HORIZON_DAYS = 4
FAST_WINDOW = 8
SLOW_WINDOW = 35
...
ECONOMIC_LOGIC_CN = "该策略以约 4 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
...
经济逻辑：
该策略以约 4 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.24 * VARIANT_WEIGHT * _zscore(valuation, 27) + 0.34 * VARIANT_WEIGHT * _zscore(quality, 27) + 0.33 * VARIANT_WEIGHT * _zscore(dividend, 27) + _ret(close, 13)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 风格轮动价值质量_v4 | style: 持有 | seed: 20295113
