...
STRATEGY_ID = "DUOYU007880"
STRATEGY_NAME = "多雨定制事件政策监管体制策略"
SOURCE_FACTOR_ID = "DUOYU007880"
CATEGORY = "政策监管体制"
STYLE = "事件"
OPERATOR_ID = "政策监管体制_v5"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 15
FAST_WINDOW = 15
SLOW_WINDOW = 23
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
RISK_NOTE_CN = "政策情绪 NLP 模型对措辞变化敏感。 事件风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 15 个交易日。", "先计算 政策监管体制 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU007880 - 多雨定制事件政策监管体制策略
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    policy_cycle = _field(panel, "industry_policy_cycle") if "industry_policy_cycle" in panel.columns else _zscore(policy_sent, 25)
    raw = -0.35 * VARIANT_WEIGHT * _zscore(reg_risk, 25) + 0.26 * VARIANT_WEIGHT * _zscore(policy_sent, 14) + 0.26 * VARIANT_WEIGHT * _zscore(policy_cycle, 25)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 政策监管体制_v5 | style: 事件 | seed: 20285514
