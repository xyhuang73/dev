...
STRATEGY_ID = "DUOYU008557"
STRATEGY_NAME = "多雨定制动量成份广度扩散策略"
SOURCE_FACTOR_ID = "DUOYU008557"
CATEGORY = "成份广度扩散"
STYLE = "动量"
OPERATOR_ID = "成份广度扩散_v4"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 15
FAST_WINDOW = 5
SLOW_WINDOW = 17
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "广度指标在风格极端偏移时可能失效。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 15 个交易日。", "先计算 成份广度扩散 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008557 - 多雨定制动量成份广度扩散策略
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.52 * VARIANT_WEIGHT * _zscore(up_ratio, 5) + 0.26 * VARIANT_WEIGHT * _zscore(new_high_ratio, 21) - _zscore(dispersion, 21).abs()
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 成份广度扩散_v4 | style: 动量 | seed: 20290213
