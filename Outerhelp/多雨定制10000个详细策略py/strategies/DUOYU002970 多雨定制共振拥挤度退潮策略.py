...
STRATEGY_ID = "DUOYU002970"
STRATEGY_NAME = "多雨定制共振拥挤度退潮策略"
SOURCE_FACTOR_ID = "DUOYU002970"
CATEGORY = "拥挤度退潮"
STYLE = "共振"
OPERATOR_ID = "拥挤度退潮_v5"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 14
FAST_WINDOW = 10
SLOW_WINDOW = 19
VARIANT_WEIGHT = 0.91
REQUIRED_FIELDS = ["attention_score", "close", "turnover"]

ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "关注度指标对新闻事件过度敏感。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 14 个交易日。", "先计算 拥挤度退潮 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU002970 - 多雨定制共振拥挤度退潮策略

策略主题：拥挤度退潮 / 共振
适用范围：宽基/行业/主题
主要周期：14 个交易日
必需字段：attention_score, close, turnover

经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    attention = _field(panel, "attention_score")
    turnover = _field(panel, "turnover")
    weakening = (_ret(close, 10) < 0).astype(float)
    raw = -_zscore(attention, 10) * weakening - 0.19 * VARIANT_WEIGHT * _zscore(turnover, 10)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 拥挤度退潮_v5 | style: 共振 | seed: 20255514
