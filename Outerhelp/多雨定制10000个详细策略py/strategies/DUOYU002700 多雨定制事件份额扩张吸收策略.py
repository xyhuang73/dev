...
STRATEGY_ID = "DUOYU002700"
STRATEGY_NAME = "多雨定制事件份额扩张吸收策略"
SOURCE_FACTOR_ID = "DUOYU002700"
CATEGORY = "份额扩张吸收"
STYLE = "事件"
OPERATOR_ID = "份额扩张吸收_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：份额扩张伴随价格韧性说明市场有效吸收了新增供给，而非纯粹追涨。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    resilience = _ret(close, 13) - _downside_vol(close, 13)
    raw = _diff(fund_share, 19) * resilience + 0.23 * VARIANT_WEIGHT * _amount_z(panel, 19)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 份额扩张吸收_v1 | style: 事件 | seed: 20255510
