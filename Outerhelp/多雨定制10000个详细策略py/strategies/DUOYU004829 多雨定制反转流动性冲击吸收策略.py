...
STRATEGY_ID = "DUOYU004829"
STRATEGY_NAME = "多雨定制反转流动性冲击吸收策略"
SOURCE_FACTOR_ID = "DUOYU004829"
CATEGORY = "流动性冲击吸收"
STYLE = "反转"
OPERATOR_ID = "流动性冲击吸收_v6"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 13
FAST_WINDOW = 11
SLOW_WINDOW = 14
VARIANT_WEIGHT = 0.95
REQUIRED_FIELDS = ["close", "turnover", "amount"]

ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "成交放量可能由事件驱动而非流动性变化。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 流动性冲击吸收 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU004829 - 多雨定制反转流动性冲击吸收策略

策略主题：流动性冲击吸收 / 反转
适用范围：宽基/行业/主题
主要周期：13 个交易日
必需字段：close, turnover, amount

经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：换手率突增伴随价格稳定=流动性吸收能力强，换手突增伴随价格下跌=流动性危机。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    shock = _zscore(turnover, 10) - _zscore(turnover, 16)
    absorption = _ret(close, 10) / (_zscore(turnover, 10).abs() + EPS)
    raw = 0.35 * VARIANT_WEIGHT * shock + 0.36 * VARIANT_WEIGHT * _zscore(absorption, 10)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 流动性冲击吸收_v6 | style: 反转 | seed: 20265415
