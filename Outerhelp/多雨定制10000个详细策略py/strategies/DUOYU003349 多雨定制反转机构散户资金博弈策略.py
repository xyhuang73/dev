...
STRATEGY_ID = "DUOYU003349"
STRATEGY_NAME = "多雨定制反转机构散户资金博弈策略"
SOURCE_FACTOR_ID = "DUOYU003349"
CATEGORY = "机构散户资金博弈"
STYLE = "反转"
OPERATOR_ID = "机构散户资金博弈_v4"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 17
FAST_WINDOW = 17
SLOW_WINDOW = 21
VARIANT_WEIGHT = 0.95
REQUIRED_FIELDS = ["close", "institutional_holding_change", "holder_count_change", "fund_share"]

ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "持有人数据频率低，信号更新不及时。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 17 个交易日。", "先计算 机构散户资金博弈 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU003349 - 多雨定制反转机构散户资金博弈策略

策略主题：机构散户资金博弈 / 反转
适用范围：宽基/行业
主要周期：17 个交易日
必需字段：close, institutional_holding_change, holder_count_change, fund_share

经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：机构增持+散户减持=聪明钱流入，机构减持+散户增持=聪明钱流出，捕捉资金结构分化。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _diff(series: pd.Series, window: int) -> pd.Series:
    w = max(1, int(window))
    return _by_etf(pd.to_numeric(series, errors="coerce"), lambda x: x.diff(w))
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    holder_change = _field(panel, "holder_count_change") if "holder_count_change" in panel.columns else _diff(_field(panel, "fund_share"), 24)
    smart_money = _inst_flow_divergence(_ret(close, 16), inst_change, 16)
    retail_pressure = -_zscore(holder_change, 24)
    raw = 0.46 * VARIANT_WEIGHT * smart_money + 0.27 * VARIANT_WEIGHT * retail_pressure + _ret(close, 24)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 机构散户资金博弈_v4 | style: 反转 | seed: 20257413
