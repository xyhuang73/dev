...
STRATEGY_ID = "DUOYU003069"
STRATEGY_NAME = "多雨定制反转拥挤度退潮策略"
SOURCE_FACTOR_ID = "DUOYU003069"
CATEGORY = "拥挤度退潮"
STYLE = "反转"
OPERATOR_ID = "拥挤度退潮_v10"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高关注度+价格转弱=拥挤退潮信号，反向捕捉过度拥挤后的均值回归。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    weakening = (_ret(close, 6) < 0).astype(float)
    raw = -_zscore(attention, 6) * weakening - 0.19 * VARIANT_WEIGHT * _zscore(turnover, 6)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 拥挤度退潮_v10 | style: 反转 | seed: 20255419
