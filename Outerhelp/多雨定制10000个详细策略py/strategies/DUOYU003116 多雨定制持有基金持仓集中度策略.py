...
STRATEGY_ID = "DUOYU003116"
STRATEGY_NAME = "多雨定制持有基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003116"
CATEGORY = "基金持仓集中度"
STYLE = "持有"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 基金持仓集中度_v2 | style: 持有 | seed: 20257111
