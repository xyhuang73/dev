...
STRATEGY_ID = "DUOYU001477"
STRATEGY_NAME = "多雨定制动量相对强弱动量策略"
SOURCE_FACTOR_ID = "DUOYU001477"
CATEGORY = "相对强弱动量"
STYLE = "动量"
OPERATOR_ID = "相对强弱动量_v10"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 15
FAST_WINDOW = 8
SLOW_WINDOW = 12
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["close", "index_close", "constituent_up_ratio"]

ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "基准指数选择偏差影响信号有效性。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 15 个交易日。", "先计算 相对强弱动量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU001477 - 多雨定制动量相对强弱动量策略

策略主题：相对强弱动量 / 动量
适用范围：宽基/行业
主要周期：15 个交易日
必需字段：close, index_close, constituent_up_ratio

经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    breadth = _field(panel, "constituent_up_ratio")
    raw = _ret(close, 15) - _ret(index_close, 15) + 0.41 * VARIANT_WEIGHT * _zscore(breadth, 13)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 相对强弱动量_v10 | style: 动量 | seed: 20248219
