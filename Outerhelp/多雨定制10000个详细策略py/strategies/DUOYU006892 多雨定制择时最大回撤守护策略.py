...
STRATEGY_ID = "DUOYU006892"
STRATEGY_NAME = "多雨定制择时最大回撤守护策略"
SOURCE_FACTOR_ID = "DUOYU006892"
CATEGORY = "最大回撤守护"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 最大回撤守护_v1 | style: 择时 | seed: 20278710
