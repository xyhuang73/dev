...
STRATEGY_ID = "DUOYU006020"
STRATEGY_NAME = "多雨定制事件隐含波动率偏斜策略"
SOURCE_FACTOR_ID = "DUOYU006020"
CATEGORY = "隐含波动率偏斜"
STYLE = "事件"
OPERATOR_ID = "隐含波动率偏斜_v5"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 5
SLOW_WINDOW = 16
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：隐含波动率下降+价格上涨=市场情绪改善，隐含波动率上升+价格下跌=恐慌。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    iv = _field(panel, "option_implied_vol") if "option_implied_vol" in panel.columns else _downside_vol(close, 18)
    iv_change = _diff(iv, 5)
    raw = -0.4 * VARIANT_WEIGHT * _zscore(iv_change, 5) + _ret(close, 18)
    raw = raw * _max_dd_guard(drawdown, -0.12)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 隐含波动率偏斜_v5 | style: 事件 | seed: 20273514
