...
STRATEGY_ID = "DUOYU008102"
STRATEGY_NAME = "多雨定制进攻股息除权效应策略"
SOURCE_FACTOR_ID = "DUOYU008102"
CATEGORY = "股息除权效应"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 股息除权效应_v3 | style: 进攻 | seed: 20285712
