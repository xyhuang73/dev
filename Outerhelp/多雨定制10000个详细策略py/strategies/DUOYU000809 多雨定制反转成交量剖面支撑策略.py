...
STRATEGY_ID = "DUOYU000809"
STRATEGY_NAME = "多雨定制反转成交量剖面支撑策略"
SOURCE_FACTOR_ID = "DUOYU000809"
CATEGORY = "成交量剖面支撑"
STYLE = "反转"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：基于 VWAP 锚定的成交量支撑信号，衡量价格位于成交量加权均价上方的强度。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：基于 VWAP 锚定的成交量支撑信号，衡量价格位于成交量加权均价上方的强度。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 成交量剖面支撑_v8 | style: 反转 | seed: 20244417
