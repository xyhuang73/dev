...
STRATEGY_ID = "DUOYU001462"
STRATEGY_NAME = "多雨定制进攻相对强弱动量策略"
SOURCE_FACTOR_ID = "DUOYU001462"
CATEGORY = "相对强弱动量"
STYLE = "进攻"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
RISK_NOTE_CN = "基准指数选择偏差影响信号有效性。 进攻风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：标的相对于基准指数的超额动量加成份广度确认，衡量相对强弱持续性。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 相对强弱动量_v10 | style: 进攻 | seed: 20246719
