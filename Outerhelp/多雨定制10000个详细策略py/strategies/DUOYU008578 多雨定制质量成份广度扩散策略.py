...
STRATEGY_ID = "DUOYU008578"
STRATEGY_NAME = "多雨定制质量成份广度扩散策略"
SOURCE_FACTOR_ID = "DUOYU008578"
CATEGORY = "成份广度扩散"
STYLE = "质量"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
RISK_NOTE_CN = "广度指标在风格极端偏移时可能失效。 质量风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 成份广度扩散_v5 | style: 质量 | seed: 20290314
