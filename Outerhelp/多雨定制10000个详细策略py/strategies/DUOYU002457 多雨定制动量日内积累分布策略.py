...
STRATEGY_ID = "DUOYU002457"
STRATEGY_NAME = "多雨定制动量日内积累分布策略"
SOURCE_FACTOR_ID = "DUOYU002457"
CATEGORY = "日内积累分布"
STYLE = "动量"
OPERATOR_ID = "日内积累分布_v9"
MARKET_SCOPE = "宽基/行业/主题"
HORIZON_DAYS = 14
FAST_WINDOW = 12
SLOW_WINDOW = 22
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["close", "open", "high", "low", "vwap", "amount"]

ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "极端行情下日内结构信号可能失效。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 14 个交易日。", "先计算 日内积累分布 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU002457 - 多雨定制动量日内积累分布策略

策略主题：日内积累分布 / 动量
适用范围：宽基/行业/主题
主要周期：14 个交易日
必需字段：close, open, high, low, vwap, amount

经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：基于日内价格位置和 VWAP 偏离的日内资金积累评估，衡量盘中承接力度。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _close_location(panel: pd.DataFrame) -> pd.Series:
    open_ = _field(panel, "open")
    high = _field(panel, "high")
    low = _field(panel, "low")
    close = _field(panel, "close")
    location = (close - low) / ((high - low).abs() + EPS) - 0.5
    body = (close - open_) / (open_.abs() + EPS)
    return location + 0.5 * body

def _vwap_support(panel: pd.DataFrame, window: int) -> pd.Series:
    close = _field(panel, "close")
    vwap = _field(panel, "vwap")
    return _zscore((close - vwap) / (vwap.abs() + EPS), window)
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    raw = _close_location(panel) + 0.32 * VARIANT_WEIGHT * _vwap_support(panel, 11) + 0.13 * VARIANT_WEIGHT * _amount_z(panel, 11)
    raw = raw * _sigmoid_gate(_close_location(panel), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 日内积累分布_v9 | style: 动量 | seed: 20253218
