...
STRATEGY_ID = "DUOYU004929"
STRATEGY_NAME = "多雨定制反转Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU004929"
CATEGORY = "Amihud 非流动性"
STYLE = "反转"
OPERATOR_ID = "amihud_非流动性_v2"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 6
FAST_WINDOW = 7
SLOW_WINDOW = 20
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
"""
DUOYU004929 - 多雨定制反转Amihud 非流动性策略
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    amihud = _zscore(ret_abs / (np.log1p(amount.clip(lower=0)) + EPS), 22)
    raw = -amihud + 0.19 * VARIANT_WEIGHT * _amount_z(panel, 7)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: amihud_非流动性_v2 | style: 反转 | seed: 20266411
