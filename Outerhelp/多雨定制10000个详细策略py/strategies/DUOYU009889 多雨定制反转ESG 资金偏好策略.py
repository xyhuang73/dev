...
STRATEGY_ID = "DUOYU009889"
STRATEGY_NAME = "多雨定制反转ESG 资金偏好策略"
SOURCE_FACTOR_ID = "DUOYU009889"
CATEGORY = "ESG 资金偏好"
STYLE = "反转"
OPERATOR_ID = "esg_资金偏好_v1"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 18
FAST_WINDOW = 12
SLOW_WINDOW = 23
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：高环保合规+资金流入=ESG 主题资金偏好，捕捉可持续投资趋势。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：高环保合规+资金流入=ESG 主题资金偏好，捕捉可持续投资趋势。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    esg_flow = _zscore(compliance, 26) * _zscore(net_flow, 11)
    raw = 0.3 * VARIANT_WEIGHT * _zscore(esg_flow, 11) + _ret(close, 26)
    score = _risk_scale(_cs_rank(raw), panel, 17)
    # instance: esg_资金偏好_v1 | style: 反转 | seed: 20300410
