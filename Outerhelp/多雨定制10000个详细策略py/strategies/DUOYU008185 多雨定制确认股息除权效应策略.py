...
STRATEGY_ID = "DUOYU008185"
STRATEGY_NAME = "多雨定制确认股息除权效应策略"
SOURCE_FACTOR_ID = "DUOYU008185"
CATEGORY = "股息除权效应"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
...
经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 股息除权效应_v7 | style: 确认 | seed: 20286016
