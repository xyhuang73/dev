...
STRATEGY_ID = "DUOYU006907"
STRATEGY_NAME = "多雨定制加速最大回撤守护策略"
SOURCE_FACTOR_ID = "DUOYU006907"
CATEGORY = "最大回撤守护"
STYLE = "加速"
OPERATOR_ID = "最大回撤守护_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    dd_score = _zscore(drawdown, 8)
    raw = _ret(close, 14) * _max_dd_guard(drawdown, -0.06) + 0.21 * VARIANT_WEIGHT * dd_score
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 最大回撤守护_v2 | style: 加速 | seed: 20278211
