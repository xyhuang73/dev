...
STRATEGY_ID = "DUOYU008335"
STRATEGY_NAME = "多雨定制轮动业绩超预期动量策略"
SOURCE_FACTOR_ID = "DUOYU008335"
CATEGORY = "业绩超预期动量"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "分析师预测存在系统性乐观偏差。 轮动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 业绩超预期动量_v7 | style: 轮动 | seed: 20288016
