...
STRATEGY_ID = "DUOYU009669"
STRATEGY_NAME = "多雨定制反转固定收益利差策略"
SOURCE_FACTOR_ID = "DUOYU009669"
CATEGORY = "固定收益利差"
STYLE = "反转"
OPERATOR_ID = "固定收益利差_v2"
MARKET_SCOPE = "债券/红利/低波"
HORIZON_DAYS = 14
FAST_WINDOW = 13
SLOW_WINDOW = 25
VARIANT_WEIGHT = 0.95
REQUIRED_FIELDS = ["close", "dividend_yield", "downside_vol"]

ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "利率变化对 carry 策略影响重大。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 14 个交易日。", "先计算 固定收益利差 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009669 - 多雨定制反转固定收益利差策略

策略主题：固定收益利差 / 反转
适用范围：债券/红利/低波
主要周期：14 个交易日
必需字段：close, dividend_yield, downside_vol

经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    carry = _zscore(dividend_yield, 28) - _downside_vol(close, 28)
    raw = 0.45 * VARIANT_WEIGHT * _zscore(carry, 12) + _ret(close, 28)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 固定收益利差_v2 | style: 反转 | seed: 20298411
