...
STRATEGY_ID = "DUOYU009745"
STRATEGY_NAME = "多雨定制确认固定收益利差策略"
SOURCE_FACTOR_ID = "DUOYU009745"
CATEGORY = "固定收益利差"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
RISK_NOTE_CN = "利率变化对 carry 策略影响重大。 确认风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：高票息/股息-低波动=优质 carry，适合固收类 标的 的持有收益评估。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 固定收益利差_v6 | style: 确认 | seed: 20298015
