...
STRATEGY_ID = "DUOYU007169"
STRATEGY_NAME = "多雨定制反转状态条件风险预算策略"
SOURCE_FACTOR_ID = "DUOYU007169"
CATEGORY = "状态条件风险预算"
STYLE = "反转"
OPERATOR_ID = "状态条件风险预算_v7"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 16
FAST_WINDOW = 16
SLOW_WINDOW = 19
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _ret(close, 21) - 0.27 * VARIANT_WEIGHT * _zscore(beta, 21).abs() - 0.24 * VARIANT_WEIGHT * _zscore(linkage, 21).abs() - 0.41 * VARIANT_WEIGHT * _zscore(downside, 21).abs()
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 状态条件风险预算_v7 | style: 反转 | seed: 20279416
