...
STRATEGY_ID = "DUOYU005614"
STRATEGY_NAME = "多雨定制集中跟踪质量策略"
SOURCE_FACTOR_ID = "DUOYU005614"
CATEGORY = "跟踪质量"
STYLE = "集中"
OPERATOR_ID = "跟踪质量_v1"
MARKET_SCOPE = "宽基/行业"
...
ECONOMIC_LOGIC_CN = "该策略以约 20 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
RISK_NOTE_CN = "跟踪误差数据频率可能低于价格数据。 集中风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 20 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 跟踪质量_v1 | style: 集中 | seed: 20270910
