...
STRATEGY_ID = "DUOYU007632"
STRATEGY_NAME = "多雨定制择时解禁到期压力策略"
SOURCE_FACTOR_ID = "DUOYU007632"
CATEGORY = "解禁到期压力"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：解禁比例大+临近解禁日=抛售压力大，解禁过后压力衰减。逆向思维：提前规避。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
RISK_NOTE_CN = "解禁计划可能变更或展期。 择时风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：解禁比例大+临近解禁日=抛售压力大，解禁过后压力衰减。逆向思维：提前规避。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 解禁到期压力_v7 | style: 择时 | seed: 20282716
