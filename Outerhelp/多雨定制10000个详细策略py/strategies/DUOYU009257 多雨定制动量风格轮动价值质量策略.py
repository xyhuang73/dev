...
STRATEGY_ID = "DUOYU009257"
STRATEGY_NAME = "多雨定制动量风格轮动价值质量策略"
SOURCE_FACTOR_ID = "DUOYU009257"
CATEGORY = "风格轮动价值质量"
STYLE = "动量"
OPERATOR_ID = "风格轮动价值质量_v5"
MARKET_SCOPE = "风格/宽基"
HORIZON_DAYS = 10
FAST_WINDOW = 15
SLOW_WINDOW = 16
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["valuation_percentile", "quality_score", "dividend_yield", "close"]

ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "风格因子在不同市场周期表现差异大。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 10 个交易日。", "先计算 风格轮动价值质量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009257 - 多雨定制动量风格轮动价值质量策略

策略主题：风格轮动价值质量 / 动量
适用范围：风格/宽基
主要周期：10 个交易日
必需字段：valuation_percentile, quality_score, dividend_yield, close

经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：低估值+高质量+高股息+价格动量=价值质量风格综合评分。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    valuation = _field(panel, "valuation_percentile")
    quality = _field(panel, "quality_score")
    dividend = _field(panel, "dividend_yield")
    raw = -0.36 * VARIANT_WEIGHT * _zscore(valuation, 20) + 0.2 * VARIANT_WEIGHT * _zscore(quality, 20) + 0.22 * VARIANT_WEIGHT * _zscore(dividend, 20) + _ret(close, 13)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 风格轮动价值质量_v5 | style: 动量 | seed: 20295214
