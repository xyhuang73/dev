...
STRATEGY_ID = "DUOYU000329"
STRATEGY_NAME = "多雨定制反转多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000329"
CATEGORY = "多时间框架动量"
STYLE = "反转"
OPERATOR_ID = "多时间框架动量_v6"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    ret_m = _ret(close, 10)
    ret_l = _ret(close, 20)
    raw = 0.39 * VARIANT_WEIGHT * ret_s + 0.44 * VARIANT_WEIGHT * ret_m + 0.17 * VARIANT_WEIGHT * ret_l - 0.28 * _downside_vol(close, 20)
    score = _risk_scale(_cs_rank(raw), panel, 15)
    # instance: 多时间框架动量_v6 | style: 反转 | seed: 20242415
