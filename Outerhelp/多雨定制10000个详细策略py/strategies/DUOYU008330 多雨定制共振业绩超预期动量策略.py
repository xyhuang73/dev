...
STRATEGY_ID = "DUOYU008330"
STRATEGY_NAME = "多雨定制共振业绩超预期动量策略"
SOURCE_FACTOR_ID = "DUOYU008330"
CATEGORY = "业绩超预期动量"
STYLE = "共振"
OPERATOR_ID = "业绩超预期动量_v7"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 13
FAST_WINDOW = 6
SLOW_WINDOW = 20
VARIANT_WEIGHT = 0.91
REQUIRED_FIELDS = ["close", "earnings_revision", "industry_momentum"]

ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "分析师预测存在系统性乐观偏差。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 业绩超预期动量 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU008330 - 多雨定制共振业绩超预期动量策略

策略主题：业绩超预期动量 / 共振
适用范围：行业/主题
主要周期：13 个交易日
必需字段：close, earnings_revision, industry_momentum

经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：盈利预测上调加速+行业动量+价格确认=业绩超预期驱动行情。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    earnings_revision = _field(panel, "earnings_revision")
    industry_momentum = _field(panel, "industry_momentum")
    surprise = _zscore(earnings_revision, 6) - _zscore(earnings_revision, 20)
    raw = 0.33 * VARIANT_WEIGHT * surprise + 0.15 * VARIANT_WEIGHT * _ret(industry_momentum, 6) + _ret(close, 6)
    raw = raw * _sigmoid_gate(surprise, 0, 4)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 业绩超预期动量_v7 | style: 共振 | seed: 20287516
