...
STRATEGY_ID = "DUOYU006668"
STRATEGY_NAME = "多雨定制减速回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006668"
CATEGORY = "回撤保护激活"
STYLE = "减速"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。"
RISK_NOTE_CN = "保护机制在V形底可能过早离场。 减速风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤保护激活_v6 | style: 减速 | seed: 20276315
