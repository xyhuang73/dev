...
STRATEGY_ID = "DUOYU009637"
STRATEGY_NAME = "多雨定制动量可转债套利策略"
SOURCE_FACTOR_ID = "DUOYU009637"
CATEGORY = "可转债套利"
STYLE = "动量"
OPERATOR_ID = "可转债套利_v6"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    arb_signal = -_zscore(premium, 7) * _sigmoid_gate(_zscore(net_flow, 7), 0, 3)
    raw = 0.38 * VARIANT_WEIGHT * arb_signal + _ret(close, 25)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 可转债套利_v6 | style: 动量 | seed: 20298215
