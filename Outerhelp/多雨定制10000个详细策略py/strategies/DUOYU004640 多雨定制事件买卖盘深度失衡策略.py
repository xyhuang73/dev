...
STRATEGY_ID = "DUOYU004640"
STRATEGY_NAME = "多雨定制事件买卖盘深度失衡策略"
SOURCE_FACTOR_ID = "DUOYU004640"
CATEGORY = "买卖盘深度失衡"
STYLE = "事件"
OPERATOR_ID = "买卖盘深度失衡_v5"
MARKET_SCOPE = "流动性较好"
HORIZON_DAYS = 10
FAST_WINDOW = 6
SLOW_WINDOW = 20
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    depth_imbalance = (_zscore(turnover, 6) - _zscore(spread, 6).abs()) / (1.0 + _zscore(spread, 22).abs())
    raw = 0.45 * VARIANT_WEIGHT * _zscore(depth_imbalance, 6) + _ret(close, 22)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 买卖盘深度失衡_v5 | style: 事件 | seed: 20265514
