...
STRATEGY_ID = "DUOYU006430"
STRATEGY_NAME = "多雨定制共振波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006430"
CATEGORY = "波动率的波动率"
STYLE = "共振"
OPERATOR_ID = "波动率的波动率_v2"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    close = _field(panel, "close")
    ret_1d = _ret(close, 1)
    vol = _by_etf(ret_1d.abs(), lambda x: x.rolling(17, min_periods=5).std())
    vol_of_vol = _by_etf(vol, lambda x: x.rolling(17, min_periods=5).std())
    raw = -0.36 * VARIANT_WEIGHT * _zscore(vol_of_vol, 6) + _ret(close, 17)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 波动率的波动率_v2 | style: 共振 | seed: 20275511
