...
STRATEGY_ID = "DUOYU009041"
STRATEGY_NAME = "多雨定制核心跨资产领先策略"
SOURCE_FACTOR_ID = "DUOYU009041"
CATEGORY = "跨资产领先"
STYLE = "核心"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：领先资产（如美股/商品）走势+风险偏好+自身价格确认，捕捉跨市场传导。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 跨资产领先_v1 | style: 核心 | seed: 20292610
