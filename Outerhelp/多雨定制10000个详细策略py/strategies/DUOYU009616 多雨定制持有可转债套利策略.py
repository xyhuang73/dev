...
STRATEGY_ID = "DUOYU009616"
STRATEGY_NAME = "多雨定制持有可转债套利策略"
SOURCE_FACTOR_ID = "DUOYU009616"
CATEGORY = "可转债套利"
STYLE = "持有"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：折价+资金流入=套利资金进场信号，捕捉 标的 价格向净值回归。 当前风格为[持有]（权重系数=0.72，DD敏感度=1.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 可转债套利_v5 | style: 持有 | seed: 20298114
