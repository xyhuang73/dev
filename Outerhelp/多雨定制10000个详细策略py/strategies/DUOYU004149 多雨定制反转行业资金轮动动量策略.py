...
STRATEGY_ID = "DUOYU004149"
STRATEGY_NAME = "多雨定制反转行业资金轮动动量策略"
SOURCE_FACTOR_ID = "DUOYU004149"
CATEGORY = "行业资金轮动动量"
STYLE = "反转"
OPERATOR_ID = "行业资金轮动动量_v8"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 6
FAST_WINDOW = 13
SLOW_WINDOW = 23
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：资金在行业间的轮动速度：短期资金流入加速的行业优于长期资金流入。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    sector_relative = _cs_rank(_zscore(net_flow, 12)) - _cs_rank(_zscore(net_flow, 26))
    raw = 0.35 * VARIANT_WEIGHT * sector_relative + _ret(close, 12)
    score = _risk_scale(_cs_rank(raw), panel, 6)
    # instance: 行业资金轮动动量_v8 | style: 反转 | seed: 20261417
