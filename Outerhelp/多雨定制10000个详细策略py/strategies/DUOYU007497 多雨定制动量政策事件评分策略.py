...
STRATEGY_ID = "DUOYU007497"
STRATEGY_NAME = "多雨定制动量政策事件评分策略"
SOURCE_FACTOR_ID = "DUOYU007497"
CATEGORY = "政策事件评分"
STYLE = "动量"
OPERATOR_ID = "政策事件评分_v7"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 11
FAST_WINDOW = 9
SLOW_WINDOW = 19
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：政策事件评分高+事件临近=政策利好预期，事件衰减后信号减弱。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = _zscore(policy, 8) * _event_proximity(distance, 10) + 0.12 * VARIANT_WEIGHT * _amount_z(panel, 8)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 政策事件评分_v7 | style: 动量 | seed: 20282216
