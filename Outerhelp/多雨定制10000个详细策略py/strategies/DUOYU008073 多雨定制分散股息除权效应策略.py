...
STRATEGY_ID = "DUOYU008073"
STRATEGY_NAME = "多雨定制分散股息除权效应策略"
SOURCE_FACTOR_ID = "DUOYU008073"
CATEGORY = "股息除权效应"
STYLE = "分散"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：高股息+除权日临近=抢权行情机会，除权后信号重置。 当前风格为[分散]（权重系数=0.77，DD敏感度=1.4）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 股息除权效应_v1 | style: 分散 | seed: 20286810
