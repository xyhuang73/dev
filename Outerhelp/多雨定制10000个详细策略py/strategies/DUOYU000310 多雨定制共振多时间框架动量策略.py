...
STRATEGY_ID = "DUOYU000310"
STRATEGY_NAME = "多雨定制共振多时间框架动量策略"
SOURCE_FACTOR_ID = "DUOYU000310"
CATEGORY = "多时间框架动量"
STYLE = "共振"
OPERATOR_ID = "多时间框架动量_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：短中长三时间维度动量加权，捕捉不同周期趋势共振，短周期灵敏、长周期稳健。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    ret_m = _ret(close, 18)
    ret_l = _ret(close, 20)
    raw = 0.36 * VARIANT_WEIGHT * ret_s + 0.5 * VARIANT_WEIGHT * ret_m + 0.33 * VARIANT_WEIGHT * ret_l - 0.34 * _downside_vol(close, 20)
    raw = raw * _sigmoid_gate(_ret(close, 18), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 多时间框架动量_v5 | style: 共振 | seed: 20242514
