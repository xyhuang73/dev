...
STRATEGY_ID = "DUOYU008798"
STRATEGY_NAME = "多雨定制质量防御切换策略"
SOURCE_FACTOR_ID = "DUOYU008798"
CATEGORY = "防御切换"
STYLE = "质量"
OPERATOR_ID = "防御切换_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。"
RISK_NOTE_CN = "防御切换可能错失反弹行情。 质量风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：风险偏好下降+股息高+下行波动低=防御性切换信号，适合避险环境。 当前风格为[质量]（权重系数=0.85，DD敏感度=1.2）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 防御切换_v2 | style: 质量 | seed: 20292311
