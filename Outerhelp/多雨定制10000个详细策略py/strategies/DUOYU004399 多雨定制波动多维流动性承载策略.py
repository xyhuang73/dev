...
STRATEGY_ID = "DUOYU004399"
STRATEGY_NAME = "多雨定制波动多维流动性承载策略"
SOURCE_FACTOR_ID = "DUOYU004399"
CATEGORY = "多维流动性承载"
STYLE = "波动"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。"
RISK_NOTE_CN = "流动性指标在极端行情中可能突变。 波动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：成交额+换手率奖励流动性，买卖价差惩罚交易成本，综合衡量 标的 承载能力。 当前风格为[波动]（权重系数=0.78，DD敏感度=1.3）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    score = _cs_rank(raw)
    # instance: 多维流动性承载_v2 | style: 波动 | seed: 20264411
