...
STRATEGY_ID = "DUOYU009020"
STRATEGY_NAME = "多雨定制事件折溢价修复策略"
SOURCE_FACTOR_ID = "DUOYU009020"
CATEGORY = "折溢价修复"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：折价标的+净值上涨+价格回升=折价修复机会，溢价过高 标的 规避。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 折溢价修复_v6 | style: 事件 | seed: 20293515
