...
STRATEGY_ID = "DUOYU001117"
STRATEGY_NAME = "多雨定制动量三重滤网组合策略"
SOURCE_FACTOR_ID = "DUOYU001117"
CATEGORY = "三重滤网组合"
STYLE = "动量"
OPERATOR_ID = "三重滤网组合_v2"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：趋势、波动、资金流三重滤网叠加，降低单一维度误判。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    vol_score = -_downside_vol(close, 13)
    flow_confirm = _zscore(_field(panel, "net_flow"), 10) if "net_flow" in panel.columns else amount_z
    raw = 0.4 * VARIANT_WEIGHT * trend_score + 0.25 * VARIANT_WEIGHT * vol_score + 0.17 * VARIANT_WEIGHT * flow_confirm
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 三重滤网组合_v2 | style: 动量 | seed: 20247211
