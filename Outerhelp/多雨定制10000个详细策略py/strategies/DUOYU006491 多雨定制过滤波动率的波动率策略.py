...
STRATEGY_ID = "DUOYU006491"
STRATEGY_NAME = "多雨定制过滤波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006491"
CATEGORY = "波动率的波动率"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v5 | style: 过滤 | seed: 20275614
