...
STRATEGY_ID = "DUOYU005742"
STRATEGY_NAME = "多雨定制进攻跟踪质量策略"
SOURCE_FACTOR_ID = "DUOYU005742"
CATEGORY = "跟踪质量"
STYLE = "进攻"
OPERATOR_ID = "跟踪质量_v8"
MARKET_SCOPE = "宽基/行业"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。"
RISK_NOTE_CN = "跟踪误差数据频率可能低于价格数据。 进攻风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[进攻]（权重系数=1.18，DD敏感度=0.7）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 跟踪质量_v8 | style: 进攻 | seed: 20269717
