...
STRATEGY_ID = "DUOYU002617"
STRATEGY_NAME = "多雨定制动量资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002617"
CATEGORY = "资金流确认强度"
STYLE = "动量"
OPERATOR_ID = "资金流确认强度_v7"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.46 * VARIANT_WEIGHT * _zscore(net_flow, 8) + _ret(close, 26) + 0.21 * VARIANT_WEIGHT * _diff(fund_share, 26)
    raw = raw * _sigmoid_gate(_zscore(net_flow, 8), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 资金流确认强度_v7 | style: 动量 | seed: 20254216
