...
STRATEGY_ID = "DUOYU002648"
STRATEGY_NAME = "多雨定制减速资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002648"
CATEGORY = "资金流确认强度"
STYLE = "减速"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。"
RISK_NOTE_CN = "大额申赎可能导致资金流指标短期失真。 减速风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 资金流确认强度_v9 | style: 减速 | seed: 20253318
