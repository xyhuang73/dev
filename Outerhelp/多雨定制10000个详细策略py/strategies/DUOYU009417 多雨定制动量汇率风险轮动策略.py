...
STRATEGY_ID = "DUOYU009417"
STRATEGY_NAME = "多雨定制动量汇率风险轮动策略"
SOURCE_FACTOR_ID = "DUOYU009417"
CATEGORY = "汇率风险轮动"
STYLE = "动量"
OPERATOR_ID = "汇率风险轮动_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.24 * VARIANT_WEIGHT * _zscore(currency_impact, 12) + _ret(nav, 20)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 汇率风险轮动_v1 | style: 动量 | seed: 20297210
