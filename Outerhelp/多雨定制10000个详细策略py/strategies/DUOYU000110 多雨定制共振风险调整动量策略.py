...
STRATEGY_ID = "DUOYU000110"
STRATEGY_NAME = "多雨定制共振风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000110"
CATEGORY = "风险调整动量"
STYLE = "共振"
OPERATOR_ID = "风险调整动量_v6"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    raw = _ret(close, 14) - 0.5 * VARIANT_WEIGHT * _downside_vol(close, 14) + 0.17 * VARIANT_WEIGHT * _amount_z(panel, 12)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 风险调整动量_v6 | style: 共振 | seed: 20241515
