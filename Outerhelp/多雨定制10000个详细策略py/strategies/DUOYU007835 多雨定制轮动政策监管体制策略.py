...
STRATEGY_ID = "DUOYU007835"
STRATEGY_NAME = "多雨定制轮动政策监管体制策略"
SOURCE_FACTOR_ID = "DUOYU007835"
CATEGORY = "政策监管体制"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "政策情绪 NLP 模型对措辞变化敏感。 轮动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 政策监管体制_v3 | style: 轮动 | seed: 20285012
