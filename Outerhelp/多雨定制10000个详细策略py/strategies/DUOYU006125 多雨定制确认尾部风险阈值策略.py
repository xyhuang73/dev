...
STRATEGY_ID = "DUOYU006125"
STRATEGY_NAME = "多雨定制确认尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006125"
CATEGORY = "尾部风险阈值"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 9 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
...
经济逻辑：
该策略以约 9 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 尾部风险阈值_v3 | style: 确认 | seed: 20273012
