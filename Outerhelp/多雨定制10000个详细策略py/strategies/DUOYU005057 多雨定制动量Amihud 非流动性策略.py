...
STRATEGY_ID = "DUOYU005057"
STRATEGY_NAME = "多雨定制动量Amihud 非流动性策略"
SOURCE_FACTOR_ID = "DUOYU005057"
CATEGORY = "Amihud 非流动性"
STYLE = "动量"
OPERATOR_ID = "amihud_非流动性_v8"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 16
FAST_WINDOW = 14
SLOW_WINDOW = 22
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "Amihud 在小盘 标的 中噪声较大。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 16 个交易日。", "先计算 Amihud 非流动性 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU005057 - 多雨定制动量Amihud 非流动性策略
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：Amihud 测度：单位成交额引起的价格冲击，值越低流动性越好。逆向使用：低冲击 = 优。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    amihud = _zscore(ret_abs / (np.log1p(amount.clip(lower=0)) + EPS), 26)
    raw = -amihud + 0.33 * VARIANT_WEIGHT * _amount_z(panel, 12)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: amihud_非流动性_v8 | style: 动量 | seed: 20267217
