...
STRATEGY_ID = "DUOYU009389"
STRATEGY_NAME = "多雨定制反转商品联动对冲策略"
SOURCE_FACTOR_ID = "DUOYU009389"
CATEGORY = "商品联动对冲"
STYLE = "反转"
OPERATOR_ID = "商品联动对冲_v6"
MARKET_SCOPE = "跨境/商品/场内"
HORIZON_DAYS = 15
FAST_WINDOW = 13
SLOW_WINDOW = 18
VARIANT_WEIGHT = 0.95
REQUIRED_FIELDS = ["close", "lead_asset_return"]

ECONOMIC_LOGIC_CN = "该策略以约 15 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "商品 标的 受 contango/backwardation 影响。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 15 个交易日。", "先计算 商品联动对冲 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU009389 - 多雨定制反转商品联动对冲策略

策略主题：商品联动对冲 / 反转
适用范围：跨境/商品/场内
主要周期：15 个交易日
必需字段：close, lead_asset_return

经济逻辑：
该策略以约 15 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    lead_asset = _field(panel, "lead_asset_return")
    commodity_link = _zscore(lead_asset, 12) * _ret(close, 12)
    raw = 0.29 * VARIANT_WEIGHT * commodity_link + _ret(close, 20)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 商品联动对冲_v6 | style: 反转 | seed: 20295415
