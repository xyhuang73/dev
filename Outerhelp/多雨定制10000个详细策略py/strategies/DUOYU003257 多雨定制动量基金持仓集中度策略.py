...
STRATEGY_ID = "DUOYU003257"
STRATEGY_NAME = "多雨定制动量基金持仓集中度策略"
SOURCE_FACTOR_ID = "DUOYU003257"
CATEGORY = "基金持仓集中度"
STYLE = "动量"
OPERATOR_ID = "基金持仓集中度_v9"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 20
FAST_WINDOW = 15
SLOW_WINDOW = 24
VARIANT_WEIGHT = 1.02
REQUIRED_FIELDS = ["close", "institutional_holding_pct", "institutional_holding_change", "top10_concentration"]

ECONOMIC_LOGIC_CN = "该策略以约 20 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "机构持仓数据季度披露存在严重滞后。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 20 个交易日。", "先计算 基金持仓集中度 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU003257 - 多雨定制动量基金持仓集中度策略

策略主题：基金持仓集中度 / 动量
适用范围：宽基/行业
主要周期：20 个交易日
必需字段：close, institutional_holding_pct, institutional_holding_change, top10_concentration

经济逻辑：
该策略以约 20 个交易日为主要持有观察期，核心逻辑是：机构持仓高+增持+适度分散=优质持仓结构。机构与价格背离时逆向捕捉。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _sigmoid_gate(series: pd.Series, center: float = 0.0, steepness: float = 5.0) -> pd.Series:
    """平滑二值门控，用于状态依赖项。"""
    x = pd.to_numeric(series, errors="coerce")
    return 1.0 / (1.0 + np.exp(-steepness * (x - center)))

def _inst_flow_divergence(price_change: pd.Series, inst_holding_change: pd.Series, window: int) -> pd.Series:
    """机构持仓变化与价格变化背离：机构增持+价格弱=聪明钱积累信号。"""
    price_z = _zscore(price_change, window)
    inst_z = _zscore(inst_holding_change, window)
    return (inst_z - price_z).replace([np.inf, -np.inf], np.nan)

def _concentration_risk(top10_conc: pd.Series, window: int) -> pd.Series:
    """持仓集中度风险：Top10 占比越高，特质风险越大。"""
    return _zscore(pd.to_numeric(top10_conc, errors="coerce"), window)
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    inst_holding = _field(panel, "institutional_holding_pct")
    inst_change = _field(panel, "institutional_holding_change")
    top10_conc = _field(panel, "top10_concentration")
    divergence = _inst_flow_divergence(_ret(_field(panel, "close"), 13), inst_change, 13)
    raw = 0.29 * VARIANT_WEIGHT * _zscore(inst_holding, 29) + 0.44 * VARIANT_WEIGHT * divergence - 0.28 * VARIANT_WEIGHT * _concentration_risk(top10_conc, 29)
    raw = raw * _sigmoid_gate(_inst_flow_divergence(_ret(close, 13), inst_change, 13), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 19)
    # instance: 基金持仓集中度_v9 | style: 动量 | seed: 20257218
