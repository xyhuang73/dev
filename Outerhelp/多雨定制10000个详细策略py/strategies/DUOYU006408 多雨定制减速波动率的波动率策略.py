...
STRATEGY_ID = "DUOYU006408"
STRATEGY_NAME = "多雨定制减速波动率的波动率策略"
SOURCE_FACTOR_ID = "DUOYU006408"
CATEGORY = "波动率的波动率"
STYLE = "减速"
...
ECONOMIC_LOGIC_CN = "该策略以约 6 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。"
RISK_NOTE_CN = "vol-of-vol 估计对参数敏感。 减速风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 6 个交易日为主要持有观察期，核心逻辑是：波动率的波动率高=不确定性高=信号噪声大，降低信号置信度。 当前风格为[减速]（权重系数=0.6，DD敏感度=1.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 波动率的波动率_v1 | style: 减速 | seed: 20275310
