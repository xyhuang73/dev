...
STRATEGY_ID = "DUOYU008712"
STRATEGY_NAME = "多雨定制择时日历效应季节策略"
SOURCE_FACTOR_ID = "DUOYU008712"
CATEGORY = "日历效应季节"
STYLE = "择时"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：日历效应（季节性规律）+成交确认，捕捉每年固定时间窗口的统计规律。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。"
RISK_NOTE_CN = "日历效应可能随市场结构变化而漂移。 择时风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：日历效应（季节性规律）+成交确认，捕捉每年固定时间窗口的统计规律。 当前风格为[择时]（权重系数=0.96，DD敏感度=0.8）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 日历效应季节_v5 | style: 择时 | seed: 20290714
