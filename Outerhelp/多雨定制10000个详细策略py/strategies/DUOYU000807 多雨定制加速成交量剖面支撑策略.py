...
STRATEGY_ID = "DUOYU000807"
STRATEGY_NAME = "多雨定制加速成交量剖面支撑策略"
SOURCE_FACTOR_ID = "DUOYU000807"
CATEGORY = "成交量剖面支撑"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：基于 VWAP 锚定的成交量支撑信号，衡量价格位于成交量加权均价上方的强度。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：基于 VWAP 锚定的成交量支撑信号，衡量价格位于成交量加权均价上方的强度。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 成交量剖面支撑_v8 | style: 加速 | seed: 20244217
