...
STRATEGY_ID = "DUOYU006900"
STRATEGY_NAME = "多雨定制事件最大回撤守护策略"
SOURCE_FACTOR_ID = "DUOYU006900"
CATEGORY = "最大回撤守护"
STYLE = "事件"
OPERATOR_ID = "最大回撤守护_v1"
...
ECONOMIC_LOGIC_CN = "该策略以约 7 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 7 个交易日为主要持有观察期，核心逻辑是：回撤深度进入危险区域时关闭信号，控制策略最大回撤在中低水平。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    rolling_peak = _rolling_max(close, 24)
    drawdown = close / (rolling_peak.abs() + EPS) - 1.0
    dd_score = _zscore(drawdown, 6)
    raw = _ret(close, 24) * _max_dd_guard(drawdown, -0.16) + 0.31 * VARIANT_WEIGHT * dd_score
    score = _risk_scale(_cs_rank(raw), panel, 7)
    # instance: 最大回撤守护_v1 | style: 事件 | seed: 20279510
