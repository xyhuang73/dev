...
STRATEGY_ID = "DUOYU007970"
STRATEGY_NAME = "多雨定制共振指数调整前置策略"
SOURCE_FACTOR_ID = "DUOYU007970"
CATEGORY = "指数调整前置"
STYLE = "共振"
OPERATOR_ID = "指数调整前置_v3"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 11
FAST_WINDOW = 8
SLOW_WINDOW = 14
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：指数调仓日前资金提前布局=前置交易机会，成交放量+临近调整日=高概率事件。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    frontrun_score = _event_proximity(rebalance_dist, 11) * _amount_z(panel, 8)
    raw = 0.42 * VARIANT_WEIGHT * _zscore(frontrun_score, 8) + _ret(close, 14)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 指数调整前置_v3 | style: 共振 | seed: 20285512
