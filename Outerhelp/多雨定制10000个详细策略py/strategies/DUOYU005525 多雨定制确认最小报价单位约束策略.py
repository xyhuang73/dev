...
STRATEGY_ID = "DUOYU005525"
STRATEGY_NAME = "多雨定制确认最小报价单位约束策略"
SOURCE_FACTOR_ID = "DUOYU005525"
CATEGORY = "最小报价单位约束"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 8 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
...
经济逻辑：
该策略以约 8 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 最小报价单位约束_v5 | style: 确认 | seed: 20269014
