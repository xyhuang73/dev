...
STRATEGY_ID = "DUOYU009830"
STRATEGY_NAME = "多雨定制共振低波动异象策略"
SOURCE_FACTOR_ID = "DUOYU009830"
CATEGORY = "低波动异象"
STYLE = "共振"
OPERATOR_ID = "低波动异象_v4"
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    quality = _field(panel, "quality_score") if "quality_score" in panel.columns else _zscore(_field(panel, "dividend_yield"), 22)
    raw = -0.71 * VARIANT_WEIGHT * _downside_vol(close, 22) + 0.3 * VARIANT_WEIGHT * _zscore(quality, 22) + _ret(close, 22)
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 低波动异象_v4 | style: 共振 | seed: 20299513
