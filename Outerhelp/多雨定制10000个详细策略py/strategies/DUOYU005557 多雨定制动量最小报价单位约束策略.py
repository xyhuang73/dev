...
STRATEGY_ID = "DUOYU005557"
STRATEGY_NAME = "多雨定制动量最小报价单位约束策略"
SOURCE_FACTOR_ID = "DUOYU005557"
CATEGORY = "最小报价单位约束"
STYLE = "动量"
OPERATOR_ID = "最小报价单位约束_v6"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 11
FAST_WINDOW = 12
SLOW_WINDOW = 16
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：高价 标的 受 tick size 约束价差占比低，低价 标的 约束大，调整后衡量真实交易成本。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = -0.4 * VARIANT_WEIGHT * _zscore(tick_constraint, 19) + 0.12 * VARIANT_WEIGHT * _amount_z(panel, 14)
    score = _risk_scale(_cs_rank(raw), panel, 10)
    # instance: 最小报价单位约束_v6 | style: 动量 | seed: 20270215
