...
STRATEGY_ID = "DUOYU007829"
STRATEGY_NAME = "多雨定制反转政策监管体制策略"
SOURCE_FACTOR_ID = "DUOYU007829"
CATEGORY = "政策监管体制"
STYLE = "反转"
OPERATOR_ID = "政策监管体制_v3"
MARKET_SCOPE = "行业/主题"
HORIZON_DAYS = 17
FAST_WINDOW = 16
SLOW_WINDOW = 18
VARIANT_WEIGHT = 0.95
REQUIRED_FIELDS = ["close", "regulatory_risk_index", "policy_sentiment_score", "industry_policy_cycle"]

ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "政策情绪 NLP 模型对措辞变化敏感。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 17 个交易日。", "先计算 政策监管体制 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU007829 - 多雨定制反转政策监管体制策略

策略主题：政策监管体制 / 反转
适用范围：行业/主题
主要周期：17 个交易日
必需字段：close, regulatory_risk_index, policy_sentiment_score, industry_policy_cycle

经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：监管风险低+政策情绪正面+行业政策扩张期=政策顺风，反之逆风。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    reg_risk = _field(panel, "regulatory_risk_index")
    policy_sent = _field(panel, "policy_sentiment_score") if "policy_sentiment_score" in panel.columns else -reg_risk
    policy_cycle = _field(panel, "industry_policy_cycle") if "industry_policy_cycle" in panel.columns else _zscore(policy_sent, 20)
    raw = -0.31 * VARIANT_WEIGHT * _zscore(reg_risk, 20) + 0.23 * VARIANT_WEIGHT * _zscore(policy_sent, 15) + 0.19 * VARIANT_WEIGHT * _zscore(policy_cycle, 20)
    score = _risk_scale(_cs_rank(raw), panel, 16)
    # instance: 政策监管体制_v3 | style: 反转 | seed: 20284412
