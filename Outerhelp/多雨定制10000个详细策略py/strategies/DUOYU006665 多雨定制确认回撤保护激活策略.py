...
STRATEGY_ID = "DUOYU006665"
STRATEGY_NAME = "多雨定制确认回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006665"
CATEGORY = "回撤保护激活"
STYLE = "确认"
...
ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。"
RISK_NOTE_CN = "保护机制在V形底可能过早离场。 确认风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[确认]（权重系数=1.03，DD敏感度=0.9）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤保护激活_v6 | style: 确认 | seed: 20276015
