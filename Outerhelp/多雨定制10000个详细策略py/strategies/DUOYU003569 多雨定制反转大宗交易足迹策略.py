...
STRATEGY_ID = "DUOYU003569"
STRATEGY_NAME = "多雨定制反转大宗交易足迹策略"
SOURCE_FACTOR_ID = "DUOYU003569"
CATEGORY = "大宗交易足迹"
STYLE = "反转"
OPERATOR_ID = "大宗交易足迹_v6"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 12
FAST_WINDOW = 15
SLOW_WINDOW = 24
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：大宗交易溢价/折价反映机构投资者对当前价格的判断，溢价=看好、折价=看淡。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "大宗交易数据覆盖面和及时性有限。 反转风格下回撤风险相对较高。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：大宗交易溢价/折价反映机构投资者对当前价格的判断，溢价=看好、折价=看淡。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    block_premium = _field(panel, "block_trade_premium") if "block_trade_premium" in panel.columns else _zscore(_amount_z(panel, 14) - _amount_z(panel, 27), 14)
    raw = 0.34 * VARIANT_WEIGHT * _zscore(block_premium, 14) + _ret(close, 27)
    score = _risk_scale(_cs_rank(raw), panel, 11)
    # instance: 大宗交易足迹_v6 | style: 反转 | seed: 20258415
