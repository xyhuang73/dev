...
STRATEGY_ID = "DUOYU001037"
STRATEGY_NAME = "多雨定制动量伽马突破策略"
SOURCE_FACTOR_ID = "DUOYU001037"
CATEGORY = "伽马突破"
STYLE = "动量"
OPERATOR_ID = "伽马突破_v8"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：低波动压缩后突破信号，捕捉波动率从收缩到扩张的临界点。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    vol_compression = 1.0 - _cs_rank(_zscore(true_range, 26).abs()).abs()
    breakout = _breakout(panel, 5)
    raw = breakout * vol_compression * 1.04 * VARIANT_WEIGHT + 0.22 * VARIANT_WEIGHT * _amount_z(panel, 5)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 伽马突破_v8 | style: 动量 | seed: 20246217
