...
STRATEGY_ID = "DUOYU003837"
STRATEGY_NAME = "多雨定制动量北向资金泄漏策略"
SOURCE_FACTOR_ID = "DUOYU003837"
CATEGORY = "北向资金泄漏"
STYLE = "动量"
OPERATOR_ID = "北向资金泄漏_v1"
MARKET_SCOPE = "宽基/行业"
HORIZON_DAYS = 14
FAST_WINDOW = 10
SLOW_WINDOW = 16
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    northbound = _field(panel, "northbound_flow") if "northbound_flow" in panel.columns else _diff(_field(panel, "fund_share"), 9)
    raw = 0.35 * VARIANT_WEIGHT * _zscore(northbound, 9) + _ret(close, 19)
    score = _risk_scale(_cs_rank(raw), panel, 12)
    # instance: 北向资金泄漏_v1 | style: 动量 | seed: 20261210
