...
STRATEGY_ID = "DUOYU007050"
STRATEGY_NAME = "多雨定制共振状态条件风险预算策略"
SOURCE_FACTOR_ID = "DUOYU007050"
CATEGORY = "状态条件风险预算"
STYLE = "共振"
OPERATOR_ID = "状态条件风险预算_v1"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 13
FAST_WINDOW = 16
SLOW_WINDOW = 28
VARIANT_WEIGHT = 0.91
REQUIRED_FIELDS = ["close", "beta_to_market", "market_linkage", "downside_vol"]

ECONOMIC_LOGIC_CN = "该策略以约 13 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。"
RISK_NOTE_CN = "风险预算参数依赖历史估计。 共振风格下回撤控制较为严格。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 13 个交易日。", "先计算 状态条件风险预算 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU007050 - 多雨定制共振状态条件风险预算策略

策略主题：状态条件风险预算 / 共振
适用范围：全市场
主要周期：13 个交易日
必需字段：close, beta_to_market, market_linkage, downside_vol

经济逻辑：
该策略以约 13 个交易日为主要持有观察期，核心逻辑是：回报减去系统性风险、联动风险和下行风险三项预算后的剩余 alpha。 当前风格为[共振]（权重系数=0.91，DD敏感度=1.05）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    beta = _field(panel, "beta_to_market")
    linkage = _field(panel, "market_linkage")
    downside = _field(panel, "downside_vol")
    raw = _ret(close, 28) - 0.34 * VARIANT_WEIGHT * _zscore(beta, 28).abs() - 0.23 * VARIANT_WEIGHT * _zscore(linkage, 28).abs() - 0.41 * VARIANT_WEIGHT * _zscore(downside, 28).abs()
    score = _risk_scale(_cs_rank(raw), panel, 13)
    # instance: 状态条件风险预算_v1 | style: 共振 | seed: 20279510
