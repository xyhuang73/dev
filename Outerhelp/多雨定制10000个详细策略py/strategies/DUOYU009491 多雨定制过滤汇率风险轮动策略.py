...
STRATEGY_ID = "DUOYU009491"
STRATEGY_NAME = "多雨定制过滤汇率风险轮动策略"
SOURCE_FACTOR_ID = "DUOYU009491"
CATEGORY = "汇率风险轮动"
STYLE = "过滤"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：价格回报与净值回报差异=汇率效应，捕捉汇率驱动的跨境 标的 机会。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 汇率风险轮动_v5 | style: 过滤 | seed: 20296614
