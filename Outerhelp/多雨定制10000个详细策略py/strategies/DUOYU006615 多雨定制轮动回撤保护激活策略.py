...
STRATEGY_ID = "DUOYU006615"
STRATEGY_NAME = "多雨定制轮动回撤保护激活策略"
SOURCE_FACTOR_ID = "DUOYU006615"
CATEGORY = "回撤保护激活"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：回撤超过阈值时激活保护性减分，回撤修复+资金回流时重新加分。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 回撤保护激活_v3 | style: 轮动 | seed: 20277012
