...
STRATEGY_ID = "DUOYU009364"
STRATEGY_NAME = "多雨定制修复商品联动对冲策略"
SOURCE_FACTOR_ID = "DUOYU009364"
CATEGORY = "商品联动对冲"
STYLE = "修复"
...
ECONOMIC_LOGIC_CN = "该策略以约 11 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。"
RISK_NOTE_CN = "商品 标的 受 contango/backwardation 影响。 修复风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 11 个交易日为主要持有观察期，核心逻辑是：商品价格领先 标的 价格的程度，适合商品 标的 的趋势跟随。 当前风格为[修复]（权重系数=0.86，DD敏感度=1.1）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 商品联动对冲_v5 | style: 修复 | seed: 20294914
