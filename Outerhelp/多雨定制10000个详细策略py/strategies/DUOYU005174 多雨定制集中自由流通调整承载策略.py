...
STRATEGY_ID = "DUOYU005174"
STRATEGY_NAME = "多雨定制集中自由流通调整承载策略"
SOURCE_FACTOR_ID = "DUOYU005174"
CATEGORY = "自由流通调整承载"
STYLE = "集中"
OPERATOR_ID = "自由流通调整承载_v5"
MARKET_SCOPE = "全市场"
HORIZON_DAYS = 10
FAST_WINDOW = 10
SLOW_WINDOW = 15
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。"
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：用换手/价差比近似自由流通调整后的真实流动性，高比值=实际可交易量大。 当前风格为[集中]（权重系数=1.1，DD敏感度=0.65）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    free_float_adj = _zscore(turnover, 19) / (1.0 + _zscore(spread, 19).abs())
    raw = 0.47 * VARIANT_WEIGHT * _zscore(free_float_adj, 8) + 0.28 * VARIANT_WEIGHT * _amount_z(panel, 8)
    score = _risk_scale(_cs_rank(raw), panel, 8)
    # instance: 自由流通调整承载_v5 | style: 集中 | seed: 20267914
