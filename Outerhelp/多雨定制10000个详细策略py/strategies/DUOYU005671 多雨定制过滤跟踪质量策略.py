...
STRATEGY_ID = "DUOYU005671"
STRATEGY_NAME = "多雨定制过滤跟踪质量策略"
SOURCE_FACTOR_ID = "DUOYU005671"
CATEGORY = "跟踪质量"
STYLE = "过滤"
OPERATOR_ID = "跟踪质量_v4"
MARKET_SCOPE = "宽基/行业"
...
ECONOMIC_LOGIC_CN = "该策略以约 12 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。"
...
经济逻辑：
该策略以约 12 个交易日为主要持有观察期，核心逻辑是：指数趋势好+跟踪误差低+跟踪偏离小=优质标的，综合评估跟踪质量。 当前风格为[过滤]（权重系数=0.8，DD敏感度=1.25）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 跟踪质量_v4 | style: 过滤 | seed: 20270613
