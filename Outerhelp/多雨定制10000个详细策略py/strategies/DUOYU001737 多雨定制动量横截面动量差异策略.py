...
STRATEGY_ID = "DUOYU001737"
STRATEGY_NAME = "多雨定制动量横截面动量差异策略"
SOURCE_FACTOR_ID = "DUOYU001737"
CATEGORY = "横截面动量差异"
STYLE = "动量"
OPERATOR_ID = "横截面动量差异_v3"
...
ECONOMIC_LOGIC_CN = "该策略以约 10 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
RISK_NOTE_CN = "高波动市场中动量差异噪声增大。 动量风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 10 个交易日。", "先计算 横截面动量差异 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU001737 - 多雨定制动量横截面动量差异策略
...
经济逻辑：
该策略以约 10 个交易日为主要持有观察期，核心逻辑是：快慢动量差异捕捉加速度变化，正差异=趋势加速，负差异=趋势减速。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    raw = 0.41 * VARIANT_WEIGHT * _zscore(momentum_diff, 4) + _ret(close, 18)
    raw = raw * _sigmoid_gate(_ret(close, 4), 0, 3)
    score = _risk_scale(_cs_rank(raw), panel, 9)
    # instance: 横截面动量差异_v3 | style: 动量 | seed: 20250212
