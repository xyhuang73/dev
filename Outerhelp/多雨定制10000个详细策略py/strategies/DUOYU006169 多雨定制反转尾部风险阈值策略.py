...
STRATEGY_ID = "DUOYU006169"
STRATEGY_NAME = "多雨定制反转尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006169"
CATEGORY = "尾部风险阈值"
STYLE = "反转"
OPERATOR_ID = "尾部风险阈值_v5"
...
ECONOMIC_LOGIC_CN = "该策略以约 19 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。"
RISK_NOTE_CN = "尾部风险模型依赖历史分布假设。 反转风格下回撤风险相对较高。"
DATA_NOTE_CN = "建议使用复权价格、交易日对齐后的净值/份额/指数/行业字段，避免未来函数和公告日错位。"

STRATEGY_RULES_CN = ["持有观察周期约 19 个交易日。", "先计算 尾部风险阈值 方向的多雨横截面分数。", "每日在标池内做横截面排序，分数越高，候选优先级越高。", "目标权重函数默认选择 Top N 标，并做单只权重上限约束。", "策略只输出分数、排名和目标权重，不负责交易执行。"]

"""
DUOYU006169 - 多雨定制反转尾部风险阈值策略
...
经济逻辑：
该策略以约 19 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[反转]（权重系数=0.95，DD敏感度=0.85）。
...
def _tail_risk_z(close: pd.Series, window: int, percentile: float = 0.05) -> pd.Series:
    """当前收益距历史尾部阈值的标准化距离。"""
    w = max(10, int(window))
    ret_1d = close.pct_change()
    tail = _by_etf(ret_1d, lambda x: x.rolling(w, min_periods=5).quantile(percentile))
    current = ret_1d
    return (current - tail) / (tail.abs() + EPS)
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    tail_z = _tail_risk_z(close, 26, 0.05)
    raw = 0.34 * VARIANT_WEIGHT * _zscore(tail_z, 5) + _ret(close, 26)
    raw = raw * _max_dd_guard(drawdown, -0.10)
    score = _risk_scale(_cs_rank(raw), panel, 18)
    # instance: 尾部风险阈值_v5 | style: 反转 | seed: 20273414
