...
STRATEGY_ID = "DUOYU008575"
STRATEGY_NAME = "多雨定制轮动成份广度扩散策略"
SOURCE_FACTOR_ID = "DUOYU008575"
CATEGORY = "成份广度扩散"
STYLE = "轮动"
...
ECONOMIC_LOGIC_CN = "该策略以约 17 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。"
RISK_NOTE_CN = "广度指标在风格极端偏移时可能失效。 轮动风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 17 个交易日为主要持有观察期，核心逻辑是：成份上涨比例高+创新高比例高+低离散=广谱健康上涨，非少数权重股拉动。 当前风格为[轮动]（权重系数=0.93，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 成份广度扩散_v5 | style: 轮动 | seed: 20290014
