...
STRATEGY_ID = "DUOYU000197"
STRATEGY_NAME = "多雨定制动量风险调整动量策略"
SOURCE_FACTOR_ID = "DUOYU000197"
CATEGORY = "风险调整动量"
STYLE = "动量"
OPERATOR_ID = "风险调整动量_v10"
...
ECONOMIC_LOGIC_CN = "该策略以约 16 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。"
...
经济逻辑：
该策略以约 16 个交易日为主要持有观察期，核心逻辑是：趋势强度扣除波动惩罚后的轮动优先级，适合作为基础评分锚。 当前风格为[动量]（权重系数=1.02，DD敏感度=0.75）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    """计算本策略的原始横截面分数。"""
    close = _field(panel, "close")
    raw = _ret(close, 18) - 0.45 * VARIANT_WEIGHT * _downside_vol(close, 18) + 0.22 * VARIANT_WEIGHT * _amount_z(panel, 13)
    score = _risk_scale(_cs_rank(raw), panel, 14)
    # instance: 风险调整动量_v10 | style: 动量 | seed: 20242219
