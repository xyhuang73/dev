...
STRATEGY_ID = "DUOYU002680"
STRATEGY_NAME = "多雨定制事件资金流确认强度策略"
SOURCE_FACTOR_ID = "DUOYU002680"
CATEGORY = "资金流确认强度"
STYLE = "事件"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：资金净流入确认价格趋势，份额变化确认趋势持续性，双重验证降低假突破概率。 当前风格为[事件]（权重系数=0.98，DD敏感度=0.72）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 资金流确认强度_v10 | style: 事件 | seed: 20254519
