...
STRATEGY_ID = "DUOYU006161"
STRATEGY_NAME = "多雨定制核心尾部风险阈值策略"
SOURCE_FACTOR_ID = "DUOYU006161"
CATEGORY = "尾部风险阈值"
STYLE = "核心"
...
ECONOMIC_LOGIC_CN = "该策略以约 18 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。"
RISK_NOTE_CN = "尾部风险模型依赖历史分布假设。 核心风格下回撤控制较为严格。"
...
经济逻辑：
该策略以约 18 个交易日为主要持有观察期，核心逻辑是：当前收益距离历史尾部阈值的距离，距尾部越远越安全。 当前风格为[核心]（权重系数=0.88，DD敏感度=1.0）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 尾部风险阈值_v5 | style: 核心 | seed: 20272614
