...
STRATEGY_ID = "DUOYU002027"
STRATEGY_NAME = "多雨定制加速路径效率动量策略"
SOURCE_FACTOR_ID = "DUOYU002027"
CATEGORY = "路径效率动量"
STYLE = "加速"
...
ECONOMIC_LOGIC_CN = "该策略以约 14 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。"
...
经济逻辑：
该策略以约 14 个交易日为主要持有观察期，核心逻辑是：Kaufman 效率比衡量趋势方向性 vs 路径噪声，高效率=趋势流畅、低效率=震荡。 当前风格为[加速]（权重系数=1.3，DD敏感度=0.55）。
...
def _compute_raw_score(panel: pd.DataFrame) -> pd.Series:
    ...
    # instance: 路径效率动量_v8 | style: 加速 | seed: 20250217
